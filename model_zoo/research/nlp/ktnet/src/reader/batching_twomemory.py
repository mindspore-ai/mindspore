...
def mask(batch_tokens, total_token_num, vocab_size, CLS=1, SEP=2, MASK=3):
    """
    Add mask for batch_tokens, return out, mask_labels, mask_pos;
    ...
    for sent_index, sent in enumerate(batch_tokens):
        ...
        for token_index, token in enumerate(sent):
            ...
            if prob > 0.15:
                ...
                if token not in (SEP, CLS):
                    mask_labels.append(sent[token_index])
        ...
        while not mask_flag:
            ...
            if sent[token_index] not in (SEP, CLS):
                mask_labels.append(sent[token_index])
                sent[token_index] = MASK
                mask_flag = True
                mask_pos.append(sent_index * max_len + token_index)
    mask_labels = np.array(mask_labels).astype("int64").reshape([-1, 1])
    mask_pos = np.array(mask_pos).astype("int64").reshape([-1, 1])
    return batch_tokens, mask_labels, mask_pos
...
def prepare_batch_data(insts,
    ...
    if mask_id >= 0:
        return_list = [src_id, pos_id, sent_id, wn_concept_ids, nell_concept_ids, self_input_mask, mask_label,
                       mask_pos] + labels_list
    else:
...
def pad_batch_data(insts,
    ...
    if isinstance(pad_idx, list):  # padding list, for concept_ids
        inst_data = np.array(
