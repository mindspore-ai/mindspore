...
class TemporalConvLayer(nn.Cell):
    ...
    def __init__(self, Kt, c_in, c_out, n_vertex, act_func):
        ...
        self.linear = nn.Dense(self.n_vertex, self.n_vertex).to_float(ms.float16)
...
class OutputBlock(nn.Cell):
    ...
    def __init__(self, Ko, last_block_channel, channels, end_channel, n_vertex, gated_act_func, drop_rate):
        ...
        self.fc1 = nn.Dense(self.channels[0], self.channels[1]).to_float(ms.float16)
        self.fc2 = nn.Dense(self.channels[1], self.end_channel).to_float(ms.float16)
