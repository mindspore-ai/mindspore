...
def train(opt, tr_dataloader, net, loss_fn, eval_loss_fn, optim, val_dataloader=None):
    '''
    train function
    '''
    ...
    eval_cb = EvalCallBack(opt, my_acc_cell, eval_data, local_train_url)
    config = CheckpointConfig(save_checkpoint_steps=10,
                              keep_checkpoint_max=5,
                              saved_network=net)
    ckpoint_cb = ModelCheckpoint(prefix='protonet', directory=local_train_url, config=config)
...
def main():
    ...
    train(options, tr_dataloader, Net, loss_fn, eval_loss_fn, optim, val_dataloader)
