...
class RPN(nn.Cell):
    ...
    def construct(self, inputs, img_metas, anchor_list, gt_bboxes, gt_labels, gt_valids):
        """Filter out boxes that may have targets

        Args:
            loss_print (tuple): loss
            rpn_cls_score (tuple): score of cls
            rpn_bbox_pred (tuple): prediction of bbox
            rpn_cls_score_total (tuple): total score of cls
            rpn_bbox_pred_total (tuple): total prediction of bbox

        Returns:
            tuple: A series of rectangular candidates
        """
        loss_print = ()
        rpn_cls_score = ()
        rpn_bbox_pred = ()
