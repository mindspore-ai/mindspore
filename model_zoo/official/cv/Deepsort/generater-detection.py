...
def extract_image_patch(image, bbox, patch_shape=None):
    ...
    if np.any(bbox[:2] >= bbox[2:]):
        return None
    sx, sy, ex, ey = bbox
    #print("图像二处理之后的坐标为：{}  {}  {}  {}".format(sx, sy, ex, ey))
...
class ImageEncoder:
    ...
    def _get_features(self, bbox_xywh, ori_img):
        ...
        if im_crops:
            #print("====0")
            #print("图片中人的数量：{}，第一个人的图片大小：{}".format(len(im_crops), im_crops[0].shape))
...
def create_box_encoder(model_filename, batch_size=32):
    ...
    def encoder_box(image, boxes):
        #features = get_features(boxes, image)
...
def generate_detections(encoder_boxes, mot_dir, output_dir, det_path=None, detection_dir=None):
    ...
    for sequence in os.listdir(mot_dir):
        ...
        detection_dir1 = os.path.join(det_path, sequence)
        detection_file = os.path.join(detection_dir1, "det/det.txt")
        #detection_file = os.path.join(detection_dir, "gt/gt.txt")
...
def parse_args():
    ...
    parser.add_argument('--train_url', type=str, default='', help='Train output directory.')
    parser.add_argument('--det_url', type=str, default='', help='Train output directory.')
    parser.add_argument('--batch_size', type=int, default=32, help='Batach size.')
    parser.add_argument("--ckpt_url", type=str, default='', help="Path to checkpoint.")
    parser.add_argument("--model_name", type=str, default="deepsort-30000_24.ckpt", help="Name of checkpoint.")
    parser.add_argument("--detection_dir", help="Path to custom detections. Defaults to", default=None)
...
if __name__ == "__main__":
    ...
    print("模型地址为：", ckpt_dir+args.model_name)
    encoder = create_box_encoder(ckpt_dir+args.model_name, batch_size=args.batch_size)
