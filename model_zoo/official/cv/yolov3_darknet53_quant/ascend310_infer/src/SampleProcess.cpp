...
Result SampleProcess::Process(const char *om_path, const char *input_folder) {
    ...
    float *inputShape = reinterpret_cast<float *>(malloc(sizeof(float) * 2));
    inputShape[0] = 416;
    inputShape[1] = 416;
    ...
    for (size_t index = 0; index < testFile.size(); ++index) {
        ...
        std::vector<void *> inputBuffers({picDevBuffer, inputShapeBuffer});
        std::vector<size_t> inputSizes({devBufferSize, 8});
