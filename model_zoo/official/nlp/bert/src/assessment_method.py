...
class F1():
    ...
    def eval(self, skip_channel=True):
        if self.mode.lower() == "binary":
            f1 = self.TP / (2 * self.TP + self.FP + self.FN)
        else:
            tp = np.sum(self.TP[1:]) if skip_channel else np.sum(self.TP)
            p = np.sum(self.P[1:]) if skip_channel else np.sum(self.P)
            ap = np.sum(self.AP[1:]) if skip_channel else np.sum(self.AP)
            f1 = 2 * tp / (ap + p)
        return f1
