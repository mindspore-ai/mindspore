...
class FlopsUtilizationCollector(Callback):
    ...
    Note:
        1. FlopsUtilizationCollector only counts the flops information of the
        following operators(including the corresponding backpropagation operators):
        MatMul, BatchMatMul, FlashAttentionScore, Conv2D
        2. Only support GRAPH_MODE.
        3. Not support Dynamic shape.

    Args:
        data_size (int): How many steps are the intervals between print information each time.
        computility (int): Whether to sow the average time of fetching data in Host.
        ...
        full_flops(bool): Whether to collect full model flops. If set full_flops to False,
            FlopsUtilizationCollector would collect shard model flops in each device. Default: ``True`` .

    Raises:
        TypeError: If data_size is not positive int.
        TypeError: If full_flops is not bool.
        AssertionError: If execute mode is not GRAPH_MODE.
    ...
    def epoch_end(self, run_context):
        ...
        if not self.mfu_calculated:
            ...
            if self.ma:
                ...
                with open(flops_path, 'a+') as f:
                    f.write(model_flops_log)
                    f.write(hardware_flops_log)
                if os.path.exists(self.time_step_path):
                    os.remove(self.time_step_path)
            if self.verbose:
        ...
        if self.ma:
            with open(self.time_step_path, 'w') as f:
                f.write(train_log + '\n')
        if self.verbose:
