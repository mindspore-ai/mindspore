...
def _restore_mutable_attr(args_list, compile_args):
    ...
    for idx in range(len(args_list)):
        # Currently the CSRTensor and COOTensor still keep the mutable attr and we don't support make them mutable.
        if isinstance(args_list[idx], (CSRTensor, COOTensor)):
