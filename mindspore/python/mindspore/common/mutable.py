...
def _check_element_type(value, depth):
    """Check if all the elements are valid."""
    depth += 1
    if depth == max_recursion:
        raise RuntimeError(f"For 'mutable', the input {type(value).__name__} nesting is more than "
                           f"maximum recursion depth {max_recursion}")

    if isinstance(value, (tuple, list)):
        for element in value:
            if not _check_element_type(element, depth):
                return False
        return True
...
def mutable(input_data, dynamic_len=False):
    ...
    """
    if not _check_element_type(input_data, 0):
        raise TypeError(
            f"For 'mutable', the 'input_data' should be one of (bool, int, float, Tensor, tuple, list, dict) "
            f"or their nested structures, but got {type(input_data).__name__}: {input_data}.")
