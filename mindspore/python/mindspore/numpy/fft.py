...
def fft(a, n=None, axis=-1, norm=None):
    ...
    Returns:
        ...
        If `n` is given, result.shape[axis] is `n`.
...
def ihfft(a, n=None, axis=-1, norm=None):
    ...
    Raises:
        TypeError: If the `a` type is not Tensor.
        TypeError: If the `a` data type is not one of: int32, int64, float32, float64.
        TypeError: If `n` or `axis` type is not int.
        ValueError: If `axis` is not in the range of "[ `-a.ndim` , `a.ndim` )".
