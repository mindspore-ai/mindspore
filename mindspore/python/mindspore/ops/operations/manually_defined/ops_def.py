...
def flash_attention_score(query, key, value, head_num, real_shift=None, drop_mask=None, padding_mask=None,
    ...
    Args:
        ...
        real_shift (Union[Tensor[float16, bfloat16], None]): Also known as pse. The position embedding code. If S
            ...
            The real_shift and shape formats are related when input_layout is `TND`.
            In the `(B, N1, 1024, S2)` and `(1, N1, 1024, S2) formats, sparse_mode must be 2. actual_seq_qlen and
            actual_seq_kvlen must be same. Max value of both actual_seq_qlen and actual_seq_klen must be larger
            than 1024.

            - (B, N1, S1, S2): If actual_seq_qlen is [x1, x2, x3] and actual_seq_kvlen is [y1, y2, y3], the sum of
              real_shift elements must be equal to ((x1 * y1 + (x2 - x1) * (y2 - y1) + (x3 - x2) * (y3 - y2))) * N1.
            - (1, N1, S1, S2): Similar to the previous format. The sum of real_shift elements must be equal to
              (y1 + (y2 - y1) + (y3 - y2)) * N1.
