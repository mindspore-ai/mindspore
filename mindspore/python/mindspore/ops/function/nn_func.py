...
def incre_flash_attention(query, key, value, attn_mask=None, actual_seq_lengths=None, pse_shift=None,
    ...
    Args:
        ...
        quant_offset2 (Tensor, optional): Quantitative parametor, the tensor with data type of float32. The shape is
            :math:`(1,)`. Default: ``None``.
        antiquant_scale (Tensor, optional): Quantitative parametor, the tensor with data type of float16 or bfloat16.
            The shape is :math:`(2, N, 1, D)` when input layout is 'BNSD' or :math:`(2, H)` when input layout is 'BSH'.
            Default: ``None``.
        antiquant_offset (Tensor, optional): Quantitative parametor, the tensor with data type of float16 or bfloat16.
            The shape is :math:`(2, N, 1, D)` when input layout is 'BNSD' or :math:`(2, H)` when input layout is 'BSH'.
            Default: ``None``.
        block_table (Tensor, optional): The tensor with data type of int32. The shape is :math:`(B, MaxBlockNumPerSeq)`,
            where :math:`MaxBlockNumPerSeq = ceil(\frac{max(actual\_seq\_length)}{block\_size} )`. Default: ``None``.
        num_heads  (int): The number of heads.
        input_layout (str): The data layout of the input qkv, support 'BSH' and 'BNSD'. Default ``'BSH'``.
        ...
        kv_padding_size (Tensor, optional): The tensor with data type of int64. The range of values is
            :math:`0\le kv\_padding\_size \le  S-max(actual\_seq\_length)`. The shape is :math:`(1,)`.
            Default: ``None``.
