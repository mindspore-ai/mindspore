...
def calculate_dyn_batch_gather(values, indices, x_shape, axis, batch_dims):
...
def calculate_batch_gather(values, indices, x_shape, axis, batch_dims):
    ...
    indices = indices + offset
    num_segments = batch_size * axis_dim
    params_grad = unsorted_segment_sum(values, indices, F.cast(num_segments, mstype.int64))
    grad_shape = shape_op(params_grad)
    ret_shape = F.hstack([Tensor(values_shape[:batch_dims], mstype.int64), F.cast(axis_dim, mstype.int64),
                          Tensor(grad_shape[1:], mstype.int64)])
    params_grad = reshape(params_grad, ret_shape)
    return params_grad
...
def get_bprop_gather_v2(self):
    ...
    def _dyn_bprop_gather_v2(x, indices, axis, dout):
        ...
        if batch_dims > 0:
            params_grad = calculate_dyn_batch_gather(values_transpose, indices, x_shp, axis, batch_dims)
        else:
    ...
    def bprop(x, indices, axis, out, dout):
        ...
        if F.is_sequence_value_unknown(shape_op(x)):
            if batch_dims > 0:
                params_grad = calculate_dyn_batch_gather(values_transpose, indices, dyn_shape_op(x), axis, batch_dims)
            else:
