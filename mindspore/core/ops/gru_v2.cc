...
std::unordered_map<std::string, int64_t> GetAttrMap(const PrimitivePtr &primitive) {
  ...
  attr_map["input_size"] = input_size;

  auto hidden_size_ptr = primitive->GetAttr("hidden_size");
  MS_EXCEPTION_IF_NULL(hidden_size_ptr);
  int64_t hidden_size = GetValue<int64_t>(hidden_size_ptr);
  attr_map["hidden_size"] = hidden_size;

  auto num_layers_ptr = primitive->GetAttr("num_layers");
  MS_EXCEPTION_IF_NULL(num_layers_ptr);
  int64_t num_layers = GetValue<int64_t>(num_layers_ptr);
  auto bidirectional_ptr = primitive->GetAttr("bidirectional");
  ...
  attr_map["real_num_layers"] = real_num_layers;
  attr_map["real_hidden_size"] = real_hidden_size;
  return attr_map;
}
