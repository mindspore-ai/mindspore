...
namespace ops {
namespace {
abstract::ShapePtr SparseDenseCwiseAddInferShape(const PrimitivePtr &primitive,
...
TypePtr SparseDenseCwiseAddInferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  auto prim_name = primitive->name();
...
AbstractBasePtr SparseDenseCwiseAddInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
                                         const std::vector<AbstractBasePtr> &input_args) {
  MS_EXCEPTION_IF_NULL(primitive);
  const int64_t kInputNum = 4;
  CheckAndConvertUtils::CheckInputArgs(input_args, kEqual, kInputNum, primitive->name());
  auto infer_type = SparseDenseCwiseAddInferType(primitive, input_args);
  auto infer_shape = SparseDenseCwiseAddInferShape(primitive, input_args);
  return abstract::MakeAbstract(infer_shape, infer_type);
}
REGISTER_PRIMITIVE_EVAL_IMPL(SparseDenseCwiseAdd, prim::kPrimSparseDenseCwiseAdd, SparseDenseCwiseAddInfer, nullptr,
                             true);
}  // namespace ops
}  // namespace mindspore
