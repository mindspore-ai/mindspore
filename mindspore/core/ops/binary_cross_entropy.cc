...
abstract::ShapePtr BinaryCrossEntroyInferShape(const PrimitivePtr &primitive,
  ...
  CheckAndConvertUtils::CheckInRange("binary_cross_entropy_infer", SizeToLong(input_args.size()), kIncludeBoth, {2, 3},
                                     prim_name);
...
TypePtr BinaryCrossEntroyInferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
  CheckAndConvertUtils::CheckInteger("binary_cross_entropy_infer", SizeToLong(input_args.size()), kEqual, 3,
                                     prim->name());
