...
namespace ops {
namespace {
abstract::ShapePtr InplaceSubInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
...
TypePtr InplaceSubInferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
...
std::vector<int64_t> InplaceSub::get_indices() {
...
AbstractBasePtr InplaceSubInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
                                const std::vector<AbstractBasePtr> &input_args) {
  return abstract::MakeAbstract(InplaceSubInferShape(primitive, input_args),
                                InplaceSubInferType(primitive, input_args));
}
REGISTER_PRIMITIVE_EVAL_IMPL(InplaceSub, prim::kPrimInplaceSub, InplaceSubInfer, nullptr, true);
}  // namespace ops
}  // namespace mindspore
