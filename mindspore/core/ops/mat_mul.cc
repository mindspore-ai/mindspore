...
abstract::ShapePtr MatMulInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  auto matmul_prim = primitive->cast<PrimMatMulPtr>();
  MS_EXCEPTION_IF_NULL(matmul_prim);
  auto prim_name = matmul_prim->name();
  ...
  auto trans_a = matmul_prim->get_transpose_a();
  auto trans_b = matmul_prim->get_transpose_b();
