...
abstract::TupleShapePtr DenseToDenseSetOperationInferShape(const PrimitivePtr &primitive,
  ...
  auto x1_shape_vec = x1_shape->GetShapeVector();
  auto x1_rank = SizeToLong(x1_shape_vec.size());
  auto x2_shape_vec = x2_shape->GetShapeVector();
  ...
  ShapeVector max_output_indices_vec = {max_num, output_rank_dim};
  ShapeVector max_output_values_vec = {max_num};
  ShapeVector output_shape_vec = {output_rank_dim};

  std::vector<abstract::BaseShapePtr> shape_tuple;
  abstract::ShapePtr output_indices_shape = std::make_shared<abstract::Shape>(max_output_indices_vec);
  abstract::ShapePtr output_values_shape = std::make_shared<abstract::Shape>(max_output_values_vec);
...
abstract::TupleShapePtr DenseToDenseSetOperationFrontendInferShape(const PrimitivePtr &primitive,
                                                                   const std::vector<AbstractBasePtr> &input_args) {
  auto prim_name = primitive->name();
  auto x1_shape = input_args[0]->GetShape();
  auto x2_shape = input_args[1]->GetShape();
...
AbstractBasePtr DenseToDenseSetOperationInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
  ...
  auto infershape = DenseToDenseSetOperationFrontendInferShape(primitive, input_args);
