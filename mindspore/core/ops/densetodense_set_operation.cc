...
abstract::TupleShapePtr DenseToDenseSetOperationInferShape(const PrimitivePtr &primitive,
  ...
  x1_num = std::accumulate(x1_shape_vec.begin(), x1_shape_vec.end(), 1, std::multiplies<int64_t>());
  x2_num = std::accumulate(x2_shape_vec.begin(), x2_shape_vec.end(), 1, std::multiplies<int64_t>());
  ...
  ShapeVector max_output_shape_vec = {output_rank_dim};

  ShapeVector min_output_indices_vec = {0, output_rank_dim};
  ShapeVector min_output_values_vec = {0};
  ShapeVector min_output_shape_vec = {output_rank_dim};
  ...
  abstract::ShapePtr output_shape =
    std::make_shared<abstract::Shape>(output_shape_vec, min_output_shape_vec, max_output_shape_vec);
