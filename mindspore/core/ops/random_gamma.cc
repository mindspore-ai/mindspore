...
abstract::ShapePtr GammaInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  auto shape_value_tensor =
    input_args[kInputIndex0]->cast<abstract::AbstractTensorPtr>()->BuildValue()->cast<tensor::TensorPtr>();
  ...
  if (true_value > max_value) {
    MS_EXCEPTION(ValueError) << "For RandomGamma"
...
void Gamma::Init(const int64_t seed, const int64_t seed2) {
  this->set_seed(seed);
  this->set_seed2(seed2);
}
int64_t Gamma::get_seed() const {
  auto value_ptr = this->GetAttr(kSeed);
  return GetValue<int64_t>(value_ptr);
}
void Gamma::set_seed(const int64_t seed) { (void)this->AddAttr(kSeed, api::MakeValue(seed)); }

int64_t Gamma::get_seed2() const {
  auto value_ptr = this->GetAttr(kSeed2);
  return GetValue<int64_t>(value_ptr);
}
void Gamma::set_seed2(const int64_t seed2) { (void)this->AddAttr(kSeed2, api::MakeValue(seed2)); }

MIND_API_OPERATOR_IMPL(Gamma, BaseOperator);
...
REGISTER_PRIMITIVE_EVAL_IMPL(Gamma, prim::kPrimRandomGamma, GammaInfer, nullptr, true);
}  // namespace ops
}  // namespace mindspore
