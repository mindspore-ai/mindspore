...
#include "ops/apply_came_part2.h"
...
abstract::TupleShapePtr ApplyCamePart2InferShape(const PrimitivePtr &primitive,
  ...
  output_shape_ptr_list[0] = std::make_shared<abstract::Shape>(r_vec);
  output_shape_ptr_list[1] = std::make_shared<abstract::Shape>(c_vec);
  output_shape_ptr_list[2] = std::make_shared<abstract::Shape>(u_vec);
  output_shape_ptr_list[3] = std::make_shared<abstract::Shape>(sum_square_u_vec);
  return std::make_shared<abstract::TupleShape>(output_shape_ptr_list);
}

TypePtr ApplyCamePart2InferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
  ...
  std::map<std::string, TypePtr> other_types = {{"sum_grad_r", sum_grad_r_type},
                                                {"sum_grad_c", sum_grad_c_type},
                                                {"sum_grad_rc", sum_grad_rc_type},
                                                {"sum_r", sum_r_type}};
...
AbstractBasePtr ApplyCamePart2Infer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
                                    const std::vector<AbstractBasePtr> &input_args) {
  MS_EXCEPTION_IF_NULL(primitive);
  CheckAndConvertUtils::CheckInputArgs(input_args, kEqual, kInputsNumOfPart2, primitive->name());
  auto infer_type = ApplyCamePart2InferType(primitive, input_args);
  auto infer_shape = ApplyCamePart2InferShape(primitive, input_args);
...
class MIND_API AGApplyCamePart2Infer : public abstract::OpInferBase {
 public:
  BaseShapePtr InferShape(const PrimitivePtr &primitive,
                          const std::vector<AbstractBasePtr> &input_args) const override {
    return ApplyCamePart2InferShape(primitive, input_args);
  }

  TypePtr InferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) const override {
    return ApplyCamePart2InferType(primitive, input_args);
  }
  AbstractBasePtr InferShapeAndType(const abstract::AnalysisEnginePtr &engine, const PrimitivePtr &primitive,
                                    const std::vector<AbstractBasePtr> &input_args) const override {
    return ApplyCamePart2Infer(engine, primitive, input_args);
  }
};

REGISTER_PRIMITIVE_OP_INFER_IMPL(ApplyCamePart2, prim::kPrimApplyCamePart2, AGApplyCamePart2Infer, false);
}  // namespace ops
}  // namespace mindspore
