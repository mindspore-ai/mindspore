 ...
 */
#include "ops/view/imag_strides_calc.h"
#include <vector>
#include <memory>
#include <set>
...
constexpr size_t kImagInputsNum = 1;
TensorStorageInfoPtrList ImagCalc(const PrimitivePtr &prim, const std::vector<ValuePtr> &inputs) {
  if (CheckInputsNull(inputs, kImagInputsNum)) {
  ...
  if (tensor->data_type() == kNumberTypeComplex64 || tensor->data_type() == kNumberTypeComplex128) {
    ...
    auto new_storage_offset = old_storage_offset * num + 1;
...
REG_VIEW_STRIDES_CALC_FUN(Imag, ImagCalc);
