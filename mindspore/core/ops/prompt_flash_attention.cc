...
void CheckOptinalInputShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args,
  ...
  if (sparse_mode != SPARSE_MODE_0 && sparse_mode != SPARSE_MODE_2) {
...
abstract::TupleShapePtr PromptFlashAttentionInferShape(const PrimitivePtr &primitive,
  ...
  if (input_layout == "BSH") {
    ...
    if (h_dim != abstract::Shape::kShapeDimAny && h_dim % num_heads != 0) {
    ...
    if (n_dim != abstract::Shape::kShapeDimAny && n_dim != num_heads) {
