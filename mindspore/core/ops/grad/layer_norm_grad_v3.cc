...
#include "ops/grad/layer_norm_grad_v3.h"
#include <memory>
...
namespace mindspore {
namespace ops {
void LayerNormGradV3::Init(const int64_t begin_norm_axis, const int64_t begin_params_axis) {
  this->set_begin_norm_axis(begin_norm_axis);
  this->set_begin_params_axis(begin_params_axis);
}
void LayerNormGradV3::set_begin_norm_axis(const int64_t begin_norm_axis) {
  (void)this->AddAttr(kBeginNormAxis, api::MakeValue(begin_norm_axis));
}
void LayerNormGradV3::set_begin_params_axis(const int64_t begin_params_axis) {
  (void)this->AddAttr(kBeginParamsAxis, api::MakeValue(begin_params_axis));
}
int64_t LayerNormGradV3::get_begin_norm_axis() const {
  auto value_ptr = this->GetAttr(kBeginNormAxis);
  MS_EXCEPTION_IF_NULL(value_ptr);
  return GetValue<int64_t>(value_ptr);
}
int64_t LayerNormGradV3::get_begin_params_axis() const {
  auto value_ptr = this->GetAttr(kBeginParamsAxis);
  MS_EXCEPTION_IF_NULL(value_ptr);
  return GetValue<int64_t>(value_ptr);
}
float LayerNormGradV3::get_epsilon() const {
  auto value_ptr = this->GetAttr(kEpsilon);
  return GetValue<float>(value_ptr);
}

MIND_API_OPERATOR_IMPL(LayerNormGradV3, BaseOperator);
class MIND_API LayerNormGradV3Infer : public abstract::OpInferBase {
 public:
  BaseShapePtr InferShape(const PrimitivePtr &primitive,
                          const std::vector<AbstractBasePtr> &input_args) const override {
    auto x_shape_ptr = input_args[kInputIndex1]->BuildShape()->cast<abstract::ShapePtr>();
...
REGISTER_PRIMITIVE_OP_INFER_IMPL(LayerNormGradV3, prim::kPrimLayerNormGradV3, LayerNormGradV3Infer, false);
}  // namespace ops
}  // namespace mindspore
