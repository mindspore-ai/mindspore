...
constexpr int kConv3DBackpropFilterDynamicRankValue = -2;
...
abstract::ShapePtr Conv3DBackpropFilterInferShape(const PrimitivePtr &primitive,
  ...
  auto input_shape =
    CheckAndConvertUtils::ConvertShapePtrToShapeMap(input_args[kConv3DBackpropFilterInputIndex]->BuildShape())[kShape];
  auto dout_shape =
  ...
  if (IsDynamicRank(filter_size_v) || IsDynamicRank(input_shape) || IsDynamicRank(dout_shape)) {
    std::vector<int64_t> out_shape = {kConv3DBackpropFilterDynamicRankValue};
...
TypePtr Conv3DBackpropFilterInferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
...
AbstractBasePtr Conv3DBackpropFilterInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
                                          const std::vector<AbstractBasePtr> &input_args) {
  MS_EXCEPTION_IF_NULL(primitive);
  auto prim_name = primitive->name();
  ...
  auto res = std::make_shared<abstract::AbstractTensor>(Conv3DBackpropFilterInferType(primitive, input_args),
                                                        Conv3DBackpropFilterInferShape(primitive, input_args));
  return res;
}
REGISTER_PRIMITIVE_EVAL_IMPL(Conv3DBackpropFilter, prim::kPrimConv3DBackpropFilter, Conv3DBackpropFilterInfer, nullptr,
                             true);
}  // namespace ops
}  // namespace mindspore
