...
namespace ops {
namespace {
constexpr int64_t kMirrorPadGradInputSize = 2;
constexpr int64_t kMirrorPadGradNumTwo = 2;
}  // namespace
void MirrorPadGrad::Init(const std::string &pad_mode) { this->set_mode(pad_mode); }

void MirrorPadGrad::set_mode(const std::string &mode) {
  (void)this->AddAttr(kNameMirrorPadGradMode, api::MakeValue(mode));
}

std::string MirrorPadGrad::get_mode() const { return GetValue<std::string>(GetAttr(kNameMirrorPadGradMode)); }

MIND_API_OPERATOR_IMPL(MirrorPadGrad, BaseOperator);

namespace {
TypePtr MirrorPadGradInferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
...
abstract::ShapePtr MirrorPadGradInferShape(const PrimitivePtr &primitive,
  ...
  if (paddings_size % kMirrorPadGradNumTwo == 1) {
  ...
  (void)CheckAndConvertUtils::CheckInteger("input numbers", SizeToLong(input_args.size()), kEqual,
                                           kMirrorPadGradInputSize, prim_name);
  ...
  for (size_t i = 0; i < static_cast<size_t>(size); i++) {
    x_shape[i] = x_shape[i] - paddings_arg[kMirrorPadGradNumTwo * i] - paddings_arg[kMirrorPadGradNumTwo * i + 1];
  }
...
abstract::AbstractBasePtr MirrorPadGradInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
  ...
  MS_EXCEPTION_IF_NULL(primitive);
  auto infertype = MirrorPadGradInferType(primitive, input_args);
  auto infershape = MirrorPadGradInferShape(primitive, input_args);
