...
constexpr size_t kConv3DBackpropInputInputIndex = 1;
constexpr size_t kConv3DBackpropInputSizeIndex = 2;
constexpr int kConv3DBackpropPadHalf = 2;
constexpr int kConv3DBackpropInputDynamicRankValue = -2;
...
abstract::ShapePtr Conv3DBackpropInputInferShape(const PrimitivePtr &primitive,
  ...
  if (IsDynamicRank(out_shape_v) || IsDynamicRank(dout_shape)) {
    out_shape_v = {kConv3DBackpropInputDynamicRankValue};
...
TypePtr Conv3DBackpropInputInferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
  MS_EXCEPTION_IF_NULL(prim);
  auto prim_name = prim->name();
  // check
  std::map<std::string, TypePtr> types;
  (void)types.emplace("x", input_args[kConv3DBackpropInputInputIndex]->BuildType());
...
AbstractBasePtr Conv3DBackpropInputInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
                                         const std::vector<AbstractBasePtr> &input_args) {
  MS_EXCEPTION_IF_NULL(primitive);
  auto prim_name = primitive->name();
  // check
  ...
  auto abs = std::make_shared<abstract::AbstractTensor>(Conv3DBackpropInputInferType(primitive, input_args),
                                                        Conv3DBackpropInputInferShape(primitive, input_args));
  return abs;
}
...
REGISTER_PRIMITIVE_EVAL_IMPL(Conv3DBackpropInput, prim::kPrimConv3DBackpropInput, Conv3DBackpropInputInfer, nullptr,
                             true);
}  // namespace ops
}  // namespace mindspore
