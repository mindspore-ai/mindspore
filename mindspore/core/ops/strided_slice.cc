...
abstract::ShapePtr StridedSliceInferShape(const PrimitivePtr &primitive,
  ...
  auto temp_begin_v = input_args[1]->BuildValue();
  auto begin_v = GetValue<std::vector<int64_t>>(temp_begin_v);
  auto temp_end_v = input_args[2]->BuildValue();
  auto end_v = GetValue<std::vector<int64_t>>(temp_end_v);
  auto temp_strides_v = input_args[3]->BuildValue();
  auto strides_v = GetValue<std::vector<int64_t>>(temp_strides_v);

  auto x_shape = CheckAndConvertUtils::ConvertShapePtrToShape("x_shape", input_args[0]->BuildShape(), prim_name);
  auto begin_shape =
    CheckAndConvertUtils::ConvertShapePtrToShape("begin_shape", input_args[1]->BuildShape(), prim_name);
  auto slice_shape =
    CheckAndConvertUtils::ConvertShapePtrToShape("slice_shape", input_args[2]->BuildShape(), prim_name);
  int64_t x_rank = x_shape.size();
  int64_t slice_len = begin_shape.size();

  std::vector<int64_t> temp_begin_pos = strided_slice_prim->TenToTwo(strided_slice_prim->get_begin_mask());
  std::vector<int64_t> begin_pos = {temp_begin_pos.begin() + 2, temp_begin_pos.end()};
  std::reverse(begin_pos.begin(), begin_pos.end());

  std::vector<int64_t> temp_end_pos = strided_slice_prim->TenToTwo(strided_slice_prim->get_end_mask());
  std::vector<int64_t> end_pos = {temp_end_pos.begin() + 2, temp_end_pos.end()};
  std::reverse(end_pos.begin(), end_pos.end());

  std::vector<int64_t> temp_ellipsis_pos = strided_slice_prim->TenToTwo(strided_slice_prim->get_ellipsis_mask());
  std::vector<int64_t> ellipsis_pos = {temp_ellipsis_pos.begin() + 2, temp_ellipsis_pos.end()};
  std::reverse(ellipsis_pos.begin(), ellipsis_pos.end());

  std::vector<int64_t> temp_new_axis_pos = strided_slice_prim->TenToTwo(strided_slice_prim->get_new_axis_mask());
  std::vector<int64_t> new_axis_pos = {temp_new_axis_pos.begin() + 2, temp_new_axis_pos.end()};
  std::reverse(new_axis_pos.begin(), new_axis_pos.end());

  std::vector<int64_t> temp_shrink_axis_pos = strided_slice_prim->TenToTwo(strided_slice_prim->get_shrink_axis_mask());
  std::vector<int64_t> shrink_axis_pos = {temp_shrink_axis_pos.begin() + 2, temp_shrink_axis_pos.end()};
  std::reverse(shrink_axis_pos.begin(), shrink_axis_pos.end());
...
TypePtr StridedSliceInferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
  ...
  auto infer_type = input_args[0]->BuildType()->type_id();
