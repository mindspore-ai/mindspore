...
#ifndef MINDSPORE_CORE_OPS_TENSOR_COPY_SLICES_H_
#define MINDSPORE_CORE_OPS_TENSOR_COPY_SLICES_H_
...
class MIND_API TensorCopySlices : public BaseOperator {
 public:
  ...
  TensorCopySlices() : BaseOperator(kNameTensorCopySlices) {
    InitIOName({"x", "value", "begin", "end", "strides"}, {"output"});
  }
};
AbstractBasePtr TensorCopySlicesInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
                                      const std::vector<AbstractBasePtr> &input_args);
using PrimTensorCopySlicesPtr = std::shared_ptr<TensorCopySlices>;
}  // namespace ops
}  // namespace mindspore

#endif  // MINDSPORE_CORE_OPS_TENSOR_COPY_SLICES_H_
