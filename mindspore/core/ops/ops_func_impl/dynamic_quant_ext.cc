...
TypePtrList DynamicQuantExtFuncImpl::InferType(const PrimitivePtr &primitive, const ValuePtrList &input_values) const {
  return {kInt8, kFloat32};
}

ShapeArray DynamicQuantExtFuncImpl::InferShape(const PrimitivePtr &primitive, const ValuePtrList &input_values) const {
  const auto &x_tensor = input_values[kIndex0]->cast<tensor::BaseTensorPtr>();
  MS_EXCEPTION_IF_NULL(x_tensor);
  const ShapeVector &x_shape = x_tensor->shape();
  if (x_shape.empty()) {
    MS_EXCEPTION(ValueError) << "Input shape size should be at least 1, but got " << x_shape.size();
  }
  ShapeVector scale_shape(x_shape.begin(), x_shape.end() - 1);
  return {x_shape, scale_shape};
}
REGISTER_SIMPLE_INFER(kNameDynamicQuantExt, DynamicQuantExtFuncImpl)
}  // namespace mindspore::ops
