...
std::pair<int64_t, int64_t> Im2ColComputeOutputHeightAndWeight(const std::pair<int64_t, int64_t> &input_hw,
                                                               const ArrayValue<int64_t> &kernel_size_array,
                                                               const ArrayValue<int64_t> &dilation_array,
                                                               const ArrayValue<int64_t> &padding_array,
                                                               const ArrayValue<int64_t> &stride_array) {
  const auto &kernel_size = kernel_size_array.ToVector();
  const auto &dilation = dilation_array.ToVector();
  const auto &padding = padding_array.ToVector();
  const auto &stride = stride_array.ToVector();
  return Im2ColComputeOutputHeightAndWeight(input_hw, kernel_size, dilation, padding, stride);
}
...
int32_t Im2ColAndCol2ImCommonCheckShape(const PrimitivePtr &primitive, const AbstractBasePtr &input_arg,
  ...
  auto check_func = [](int64_t dim_value) {
  ...
  auto check_result = std::all_of(input_shape.begin() + first_dim_after_batch, input_shape.end(), check_func);
  ...
  if (MS_UNLIKELY(is_dynamic_dim(input_height) || is_dynamic_dim(input_width) || is_kernel_unknown ||
    ...
    if (MS_UNLIKELY(output_height < 1 || output_width < 1)) {
      MS_EXCEPTION(ValueError) << "For " << primitive->name() << ", given input with spatial size (" << input_height
                               ...
                               << output_width << ", " << output_width << "), which is too small (non-positive).";
    }
...
int32_t Col2ImExtFuncImpl::CheckValidation(const PrimitivePtr &primitive,
  ...
  const size_t ele_num = 2;
  const auto &kernel_size = kernel_size_opt.value();
  (void)Im2ColAndCol2ImCommonCheckArray(primitive, kernel_size, "kernel_size", ele_num, 0);

  if (MS_UNLIKELY(kernel_size.HasUnknownValue())) {
    return OP_CHECK_RETRY;
  }
  ...
  const auto &output_size = output_size_opt.value();
  (void)Im2ColAndCol2ImCommonCheckArray(primitive, output_size, "output_size", ele_num, 0);
  ...
  const auto &dilation = arrays[kIndex0];
  const auto &padding = arrays[kIndex1];
  const auto &stride = arrays[kIndex2];
