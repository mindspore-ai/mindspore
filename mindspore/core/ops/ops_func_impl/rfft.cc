...
namespace mindspore {
namespace ops {
BaseShapePtr RFFTFuncImpl::InferShape(const PrimitivePtr &primitive,
  ...
  if (dim_opt.has_value()) {
    ...
    tmp_pos = dim < 0 ? static_cast<int64_t>(x_rank) + dim : dim;
  } else {
    tmp_pos = static_cast<int64_t>(x_rank) - 1;
  }
  if (!input_args[kInputIndex1]->GetType()->isa<TypeNone>()) {
    // if n is given, out.shape[dim] is (n/2) + 1 or (n+1) / 2
    auto n_opt = GetScalarValue<int64_t>(input_args[kInputIndex1]->GetValue());
    if (n_opt.has_value()) {
      auto n = n_opt.value();
      y_shape[tmp_pos] = n / kRealFFTSideNum + 1;
    }
  } else {
    // if n is not given, n == input.shape[dim]
    auto n = input_shape[tmp_pos];
    if (n != abstract::Shape::kShapeDimAny) {
      y_shape[tmp_pos] = n / kRealFFTSideNum + 1;
    }
  }
...
TypePtr RFFTFuncImpl::InferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) const {
...
*/
int32_t RFFTFuncImpl::CheckValidation(const PrimitivePtr &primitive,
                                      const std::vector<AbstractBasePtr> &input_args) const {
