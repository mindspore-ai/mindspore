...
ShapeArray ZerosFuncImpl::InferShape(const PrimitivePtr &primitive, const ValuePtrList &input_values) const {
...
TypePtrList ZerosFuncImpl::InferType(const PrimitivePtr &primitive, const ValuePtrList &input_values) const {
  auto prim_name = primitive->name();
...
REGISTER_SIMPLE_INFER(kNameZeros, ZerosFuncImpl)
}  // namespace ops
}  // namespace mindspore
