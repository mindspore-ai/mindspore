...
BaseShapePtr FFTFreqFuncImpl::InferShape(const PrimitivePtr &primitive,
  ...
  if (IsNoneOrAnyValue(n_opt)) {
    ...
    if (n_value.has_value()) {
      (void)out_shape.emplace_back(n_value.value());
    }
  }
  return std::make_shared<abstract::TensorShape>(out_shape);
}

TypePtr FFTFreqFuncImpl::InferType(const PrimitivePtr &primitive,
                                   const std::vector<AbstractBasePtr> &input_args) const {
...
int32_t FFTFreqFuncImpl::CheckValidation(const PrimitivePtr &primitive,
                                         const std::vector<AbstractBasePtr> &input_args) const {
