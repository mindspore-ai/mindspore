...
namespace mindspore {
namespace ops {
BaseShapePtr ReflectionPad2DFuncImpl::InferShape(const PrimitivePtr &primitive,
                                                 const std::vector<AbstractBasePtr> &input_args) const {
  constexpr size_t kReflectionPad2DDim = 2;
  return PadInferShapeBase(primitive, input_args, kReflectionPad2DDim);
}

TypePtr ReflectionPad2DFuncImpl::InferType(const PrimitivePtr &primitive,
                                           const std::vector<AbstractBasePtr> &input_args) const {
