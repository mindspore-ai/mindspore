...
namespace ops {
namespace {
constexpr size_t kInputQueryBSHRankIFA = 3;
constexpr size_t kInputQueryBNSDRankIFA = 4;
...
void CheckShapeSizeRight(const PrimitivePtr &primitive, size_t shape_size) {
  auto op_name = primitive->name();
  if (shape_size != kInputQueryBSHRankIFA && shape_size != kInputQueryBNSDRankIFA) {
...
std::vector<int64_t> ObtainCorrShape(const std::vector<AbstractBasePtr> &input_args, size_t index) {
  ...
  if (CheckIsFrontend(input_args)) {
    ...
    out_shape = elements[index]->BuildShape()->cast<abstract::ShapePtr>()->shape();
  }
  return out_shape;
}
...
std::vector<int64_t> GetIFADynInputShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args,
  ...
  if (!CheckIsFrontend(input_args)) {
    ...
    if (IsDynamicRank(shape_vec)) {
      if (input_layout == "BSH") {
        return std::vector(kInputQueryBSHRankIFA, abstract::Shape::kShapeDimAny);
      } else {
        return std::vector(kInputQueryBNSDRankIFA, abstract::Shape::kShapeDimAny);
      }
    }
    return shape_vec;
  }
...
void CheckActualSeqLengthsShapeValue(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args,
  ...
  auto op_name = primitive->name();
  auto asl_type = input_args[kIncreFlashAttentionInputActualSeqLengths]->BuildType();
...
abstract::ShapePtr IncreFlashAttentionInferShapeBSH(const PrimitivePtr &primitive,
  ...
  if (IsDynamicRank(query_shape)) {
    query_shape = std::vector(kInputQueryBSHRankIFA, abstract::Shape::kShapeDimAny);
  ...
  ShapeVector attention_out_shape(kInputQueryBSHRankIFA, abstract::Shape::kShapeDimAny);
...
abstract::ShapePtr IncreFlashAttentionInferShapeBNSD(const PrimitivePtr &primitive,
  ...
  std::vector<int64_t> value_shape = GetIFADynInputShape(primitive, input_args, kIncreFlashAttentionInputValueIndex);
  if (IsDynamicRank(query_shape)) {
    query_shape = std::vector(kInputQueryBNSDRankIFA, abstract::Shape::kShapeDimAny);
  } else if (query_shape[0] > 50) {
    MS_LOG(EXCEPTION) << op_name << ": The batch must not be bigger than 50, but got " << query_shape[0];
  }
  ...
  ShapeVector attention_out_shape(kInputQueryBNSDRankIFA, abstract::Shape::kShapeDimAny);
  attention_out_shape[0] = GetDimension({query_shape[0], key_shape[0], value_shape[0]}, op_name, "B");
