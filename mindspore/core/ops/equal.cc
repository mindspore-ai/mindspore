...
ValuePtr EqualInferValue(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) {
  ...
  switch (type_id) {
    ...
    case kNumberTypeFloat: {
      EqualImpl<float>(x1_tensor->data_c(), x2_tensor->data_c(), result_tensor->data_c(), data_size, need_broad_cast);
      break;
    }
    ...
    case kNumberTypeFloat64: {
      EqualImpl<double>(x1_tensor->data_c(), x2_tensor->data_c(), result_tensor->data_c(), data_size, need_broad_cast);
      break;
    }
