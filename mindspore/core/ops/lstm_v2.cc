...
abstract::TupleShapePtr LSTMV2InferShape(const PrimitivePtr &primitive,
  ...
  if (is_dynamic_shp) {
    ...
    if (h_max_shape.empty() || seq_max_shape.empty()) {
      MS_LOG(EXCEPTION) << "For LSTMV2, only the batch size can be dynamic shape, so that the input_shape[1], "
    ...
    ShapeVector hn_cn_min_shape = {real_num_layers, x_min_shape[1], hidden_size};
    ShapeVector hn_cn_max_shape = {real_num_layers, x_max_shape[1], hidden_size};
    auto hn_shape_ptr = std::make_shared<abstract::Shape>(hn_shape, hn_cn_min_shape, hn_cn_max_shape);
    auto cn_shape_ptr = std::make_shared<abstract::Shape>(cn_shape, hn_cn_min_shape, hn_cn_max_shape);
