...
void LLMManager::init() {
  ...
  if (!llm_seq_length_idx_env.empty()) {
    ...
    MS_LOG(WARNING) << "LLM Manager init: enable multi_level_seq_length with graph input index: "
  ...
  MS_LOG(WARNING) << "LLM Manager init: use seq_length_level_size: " << seq_length_level_size_;
