...
std::string AbstractSequence::ToString() const {
  ...
  ss << ", dynamic_len:" << dynamic_len_ << ", is dyn arg:" << is_arg_of_dyn_len_param_;
  ss << "}";
  ss << "\n";
  return ss.str();
}
...
AbstractBasePtr AbstractTuple::Clone() const {
  ...
  ret->is_arg_of_dyn_len_param_ = is_arg_of_dyn_len_param_;
  ret->set_dynamic_len(dynamic_len_);
...
AbstractBasePtr AbstractTensor::Join(const AbstractBasePtr &other) {
  ...
  if (this->has_user_data("__raise_flag__") && (is_none || is_raise)) {
    ...
    AbstractBasePtr join_res = std::make_shared<AbstractTensor>(type, shape);
