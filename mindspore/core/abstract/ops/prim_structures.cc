...
AbstractBasePtr InferImplExtractKeywordArg(const AnalysisEnginePtr &, const PrimitivePtr &primitive,
  ...
  if (args_spec_list.size() == check_key_input_size) {
    AbstractScalarPtr key = CheckArg<AbstractScalar>(op_name, args_spec_list, 0);
    kwarg = CheckArg<AbstractKeywordArg>(op_name, args_spec_list, 1);
  ...
  } else if (args_spec_list.size() == only_kw_input_size) {
    kwarg = CheckArg<AbstractKeywordArg>(op_name, args_spec_list, 0);
  } else {
