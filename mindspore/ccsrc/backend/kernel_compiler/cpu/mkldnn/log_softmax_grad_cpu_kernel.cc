...
void LogSoftmaxGradCPUKernel::InitKernel(const CNodePtr &kernel_node) {
  ...
  auto prim_desc = CreateDesc<dnnl::logsoftmax_forward::primitive_desc>(desc, MKLKernelEngine::Get().engine());
  ...
  auto backward_prim_desc =
    CreateDesc<dnnl::logsoftmax_backward::primitive_desc>(backward_desc, MKLKernelEngine::Get().engine(), prim_desc);
