...
#include "nnacl/int8/splice_int8.h"
void SpliceInt8(const int8_t *src_data, int src_row, int src_col, const SpliceParameter *splice_parameter,
  ...
  for (int r = 0; r < dst_row; ++r) {
    for (int off = 0; off < splice_parameter->context_dim_; ++off) {
      ...
      const int8_t *tmp_src_data = src_data + r_off * src_col * sizeof(int8_t);
      int8_t *tmp_dst_data = dst_data + r * dst_col * sizeof(int8_t);
      memcpy(tmp_dst_data + off * src_col, tmp_src_data, src_col * sizeof(int8_t));
    }
  }
}
