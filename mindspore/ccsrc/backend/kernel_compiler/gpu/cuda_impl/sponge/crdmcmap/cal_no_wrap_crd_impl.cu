...
__global__ void Calculate_No_Wrap_Crd(int atom_numbers, INT_VECTOR *box_map_times, VECTOR box, VECTOR *crd,
  ...
  for (int i = threadIdx.x; i < atom_numbers; i = i + blockDim.x) {
    nowrap_crd[i].x = static_cast<float>(box_map_times[i].int_x) * box.x + crd[i].x;
    nowrap_crd[i].y = static_cast<float>(box_map_times[i].int_y) * box.y + crd[i].y;
    nowrap_crd[i].z = static_cast<float>(box_map_times[i].int_z) * box.z + crd[i].z;
  }
}
...
void calculatenowrapcrd(int atom_numbers, int *box_map_times_f, float *box_f, float *crd_f, float *nowrap_crd_f,
  ...
  Calculate_No_Wrap_Crd<<<block_per_grid, thread_per_block, 0, stream>>>(atom_numbers, box_map_times, box[0], crd,
                                                                         nowrap_crd);
  return;
}
