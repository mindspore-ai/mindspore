...
__global__ void v2_Coordinate_Refresh(const int virtual_numbers, const VIRTUAL_TYPE_2 *v_info,
  ...
  if (i < virtual_numbers) {
    ...
    VECTOR rv1 =
      a * Get_Periodic_Displacement(uint_r2, uint_r1, scaler) + b * Get_Periodic_Displacement(uint_r3, uint_r1, scaler);

    coordinate[atom_v] = coordinate[atom_1] + rv1;
  }
}

void v2CoordinateRefresh(int atom_numbers, int virtual_numbers, const float *v_info_f, const int *uint_crd_f,
  ...
  VIRTUAL_TYPE_2 *v_info = const_cast<VIRTUAL_TYPE_2 *>(reinterpret_cast<const VIRTUAL_TYPE_2 *>(v_info_f));

  v2_Coordinate_Refresh<<<block_per_grid, thread_per_block, 0, stream>>>(virtual_numbers, v_info, uint_crd, scaler[0],
                                                                         crd);
