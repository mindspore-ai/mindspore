...
#ifndef MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_SPONG_MD_ITERATION_LEAP_FROG_LIUJIAN_GPU_WITH_MAX_VEL_KERNEL_H_
#define MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_SPONG_MD_ITERATION_LEAP_FROG_LIUJIAN_GPU_WITH_MAX_VEL_KERNEL_H_
...
class MDIterationLeapFrogLiujianMaxVelCudaGpuKernel : public GpuKernel {
 public:
  MDIterationLeapFrogLiujianMaxVelCudaGpuKernel() {}
  ~MDIterationLeapFrogLiujianMaxVelCudaGpuKernel() override = default;
  ...
  bool Init(const CNodePtr &kernel_node) override {
    ...
    max_vel = static_cast<float>(GetAttr<float>(kernel_node, "max_vel"));
    InitSizeLists();
    return true;
  }
  ...
  bool Launch(const std::vector<AddressPtr> &inputs, const std::vector<AddressPtr> &workspace,
    ...
    MD_Iteration_Leap_Frog_With_LiuJian_With_Max_Vel(
      atom_numbers, half_dt, dt, exp_gamma, float4_numbers, inverse_mass, sqrt_mass_inverse, vel, crd, frc, acc,
      reinterpret_cast<curandStatePhilox4_32_10_t *>(rand_state), rand_frc, output, max_vel,
