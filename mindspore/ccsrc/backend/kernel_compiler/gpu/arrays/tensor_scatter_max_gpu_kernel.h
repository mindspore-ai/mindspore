...
#ifndef MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_ARRAYS_TENSOR_SCATTER_MAX_GPU_KERNEL_H
#define MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_ARRAYS_TENSOR_SCATTER_MAX_GPU_KERNEL_H

#include <vector>
#include <algorithm>

#include "backend/kernel_compiler/gpu/cuda_impl/tensor_scatter_max.cuh"
...
class TensorScatterMaxGpuKernelMod : public NativeGpuKernelMod {
 public:
  TensorScatterMaxGpuKernelMod()
      : input_size_(1),
        update_size_(1),
        indices_size_(1),
        output_size_(1),
  ...
  bool Launch(const std::vector<AddressPtr> &inputs, const std::vector<AddressPtr> &workspace,
    ...
    if (!memcpy_flag_) {
      ...
      CHECK_CUDA_RET_WITH_EXCEPT(kernel_node_,
                                 ...
                                 "cudaMemcpy failed in TensorScatterMaxGpuKernelMod::Launch.");
      CHECK_CUDA_RET_WITH_EXCEPT(kernel_node_,
    ...
    TensorScatterMax(input, indices, update, output, block_size_, update_size, output_size, indices_dim_0_,
...
#endif  // MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_ARRAYS_TENSOR_SCATTER_MAX_GPU_KERNEL_H
