...
void ParseListIntAttrValue(const mindspore::ValuePtr &value, nlohmann::json *attr_obj) {
...
void TbeKernelJsonCreator::ParseAttrValue(const std::string &type, const mindspore::ValuePtr &value,
  ...
  if (type == kVTypeInt) {
    ...
    auto attr_str_value = GetValue<std::string>(value);
    if (attr_str_value == kOpFormat_FRAC_Z) {
      attr_str_value = kOpFormat_FRACTAL_Z;
    }
    (*attr_obj)[kJValue] = attr_str_value;
    ...
    ParseListIntAttrValue(value, attr_obj);
    ...
    for (size_t m = 0; m < input_json[i].size(); m++) {
      ...
      for (size_t j = 0; j < input_json[i][m][kJShape].size(); ++j) {
        if (input_json[i][m][kJShape][j] == -1) {
          ...
          size_i = SizetMulWithOverflowCheck(size_i, LongToSize(input_max_shape[j]));
          continue;
        }
        size_i = SizetMulWithOverflowCheck(size_i, static_cast<size_t>(input_json[i][m][kJShape][j]));
      }
    ...
    for (size_t m = 0; m < output_json[i].size(); m++) {
      ...
      for (size_t j = 0; j < output_json[i][m][kJShape].size(); ++j) {
        if (output_json[i][m][kJShape][j] == -1) {
          ...
          size_i = SizetMulWithOverflowCheck(size_i, LongToSize(output_max_shape[j]));
          continue;
        }
        size_i = SizetMulWithOverflowCheck(size_i, static_cast<size_t>(output_json[i][m][kJShape][j]));
      }
...
bool TbeKernelBuild::GetInputLayers(const std::vector<mindspore::AnfNodePtr> &input_nodes,
  ...
  auto result = std::find_if(compute_nodes.begin(), compute_nodes.end(), [](const auto &it) {
    auto node_name = AnfAlgo::GetCNodeName(it);
    return node_name == kConv2DBackpropInputOpName;
  });
...
bool TbeKernelBuild::IsDynamicInput(const mindspore::CNodePtr &cnode) {
  ...
  if (dynamic_input_attr != nullptr) {
    ...
    if (dyn_input_sizes.empty()) {
      MS_LOG(EXCEPTION) << "Cnode's dynamic input attr is empty, the cnode is " << cnode->fullname_with_scope();
    }
    if (dyn_input_size != 1) {
