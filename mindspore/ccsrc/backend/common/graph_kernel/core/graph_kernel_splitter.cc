...
Area::Area(const AnfNodePtrList &anf_arr) {
...
void Area::CreateParameters(const FuncGraphPtr &func_graph,
...
void Area::CreateReturnNode(const FuncGraphPtr &func_graph, mindspore::HashMap<AnfNodePtr, size_t> *tuple_node_index) {
...
void Area::AddTraitor(const AnfNodePtr &node) {
  if (std::find(traitor_nodes_.begin(), traitor_nodes_.end(), node) == traitor_nodes_.end()) {
    (void)traitor_nodes_.emplace_back(node);
  }
}

const mindspore::HashSet<AnfNodePtr> &Area::nodes() const { return nodes_; }
const std::vector<AnfNodePtr> &Area::spy_cnodes() const { return spy_cnodes_; }
bool Area::IsExternalCNode(const AnfNodePtr &node) const { return node->isa<CNode>() && this->nodes_.count(node) == 0; }
...
template <typename T>
std::shared_ptr<T> MakeSplitter(const CNodePtr &main_cnode, const SplitSchemerPtr &split_schemer) {
  MS_EXCEPTION_IF_NULL(main_cnode);
  MS_EXCEPTION_IF_NULL(main_cnode->func_graph());
  MS_EXCEPTION_IF_NULL(split_schemer);
  return std::make_shared<T>(main_cnode, split_schemer);
}

// Maintain new subgraphs in main graph.
void Splitter::RebuildGraph(const std::vector<size_t> &cnodes_group_id) {
  BindFuncGraph();
  RecoverParameter();
...
void Splitter::BindFuncGraph() const {
...
void Splitter::RecoverParameter() {
  for (const auto &cnode : new_subgraph_cnodes_) {
    auto sub_func_graph = GetCNodeFuncGraph(cnode);
...
CNodePtr Splitter::InlineSubFuncGraph(const CNodePtr &main_node) {
...
void Splitter::SetSplitNodeName(const std::vector<size_t> &cnodes_group_id) const {
...
void Splitter::ConnectToMainGraph(const std::vector<size_t> &cnodes_group_id) {
...
void Splitter::UpdateMainNodesKernelInfo() const {
...
void Splitter::AreaExpand(const Area &area) {
...
ParameterPtr Splitter::ParameterClone(const ParameterPtr &param, const FuncGraphPtr &func) {
...
void SplitterCCE::PostProcess() { SetCNodeUseCCE(); }
void SplitterCCE::SetCNodeUseCCE() {
  for (size_t i = 0; i < new_subgraph_cnodes_.size(); ++i) {
    new_subgraph_cnodes_[i]->AddAttr("use_akg_cce", MakeValue(true));
  }
}
...
bool GraphKernelSplitter::TrySplit(const CNodePtr &sub_root_cnode) {
  ...
  auto splitter = MakeSplitter<Splitter>(sub_root_cnode, schm);
...
bool GraphKernelSplitter::Run(const FuncGraphPtr &func_graph) {
  ...
  for (auto iter = todos.crbegin(); iter != todos.crend(); ++iter) {
    ...
    if (node != nullptr && AnfUtils::IsGraphKernel(node) && CanSplit(node)) {
...
bool GraphKernelSplitter::CanSplit(const CNodePtr &node) const {
  // do not process cce ops
  return !GkUtils::IsAkgCceLibNode(node);
}

bool GraphKernelSplitterForCCE::CanSplit(const CNodePtr &node) const {
  // only for cce ops
  return GkUtils::IsAkgCceLibNode(node);
}

bool GraphKernelSplitterForCCE::TrySplit(const CNodePtr &sub_root_cnode) {
  MS_LOG(DEBUG) << "AscendCCE Split process node: " << sub_root_cnode->fullname_with_scope();
  auto schm = GetSplitSchema(inner::kAscendCCE);
  MS_EXCEPTION_IF_NULL(schm);
  auto splitter = MakeSplitter<SplitterCCE>(sub_root_cnode, schm);
  MS_EXCEPTION_IF_NULL(splitter);
  bool result = splitter->Split();
  MS_LOG(DEBUG) << "Split node completed, result: " << result;
  return result;
