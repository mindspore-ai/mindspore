...
void MathInfo::UpdateExprRoot() const {
  ...
  if (relation_expr_.s->relation_expr_.s == nullptr) {
    // "this" is second layer node, do not need to update
    return;
  }
  // "this" is third or lower layer node, link it to the root node.
  auto new_parent = relation_expr_.s->root();
  const auto &old_parent_expr = relation_expr_.s->relation_expr_;
...
void MathInfo::SetEqual(const IntSymbolPtr &other) {
  ...
  auto s2 = ToIntSymbol();
  auto s4 = other;
  if (s1->math_info_id_ < s3->math_info_id_) {
    MS_LOG(DEBUG) << "Set " << s4->ToString() << " equals to " << s2->ToString();
    if (s4->relation_expr_.s == nullptr) {
      s4->relation_expr_ = {s2, kFrac1, kFrac0};
    } else {
      s3->relation_expr_ = {s2, s4->relation_expr_.a.rec(), -(s4->relation_expr_.b / s4->relation_expr_.a)};
      s3->UpdateExprRoot();
    }
    ...
    if (relation_expr_.s == nullptr) {
      ...
      s1->relation_expr_ = {s4, s2->relation_expr_.a.rec(), -(s2->relation_expr_.b / s2->relation_expr_.a)};
      s1->UpdateExprRoot();
    }
  }
}
...
void MathInfo::PrintMathInfo(std::ostringstream &oss) const {
  oss << "<[";
