...
bool CanExpandFallback(const AnfNodePtr &node) {
  ...
  static const std::vector<OpWithLevel> expander_fallback_ops_with_level = {
    ...
    {kAllTarget, OpLevel_0, prim::kPrimDense},
...
FuncGraphPtr PyExpander::ExpandToGraphByCallPyFn(const CNodePtr &node) {
  ...
  auto func_graph = JsonDescToAnf(kernel_desc_str);
  // the function graph generated by JsonDescToAnf still has some input values in attributes,
  ...
  for (const auto &n : nodes) {
    if (ConvertOpUtils::ConvertAttrToInput(n)) {
      ...
      cb->SetBasicNodeKernelInfo(cnode, {{output_shape, output_type, output_format}});
    }
  }
  return func_graph;
}
