 ...
 */
#include "backend/common/pass/opt_micro_mem.h"
...
bool OptGraphMicroMem(const FuncGraphPtr &graph) {
  if (!NeedProcess()) {
    return false;
  }
...
bool OptMicroMem::Run(const FuncGraphPtr &func_graph) {
  MS_EXCEPTION_IF_NULL(func_graph);
  return OptGraphMicroMem(func_graph);
}
}  // namespace opt
}  // namespace mindspore
