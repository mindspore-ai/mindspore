...
namespace mindspore {
namespace opt {
const BaseRef ConcatOffsetV1Fusion::DefinePattern() const {
  VarPtr Xs = std::make_shared<SeqVar>();
  return VectorRef({prim::kPrimConcatOffsetV1, Xs});
}

const AnfNodePtr ConcatOffsetV1Fusion::Process(const FuncGraphPtr &graph, const AnfNodePtr &node,
  ...
  for (size_t idx = kIndex2; idx < cnode->inputs().size(); ++idx) {
