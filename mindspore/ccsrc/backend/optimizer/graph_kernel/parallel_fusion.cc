...
void ProcessDependCNode(OrderedMap<AnfNodePtr, NodeRelation> *node_rels) {
  for (auto &[node, node_rel] : (*node_rels)) {
    ...
    // Make attched nodes deattach with node.
...
void FindControlDependRealRelation(const AnfNodePtr &cur_node,
                                   const std::unordered_map<AnfNodePtr, AnfNodePtrList> &control_depend_info,
                                   std::unordered_map<AnfNodePtr, std::list<AnfNodePtr>> *processed,
                                   std::vector<std::list<AnfNodePtr>> *to_links) {
  // Name a ControlDepend node as CD1, name its first input as C, and second input as D, name its out node is K.
  // then the real relation is C->D->K.
  // What's more:
  // 1. If CD1's second input is a ControlDepend, named as CD2 with input E and F, then the real relation is C->E->F->K.
  // 2. If CD1's first input is a ControlDepend, named as CD3 with input G and H, then the real relation is G->H->D->K.
  // 3. If CD1's two inputs is CD2 and CD3, then the real relation is G->H->E->F->K.

  std::stack<std::tuple<AnfNodePtr, int, std::list<AnfNodePtr>>> st;
  st.push({cur_node, 0, {}});
  while (!st.empty()) {
    auto [node, status, nlist] = st.top();
    st.pop();
    auto node_iter = control_depend_info.find(node);
    if (node_iter == control_depend_info.end() || node_iter->second.size() != 3) {
      MS_LOG(EXCEPTION) << "ControlDepend node \"" << node->ToString() << "\" should have been processed right!";
    }
    switch (status) {
      case 0:
        // Status 0: initail status, should process its two inputs first.
        if (IsPrimitiveCNode(node_iter->second[0], prim::kPrimControlDepend)) {
          auto iter = processed->find(node_iter->second[0]);
          if (iter == processed->end()) {
            st.push({node, 0, nlist});
            st.push({node_iter->second[0], 0, {}});
            continue;
          } else {
            std::copy(iter->second.rbegin(), iter->second.rend(), std::front_inserter(nlist));
          }
        } else {
          nlist.push_front(node_iter->second[0]);
        }
        st.push({node, 1, nlist});
        break;
      case 1:
        // Status 1: first input is processed, should process its second input.
        if (IsPrimitiveCNode(node_iter->second[1], prim::kPrimControlDepend)) {
          auto iter = processed->find(node_iter->second[1]);
          if (iter == processed->end()) {
            st.push({node, 1, nlist});
            st.push({node_iter->second[1], 0, {}});
            continue;
          } else {
            std::copy(iter->second.begin(), iter->second.end(), std::back_inserter(nlist));
          }
        } else {
          nlist.push_back(node_iter->second[1]);
        }
        st.push({node, 2, nlist});
        break;
      case 2:
        // Status 2: all inputs are doned, add to to_links and processed!
        if (!(node_iter->second[2])) {
          to_links->push_back(nlist);
        } else if (!IsPrimitiveCNode(node_iter->second[2], prim::kPrimControlDepend)) {
          nlist.push_back(node_iter->second[2]);
          to_links->push_back(nlist);
        }
        processed->insert({node, nlist});
        break;
      default:
        MS_LOG(EXCEPTION) << "ControlDepend node's status is supposed to be 0, 1 or 2, but got " << status;
    }
  }
}
...
void ReLinkNodesOfControlDependByRelation(const std::unordered_map<AnfNodePtr, AnfNodePtrList> &control_depend_info,
  ...
  for (const auto &m : control_depend_info) {
    if (processed.count(m.first) != 0) {
      continue;
    }
    FindControlDependRealRelation(m.first, control_depend_info, &processed, &to_links);
  }

  // ReLink!
  for (const auto &rel : to_links) {
    auto prior_iter = rel.begin();
    auto behind_iter = rel.begin();
    behind_iter++;
    for (; behind_iter != rel.end(); prior_iter++, behind_iter++) {
      (*node_rels)[*prior_iter].nexts.insert(*behind_iter);
      (*node_rels)[*behind_iter].pres.insert(*prior_iter);
    }
  }
}
...
void ProcessControlDependCNode(OrderedMap<AnfNodePtr, NodeRelation> *node_rels) {
  ...
  for (auto &[node, node_rel] : (*node_rels)) {
    ...
    auto successors = node_rel.nexts;
    AnfNodePtr next_node;
    if (successors.size() > 0) {
      if (successors.size() > 1) {
        MS_LOG(EXCEPTION) << "ControlDepend node \"" << (*successors.begin())->ToString()
                          << "\" should have only one successor!";
      }
      next_node = (*successors.begin());
      (*node_rels)[next_node].pres.erase(node);
      node_rel.nexts.erase(next_node);
    }
    ...
    for (auto &first_node : real_prior_nodes) {
      for (auto &second_node : real_behind_nodes) {
        ...
        control_depend_info.insert({node, {first_node, second_node, next_node}});
      }
    }
...
void DumpParallelGroups(const std::vector<std::vector<AnfNodePtrList>> &groups) {
  MS_LOG(DEBUG) << "There are " << groups.size() << " parallel groups, their detail is: " << std::endl;
  int i = 0;
  for (const auto group : groups) {
    std::stringstream buf;
    buf << "[" << i << " group] " << group.size() << ":" << std::endl;
...
std::tuple<AnfNodePtrList, std::vector<int>> ParallelOpFusion::GetAvaliableNodesByOffset(
  ...
  for (auto offset : offsets) {
    if (offset >= limit) {
      MS_LOG(EXCEPTION) << "Index offset is exceed the limit of unsed nodes.";
    }
