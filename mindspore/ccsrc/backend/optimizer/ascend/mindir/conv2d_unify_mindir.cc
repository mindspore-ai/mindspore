...
bool NeedUpdate(const CNodePtr &conv2d, std::vector<size_t> in_shape, std::vector<size_t> out_shape) {
  ...
  int64_t data_format;
  if (data_format_ptr->isa<Int64Imm>()) {
    data_format = data_format_ptr->cast<Int64ImmPtr>()->value();
  } else {
    auto data_format_str = data_format_ptr->cast<StringImmPtr>()->value();
    data_format = CheckAndConvertUtils::GetDataFormatEnumValue(data_format_str);
  }
