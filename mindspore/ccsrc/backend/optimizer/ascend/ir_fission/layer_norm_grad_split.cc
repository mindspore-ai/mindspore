...
void LayerNormGradSplit::CreateOutputsOfLayerNormXBackprop(
  ...
  auto shapes = {AnfAlgo::GetOutputInferShape(layer_norm_grad, 0),
                 AnfAlgo::GetPrevNodeOutputInferShape(layer_norm_grad, 1)};
  AnfAlgo::SetOutputInferTypeAndShape(types, shapes, layer_norm_x_backprop.get());

  CreateMultipleOutputsOfAnfNode(graph, layer_norm_x_backprop, kLayerNormXBackpropOutputNum,
...
void LayerNormGradSplit::CreateOutputsOfLayerNormBetaGammaBackprop(
  ...
  std::vector<AnfNodePtr> layer_norm_beta_gamma_backprop_inputs = {NewValueNode(prim), layer_norm_grad->input(2),
                                                                   res_for_gamma};
...
const AnfNodePtr LayerNormGradSplit::Process(const FuncGraphPtr &graph, const AnfNodePtr &node,
  ...
  if (layer_norm_x_backprop_outputs.size() != kLayerNormXBackpropOutputNum) {
  ...
  CreateOutputsOfLayerNormBetaGammaBackprop(graph, cnode, layer_norm_x_backprop_outputs[1],
                                            &layer_norm_beta_gamma_backprop_outputs);
