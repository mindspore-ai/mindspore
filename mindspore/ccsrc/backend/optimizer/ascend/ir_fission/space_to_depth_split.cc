...
tensor::TensorPtr CreateTensor(const AnfNodePtr &node) {
  ...
  int64_t input_channel = SizeToLong(x_shape[1]);
  int64_t assist_input_channel = SizeToLong(x_shape[1]) * block_size * block_size;
  ...
  MS_LOG(INFO) << "For SpaceToDepth op, assist input shape is: (" << assist_input_channel << ", " << input_channel
  ...
  int64_t window_size = assist_input_shape[2] * assist_input_shape[3];
  ...
  auto elem_num = dest_size * kFloat16Len;
...
const AnfNodePtr SpaceToDepthSplit::Process(const FuncGraphPtr &graph, const AnfNodePtr &node, const EquivPtr &) const {
  ...
  TypeId x_dtype = AnfAlgo::GetOutputInferDataType(ori_inputs[1], 0);
