...
constexpr size_t kSupportedBatchSize = 32;
...
bool CheckSupported(const CNodePtr &conv_back_filter) {
  ...
  if (y_shape.size() != 4 || x_shape.size() != 4 || out_shape.size() != 4) {
    MS_LOG(EXCEPTION) << "The dim of Conv2dBackpropFilter's input and output should be 4";
  }
  if (x_shape[0] != kSupportedBatchSize) {
    return false;
  }
  ...
  for (auto &c : supported_cases) {
    if (c[0] == x_shape[1] && c[1] == y_shape[1] && c[2] == x_shape[2] && c[3] == x_shape[3] && c[4] == y_shape[2] &&
        c[5] == y_shape[3] && c[6] == out_shape[2] && c[7] == out_shape[3]) {
      supported = true;
      break;
    }
  }
...
const AnfNodePtr BNReduceGradConv2dBackpropFilterFusion::Process(const FuncGraphPtr &graph, const AnfNodePtr &node,
  ...
  auto bnreduce_grad = CheckAnfNodeIfCNodeAndInputSize(conv_back_filter->input(1), kBNTrainingReduceGradInputNum);
