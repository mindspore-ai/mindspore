...
void ParameterServer::Run(const FuncGraphPtr &func_graph) {
  ...
  Init(func_graph);
  server_node_->Start();
  PSContext::instance()->SetPSRankId(UintToInt(server_node_->rank_id()));
  thread_->join();
  SyncEmbeddingTables();
...
void ParameterServer::InitOptimInputsShape(const Keys &keys, const Values &values, const Lengths &lengths) {
  ...
  for (size_t i = 0; i < keys.size(); i++) {
    ...
    for (int64_t j = 0; j < lengths[i]; j++) {
      shape->push_back(FloatToSize(values[val_idx]));
      original_shape->push_back(FloatToSize(values[val_idx++]));
    }
  }
