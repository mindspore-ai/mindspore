...
void UpdateCompositeInputsKernelInfo(const CNodePtr &kernel_node, const std::vector<AnfNodePtr> &input_list,
  ...
  // We set same format to all inputs of composite subgraph, and process this latter.
  // We set dtype to inputs of composite subgraph same as infer dtypes.
  size_t input_num = AnfAlgo::GetInputTensorNum(kernel_node);
  for (size_t i = 0; i < input_num; ++i) {
    auto input_kernel_node = AnfAlgo::VisitKernel(kernel_node->input(i + 1), 0).first;
    MS_EXCEPTION_IF_NULL(input_kernel_node);
...
void SetCompositeKernelInfo(const CNodePtr &kernel_node, const std::vector<std::pair<AnfNodePtr, size_t>> &output_index,
...
void SelectCompositeKernelInfo(const CNodePtr &kernel_node, const FuncGraphPtr &func_graph) {
  ...
  MS_LOG(DEBUG) << "Composite[" << func_graph->ToString() << "] use same input format[" << default_format
                << "] for ParameterWeight.";

  std::vector<std::string> graph_input_format;
  std::vector<TypeId> graph_input_type;
  UpdateCompositeInputsKernelInfo(kernel_node, input_list, default_format, use_same_format, &graph_input_format,
                                  &graph_input_type);
  ...
  SetCompositeKernelInfo(kernel_node, output_index, graph_input_format, graph_input_type);
}
}  // namespace ascend
}  // namespace device
}  // namespace mindspore
