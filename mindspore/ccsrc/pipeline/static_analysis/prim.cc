...
py::dict ConvertAbstractToPython(const AbstractBasePtr &abs_base) {
  ...
  if (abs_base->isa<AbstractTensor>()) {
    ...
    if ((*value == *kAnyValue)) {
      MS_LOG(EXCEPTION) << "Unsupported parameter type for python primitive. Abstract base info: "
                        << abs_base->ToString();
    }
    MS_LOG(EXCEPTION) << "Unsupported parameter type for python primitive, the parameter value is "
                      << value->ToString();
  }
  return dic;
}
