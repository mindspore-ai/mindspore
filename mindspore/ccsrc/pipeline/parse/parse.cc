...
AnfNodePtr Parser::ProcessBoolOpValueList(const FunctionBlockPtr &block, const py::list value_list, AstSubType mode) {
  ...
  if (value_list.size() == 1) {
    AnfNodePtr test_node = ParseExprNode(block, value_list[0]);
    return test_node;
  } else {
    ...
    // if it is or  we can continue to next .
    if (mode == AST_SUB_TYPE_AND) {
      ...
      MS_LOG(ERROR) << "Don't support mode = " << mode;
      return nullptr;
    }
    ...
    CNodePtr test_cnode = block->ForceToBoolNode(test_node);
    ...
    CNodePtr switch_app =
      block->func_graph()->NewCNode({NewValueNode(prim::kPrimSwitch), test_cnode,
                                     NewValueNode(true_block->func_graph()), NewValueNode(false_block->func_graph())});
