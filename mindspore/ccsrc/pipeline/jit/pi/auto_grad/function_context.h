...
class FunctionContext : public std::enable_shared_from_this<FunctionContext> {
 public:
  ...
  explicit FunctionContext(const ValuePtr &output) : FunctionContext(kNone, output) {}
  ...
  ///
  /// \return The instance of FunctionContext.
  explicit FunctionContext(const ValuePtr &fn, const ValuePtr &output)
      : FunctionContext(fn, output,
                        ValuePtrList(output->isa<ValueTuple>() ? output->cast<ValueTuplePtr>()->size() : 1, kNone)) {}

  /// \brief The constructor of FunctionContext.
  ///
  /// \param[in] fn The function of FunctionContext.
  /// \param[in] output The output of FunctionContext.
  /// \param[in] dout The gradient value list of FunctionContext.
  ///
  /// \return The instance of FunctionContext.
  explicit FunctionContext(const ValuePtr &fn, const ValuePtr &output, const ValuePtrList &dout)
  ...
  const ValuePtrList &GetInputs() const { return inputs_; }
  ...
  void SetInputs(const ValuePtrList &inputs) { inputs_ = inputs; }
  ...
  /// \brief Get the grad value list of the function node.
  ///
  /// \return The grad value list of the function node.
  const ValuePtrList &GetGrad() const { return dout_; }

  /// \brief Set the grad value list of the function node.
  ///
  /// \param[in] grads The grad value list.
  void SetGrad(const ValuePtrList &grads) { dout_ = grads; }

  /// \brief Set the grad value list of the function node.
  ///
  /// \param[in] grad The grad value list.
  /// \param[in] index The index of the tensor in input.
  void SetGrad(const ValuePtr &grad, size_t index) { dout_[index] = grad; }
