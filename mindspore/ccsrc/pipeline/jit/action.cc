...
bool AcceptableReturnValue(const AnfNodePtr &input0) {
  ...
  for (const auto &node : all_nodes) {
    ...
    if (!IsValueNode<Primitive>(cnode->input(0))) {  // If cnode is a call node.
      ...
      if (IsPrimitiveCNode(input0, prim::kPrimSwitch) || IsPrimitiveCNode(input0, prim::kPrimSwitchLayer) ||
        ...
        if (AcceptableReturnValue(input0)) {
          continue;
        }
      }
...
void SetRunMode(const ResourcePtr &resource) {
  ...
  if (!is_task_sink && mode == kGraphMode && enable_hccl && (!common::UseHostCollective() || common::UseHcclCM())) {
    MS_LOG(EXCEPTION)
      << "Current execute mode is kernelbykernel, the processes must be launched with OpenMPI or Dynamic Cluster.";
  }
}
