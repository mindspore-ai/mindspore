...
void CheckDynamicLenUnify(const ResourcePtr &resource) {
  MS_EXCEPTION_IF_NULL(resource);
  ...
  if (!ProcessFuncGraph(resource->func_graph(), std::make_shared<UnifyDynamicLenFunc>(CheckUnifyDynamicLen))) {
    ...
    if (renormalize_time != 1) {
      MS_LOG(WARNING) << "For dynamic len unify, renormalize has been run for three time.";
    }
