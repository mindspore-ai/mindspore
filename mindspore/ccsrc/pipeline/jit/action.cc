...
void AddMutableNodeForDynamicArg(const FuncGraphPtr &func_graph) {
  MS_EXCEPTION_IF_NULL(func_graph);
  ...
  for (auto &node : nodes) {
    ...
    const auto &abs_sequence = node->abstract()->cast<abstract::AbstractSequencePtr>();
...
void UnifyDynamicLen(const ResourcePtr &resource) {
...
#endif
  AddMutableNodeForDynamicArg(func_graph);
#ifdef ENABLE_DUMP_IR
  if (enable_save_graphs) {
  ...
  FuncGraphPtr new_func_graph = Renormalize(resource, func_graph, args_abs);
...
bool IsDynamicLenUnify(const FuncGraphPtr &root_graph) {
...
void TaskEmitActionForMindRT(const ResourcePtr &resource) {
  ...
  if (!IsDynamicLenUnify(resource->func_graph())) {
