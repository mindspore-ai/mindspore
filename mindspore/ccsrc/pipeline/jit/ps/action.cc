...
void GenerateTopGraphParams(const FuncGraphPtr &fg, std::vector<AnfNodePtr> *params,
  ...
  const auto &trainable_parameters = py::getattr(obj, "parameters_and_names", py::none())();
  for (const auto &tr : trainable_parameters) {
...
void GeneralizeReusingGraph(const FuncGraphPtr &func_graph, const FuncGraphPtr &top_func_graph) {
  ...
  // Can not define reference because the cnodes_index will be changed.
  const auto cnodes_index = fg->func_graph_cnodes_index();
  for (const auto &cnode_index : cnodes_index) {
