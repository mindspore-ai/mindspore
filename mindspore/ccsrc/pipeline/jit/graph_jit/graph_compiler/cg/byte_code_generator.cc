...
void ByteCodeGenerator::Visit_(const ir::LoadNodePtr &node) {
  ...
  bool is_load_global = op_code == LOAD_GLOBAL;
  int arg = GetValueIndex(node->GetArg(0)->cast<ir::ValuePtr>());
  if (is_load_global) {
    ir::ValuePtr value = node->GetArg(1)->cast<ir::ValuePtr>();
    if (value->GetScope() == ir::kScopeGlobal) {
      globals_[co_names_[arg]] = value->GetValue();
    } else {
      builtins_[co_names_[arg]] = value->GetValue();
    }
  }
  CheckInstrOffset(node);
  GenerateInstr(node->GetOpCode(), arg);
...
int ByteCodeGenerator::GetValueIndex(const ir::ValuePtr &node) {
  py::object value = node->GetValue();
  switch (node->GetScope()) {
    case ir::kScopeConst: {
      if (co_consts_map_.find(value.ptr()) == co_consts_map_.end()) {
        co_consts_map_[value.ptr()] = co_consts_.size();
        co_consts_.append(value);
      }
      return co_consts_map_.at(value.ptr());
    }
    case ir::kScopeLocal: {
      auto name = py::cast<std::string>(value);
      if (co_var_names_map_.find(name) == co_var_names_map_.end()) {
        co_var_names_map_[name] = co_var_names_.size();
        co_var_names_.append(value);
      }
      return co_var_names_map_.at(name);
    }
    case ir::kScopeName: {
      auto name = py::cast<std::string>(value);
      if (co_names_map_.find(name) == co_names_map_.end()) {
        co_names_map_[name] = co_names_.size();
        co_names_.append(value);
      }
      return co_names_map_.at(name);
    }
    case ir::kScopeFreeVar: {
      auto name = py::cast<std::string>(value);
      if (co_free_vars_map_.find(name) == co_free_vars_map_.end()) {
        co_free_vars_map_[name] = co_free_vars_.size();
        co_free_vars_.append(value);
      }
      return cell_var_cnt_ + co_free_vars_map_.at(name);
    }
    case ir::kScopeCellVar: {
      auto name = py::cast<std::string>(value);
      if (co_cell_vars_map_.find(name) == co_cell_vars_map_.end()) {
        co_cell_vars_map_[name] = co_cell_vars_.size();
        co_cell_vars_.append(value);
      }
      return co_cell_vars_map_.at(name);
    }
    case ir::kScopeClousre: {
      auto name = node->GetName();
      MS_EXCEPTION_IF_CHECK_FAIL(co_free_vars_map_.find(name) != co_free_vars_map_.end(), "Closure must have a key.");
      if (clousre_map_.find(name) == clousre_map_.end()) {
        clousre_map_[name] = clousre_.size();
        clousre_.append(value);
      }
      return clousre_map_.at(name);
    }
    case ir::kScopeBuiltIn:
    case ir::kScopeGlobal: {
      MS_EXCEPTION_IF_CHECK_FAIL(node->isa<ir::Value>(), "Global must be a python object.");
      break;
    }
    default: {
      MS_EXCEPTION(ValueError) << node->ToString() << " is not expected.";
      break;
    }
  }
  return -1;
