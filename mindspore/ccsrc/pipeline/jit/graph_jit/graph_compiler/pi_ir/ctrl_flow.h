...
#include <string>
#include <vector>
#include "pipeline/jit/graph_jit/graph_compiler/pi_ir/node.h"
...
class FunctionNode : public Node {
 public:
  ...
  /**
   * \brief Set the new nodes to function node.
   *
   * \param[in] nodes the new nodes of function.
   */
  void SetNodes(const NodePtrList &nodes) { nodes_ = nodes; }
...
class IfNode : public Node {
 public:
   ...
   */
  void AddThen(const NodePtr &node) { then_.push_back(node); }

  /**
   * \brief Set the new nodes as then body of if node.
   *
   * \param[in] nodes the new nodes of then body.
   */
  void SetThen(const NodePtrList &nodes) { then_ = nodes; }
   ...
   * \param[in] node the new node.
   */
  void AddElse(const NodePtr &node) { else_.push_back(node); }

  /**
   * \brief Set the new nodes as else body of if node.
   *
   * \param[in] nodes the new nodes.
   */
  void SetElse(const NodePtrList &nodes) { else_ = nodes; }
