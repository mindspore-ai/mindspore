...
#include "pipeline/jit/graph_jit/graph_compiler/pi_ir/value.h"
...
void InputsCollector::AddInput(const ir::NodePtr &input) {
  ...
  if (input_names_.find(value->GetValue()) == input_names_.end()) {
...
void FuncWrapper::GenerateReturn() const {
  ...
  if (!nodes.empty() && nodes.back()->isa<ir::ReturnNode>()) {
    MS_EXCEPTION(NotSupportError) << "Not Support Return in If yet.";
    return;
  }
