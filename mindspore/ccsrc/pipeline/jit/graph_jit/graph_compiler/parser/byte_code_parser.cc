...
void ByteCodeParser::ParseLoadConst(const InstrPtr &instr) {
  ...
  ir::ValuePtr arg = std::make_shared<ir::Value>(opnd, ir::kScopeConst);
  ir::NodePtrList args = {arg};
  ir::NodePtr node = std::make_shared<ir::LoadNode>(instr->GetOpCode(), args);
  node->SetLineNo(instr->GetStartsLine());
  PushStack(node);
}
...
void ByteCodeParser::ParseLoadName(const InstrPtr &instr) {
  ...
  ir::ValuePtr arg = std::make_shared<ir::Value>(name, ir::kScopeName);
  ir::NodePtrList args = {arg};
  ir::NodePtr node = std::make_shared<ir::LoadNode>(instr->GetOpCode(), args);
  node->SetLineNo(instr->GetStartsLine());
  PushStack(node);
...
void ByteCodeParser::ParseLoadAttr(const InstrPtr &instr) {
  ...
  ir::ValuePtr attr = std::make_shared<ir::Value>(attr_name, ir::kScopeName);
  auto top = PopStack();
  ir::NodePtrList opnds = {attr, top};
...
void ByteCodeParser::ParseLoadGlobal(const InstrPtr &instr) {
  py::object name = py::cast<py::object>(PyTuple_GET_ITEM(code_.co_names, instr->GetArg()));
  ir::ValuePtr key = std::make_shared<ir::Value>(name, ir::kScopeName);
  bool is_global = globals_.contains(name);
  py::object global = is_global ? globals_[name] : builtins_[name];
  ir::ValuePtr value = std::make_shared<ir::Value>(global, (is_global ? ir::kScopeGlobal : ir::kScopeBuiltIn));
  ir::NodePtrList opnds = {key, value};
  ir::NodePtr node = std::make_shared<ir::LoadNode>(instr->GetOpCode(), opnds);
  node->SetLineNo(instr->GetStartsLine());
  PushStack(node);
}
...
void ByteCodeParser::ParseLoadFast(const InstrPtr &instr) {
  ...
  ir::ValuePtr value = std::make_shared<ir::Value>(name, ir::kScopeLocal);
  ir::NodePtrList opnds = {value};
  ir::NodePtr node = std::make_shared<ir::LoadNode>(instr->GetOpCode(), opnds);
  node->SetLineNo(instr->GetStartsLine());
  PushStack(node);
}
...
void ByteCodeParser::ParseLoadClosure(const InstrPtr &instr) {
  ...
  if (cell_var_size > index) {
    ...
    ir::ValuePtr key = std::make_shared<ir::Value>(name, ir::kScopeCellVar);
    opnds.push_back(key);
  } else {
    index -= cell_var_size;
    auto name = py::cast<py::object>(PyTuple_GET_ITEM(code_.co_freevars, index));
    ir::ValuePtr key = std::make_shared<ir::Value>(name, ir::kScopeFreeVar);
    opnds.push_back(key);
    ir::ValuePtr value = std::make_shared<ir::Value>(clousre_[index], py::cast<std::string>(name), ir::kScopeClousre);
    opnds.push_back(value);
  }
  ir::NodePtr node = std::make_shared<ir::LoadNode>(instr->GetOpCode(), opnds);
  node->SetLineNo(instr->GetStartsLine());
  PushStack(node);
...
void ByteCodeParser::ParseLoadMethod(const InstrPtr &instr) {
  py::object name = py::cast<py::object>(PyTuple_GET_ITEM(code_.co_names, instr->GetArg()));
  ir::ValuePtr method = std::make_shared<ir::Value>(name, ir::kScopeName);
  auto top = PopStack();
  ir::NodePtrList opnds = {method, top};
  ir::NodePtr node = std::make_shared<ir::LoadNode>(instr->GetOpCode(), opnds);
  node->SetLineNo(instr->GetStartsLine());
  PushStack(node);
}
