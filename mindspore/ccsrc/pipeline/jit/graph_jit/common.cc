...
#include "pipeline/jit/graph_jit/graph_compiler/cg/byte_code_generator.h"
...
static void freeJitCompileResults(void *jitCompileResults) {
  ...
  Py_XDECREF(c->compiled.callable);
  c->compiled.callable = nullptr;
  c->codehub.reset();
...
static void GraphCapture(JitCompileResults *jcr) {
  ...
  bool is_graph_break = cg.TryToBreakGraphIfUnsupported();
...
void AddConfigToGuard(const GraphJitConfig &c, OptGuardPtr guard) {
  ...
  cfg[CFG_SPECIALIZE_INT_FLOAT] = c.GetBoolConfig(GraphJitConfig::kGuardSpecializeIntFloat);
  cfg[CFG_SPECIALIZE_TENSOR] = c.GetBoolConfig(GraphJitConfig::kGuardSpecializeTensor);
}
...
static void CallGraphCompiler(JitCompileResults *jcr, PyFunctionObject *func, PyFrameObject *frame,
  ...
  jcr->output_modified = ms_func_graph->modify_output();
...
static py::object CallGraph(const JitCompileResults *c, const py::object &args, const py::object &kwvargs) {
  ...
  if (c->output_modified) {
    // new_output_abs = {original_output_abs, added_output_abs}
    // original_output_abs : AbstractTuple | AbstractTensor | AbstractScalar
    // added_output_abs : AbstractTuple
    bool is_valid = PyTuple_Check(res) && (PyTuple_Size(res) == 2) && PyTuple_Check(PyTuple_GetItem(res, 1));
    MS_EXCEPTION_IF_CHECK_FAIL(is_valid, "The return value is invalid.");
    REPLACE_PY_MEMBER(res, PyTuple_GET_ITEM(res, 0));
  }
...
static py::object CallCompiledResults(PyThreadState *tstate, const PyFrameObject *f, const JitCompileResults *c) {
  ...
  if (c->func != nullptr && c->tbs != nullptr) {
    c->tbs->Clear();
  }
  return res;
}
...
py::bool_ graph_jit_should_compile(const py::object &funcHandle, const py::object &tag) {
  ...
  if (func != nullptr && PyFunction_Check(func)) {
    raw_func_name = mindspore::jit::graph::Utils::GetPyName(reinterpret_cast<PyFunctionObject *>(func)->func_name);
  }
...
py::bool_ graph_jit_enable() { return py::bool_(false); }
py::bool_ graph_jit_disable() { return py::bool_(false); }
py::bool_ graph_jit_should_compile(const py::object &func, const py::object &tag) {
