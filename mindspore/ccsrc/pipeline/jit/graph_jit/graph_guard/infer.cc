...
static PyObject *g_ms_module = NULL;
static PyObject *g_ms_type = NULL;
static PyObject *g_tensor_type = NULL;

static bool InitMsModule() {
  if (g_ms_module == NULL) {
    g_ms_module = PyImport_ImportModule("mindspore._c_expression");
  }
  return g_ms_module != NULL && g_ms_module != Py_None;
}
...
static bool InitMsTensor() {
  if (g_tensor_type == NULL && InitMsModule()) {
    g_tensor_type = PyObject_GetAttrString(g_ms_module, "MetaTensor");
  }
  return g_tensor_type != NULL && g_tensor_type != Py_None && PyType_Check(g_tensor_type);
}
...
bool InferEngine::Deinit() {
  if (bInit_) {
    if (!g_tensor_type) {
      Py_DECREF(g_tensor_type);
    }
    if (!g_ms_type) {
      Py_DECREF(g_ms_type);
    }
    if (!g_ms_module) {
      Py_DECREF(g_ms_module);
    }
    bInit_ = false;
  }
  return bInit_;
...
bool IsStubTensorType(PyTypeObject *tp) { TYPE_CHECK("mindspore.common._stub_tensor", "StubTensor", false); }
...
py::object FindTensorMethodMap(const std::string &name) {
  const auto &tensor_meth_map = mindspore::pipeline::GetMethodMap().find(TypeId::kObjectTypeTensorType)->second;
  auto iter = tensor_meth_map.find(name);
  if (iter == tensor_meth_map.end()) {
    return py::object();
  }
  const mindspore::Any &any = iter->second;
  if (any.is<std::string>()) {
    const std::string &method_name = any.cast<std::string>();
    py::object func = Utils::GetModuleAttr(parse::kStandardMethodModelName, method_name.c_str(), false, true);
    return func;
  }
  if (any.is<PrimitivePtr>()) {
    return py::object();
  }
  return py::object();
}
