 ...
 */
#include "pipeline/jit/graph_jit/graph_guard/guard_utils.h"
...
class TensorData : public MetaTensorData {
 public:
  ...
  bool operator==(const ItemData &obj) override {
    ...
    ret = ret && other.init_flag_ == init_flag_ && other.is_forward_output_ == is_forward_output_ &&
          ...
          other.padding_type_.compare(padding_type_) == 0 && other.compression_type_ == compression_type_ &&
 ...
 protected:
  std::string ToStringIntern() override {
    ...
    ret += DESC(padding_type_) + DESC_STRING(is_forward_output_) + DESC_STRING(init_flag_) + DESC_STRING(graph_output_);
    ret +=
  ...
  void StoreTensor(mindspore::tensor::TensorPtr tensor_ptr) {
    ...
    padding_type_ = tensor_ptr->padding_type();
...
static ItemDataPtr CreateTensorData(PyObject *obj, bool need_specialize, int recurse_depth) {
  ItemDataPtr dp;
  if (py::isinstance<mindspore::tensor::MapTensor>(obj)) {
    dp = std::make_shared<MapTensorData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::tensor::Tensor>(obj)) {
    dp = std::make_shared<TensorData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::ParamInfo>(obj)) {
    dp = std::make_shared<ParamInfoData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::tensor::MetaTensor>(obj)) {
    dp = std::make_shared<MetaTensorData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::tensor::TensorData>(obj)) {
    dp = std::make_shared<TensorDataData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::tensor::RowTensor>(obj)) {
    dp = std::make_shared<RowTensorData>(obj, false, recurse_depth);
  } else if (py::isinstance<mindspore::tensor::COOTensor>(obj)) {
    dp = std::make_shared<COOTensorData>(obj, false, recurse_depth);
  } else if (py::isinstance<mindspore::tensor::CSRTensor>(obj)) {
    dp = std::make_shared<CSRTensorData>(obj, false, recurse_depth);
  } else if (py::isinstance<mindspore::PrimitivePyAdapter>(obj)) {
    dp = std::make_shared<PrimitiveData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::Cell>(obj)) {
    dp = std::make_shared<CellData>(obj, need_specialize, recurse_depth);
  } else {
    dp = std::make_shared<UnknownData>(obj, need_specialize, recurse_depth);
  }
  return dp;
}
...
static ItemDataPtr CreateData(PyObject *obj, bool need_specialize, int recurse_depth) {
  ItemDataPtr dp;
  if (PyLong_Check(obj)) {
    dp = std::make_shared<IntData>(obj, need_specialize, recurse_depth);
  } else if (PyFloat_Check(obj)) {
    dp = std::make_shared<FloatData>(obj, need_specialize, recurse_depth);
  } else if (PyBool_Check(obj)) {
    dp = std::make_shared<BoolData>(obj, need_specialize, recurse_depth);
  } else if (PyBytes_Check(obj)) {
    dp = std::make_shared<BytesData>(obj, need_specialize, recurse_depth);
  } else if (PyUnicode_Check(obj)) {
    dp = std::make_shared<StringData>(obj, need_specialize, recurse_depth);
  } else if (PyList_Check(obj)) {
    dp = std::make_shared<ListData>(obj, need_specialize, recurse_depth);
  } else if (PyTuple_Check(obj)) {
    dp = std::make_shared<ListData>(obj, need_specialize, recurse_depth);
  } else if (PySet_Check(obj)) {
    dp = std::make_shared<ListData>(obj, need_specialize, recurse_depth);
    ...
    dp = std::make_shared<DictData>(obj, need_specialize, recurse_depth);
  } else if (PyComplex_Check(obj)) {
    dp = std::make_shared<ComplexData>(obj, need_specialize, recurse_depth);
  } else if (PySlice_Check(obj)) {
    dp = std::make_shared<SliceData>(obj, need_specialize, recurse_depth);
  } else if (PyFunction_Check(obj)) {
    dp = std::make_shared<FunctionData>(obj, need_specialize, recurse_depth);
  } else if (PyMethod_Check(obj)) {
    dp = std::make_shared<MethodData>(obj, need_specialize, recurse_depth);
  } else if (PyInstanceMethod_Check(obj)) {
    dp = std::make_shared<InstanceMethodData>(obj, need_specialize, recurse_depth);
  } else if (PyType_Check(obj)) {
    dp = std::make_shared<TypeData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<py::array>(obj)) {
    dp = std::make_shared<NumpyData>(obj, need_specialize, recurse_depth);
  } else if (py::isinstance<mindspore::Type>(obj)) {
    dp = std::make_shared<TensorTypeData>(obj, need_specialize, recurse_depth);
  } else {
    dp = CreateTensorData(obj, need_specialize, recurse_depth);
  }
  return dp;
}
...
static ItemDataPtr CreateItem(PyObject *obj, bool need_specialize, int recurse_depth) {
  ...
  if (obj != NULL && obj != Py_None) {
    ...
    auto pyObj = py::cast<py::object>(obj);
    if (py::hasattr(pyObj, mutable_attr) && py::cast<bool>(py::getattr(pyObj, mutable_attr))) {
      need_specialize = false;
    }
