 ...
 */
#include "pipeline/jit/graph_jit/graph_capture/abstract_object.h"
#include <algorithm>
#include <string>
#include <utility>
#include <vector>
...
AObject *AbstractObjectBase::MakeAObject(AObject::Type type, PyTypeObject *tp, PyObject *o, RecMap *m) {
  ...
  switch (type) {
    ...
    case kTypeString:
      ...
      res->SetTypeObject(&PyUnicode_Type);
      break;
...
AObject *AbstractType::BuildAbstractInstance(const std::vector<AObject *> &args, int opcode) {
  ...
  switch (type_type_) {
    ...
    case kTypeTuple:
      ...
      if (args[0]->GetType() == kTypeTuple || args[0]->GetType() == kTypeList) {
...
AbstractTuple::AbstractTuple(Type type, py::object seq, RecMap *rec)
  ...
  if (!seq.ptr()) {
    this->SetTypeObject(type == kTypeList ? &PyList_Type : &PyTuple_Type);
    return;
  }
  element_valid_ = true;
  this->SetTypeObject(Py_TYPE(seq.ptr()));
...
AbstractDict::AbstractDict(Type type, py::object seq, RecMap *rec)
    : AbstractSequence(type, seq),
      ...
      modify_(false) {
  if (!seq.ptr()) {
    this->SetTypeObject(&PyDict_Type);
    return;
  }
  element_valid_ = true;
  this->SetTypeObject(Py_TYPE(seq.ptr()));
...
bool AbstractTuple::Update() {
  ...
  for (Py_ssize_t i = items_.size() - 1; i >= 0; --i) {
...
AbstractTensor::AbstractTensor(const py::object &o, bool is_stub) : AbstractObject(kTypeTensor, o), is_stub_(is_stub) {
  PyTypeObject *tp = o.ptr() == nullptr ? Py_TYPE(GetUninitializedTensor()) : Py_TYPE(o.ptr());
  this->SetTypeObject(tp);
  // cache it
  (void)GetAttr("shape");
  (void)GetAttr("dtype");
}
...
AObject *AbstractTensor::GetAttr(const std::string &name) {
  ...
  // generic attribute
  PyObject *op = PyObject_GetAttrString(GetUninitializedTensor(), name.c_str());
