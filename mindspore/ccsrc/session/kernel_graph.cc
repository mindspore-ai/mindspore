...
void KernelGraph::SetExecOrderByDefault() {
  ...
  std::stack<AnfNodePtr> zero_output_nodes;
  std::unordered_set<AnfNodePtr> visited_nodes;
  auto clear_output = [&zero_output_nodes, &visited_nodes, this](const AnfNodePtr &input) -> void {
    if (node_output_num_[input] == 0 && visited_nodes.find(input) == visited_nodes.end()) {
      ...
      zero_output_nodes.emplace(input);
      if (input->isa<CNode>() && AnfAlgo::IsRealKernel(input)) {
        execution_order_.push_back(input->cast<CNodePtr>());
      }
    }
  };
  zero_output_nodes.emplace(get_return());
  while (!zero_output_nodes.empty()) {
    auto node = zero_output_nodes.top();
