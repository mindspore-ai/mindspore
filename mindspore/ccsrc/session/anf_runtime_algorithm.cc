...
bool AnfRuntimeAlgorithm::IsCompositeKernel(const AnfNodePtr &node) {
  MS_EXCEPTION_IF_NULL(node);
  // composite op kernel should be a real cnode kernel.
  ...
  // composite op kernel should has func_graph as first input.
  ...
  MS_EXCEPTION_IF_NULL(func_graph);
  return func_graph->has_attr(FUNC_GRAPH_FLAG_COMPOSITE);
}
