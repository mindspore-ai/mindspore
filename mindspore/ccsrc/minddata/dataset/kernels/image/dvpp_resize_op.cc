...
Status DvppResizeOp::Compute(const std::shared_ptr<DeviceTensorAscend910B> &input,
  ...
  CHECK_FAIL_RETURN_UNEXPECTED(input->GetShape().Rank() == 4, "DvppResize: the input rank is not 4.");
  ...
  RETURN_IF_NOT_OK(DvppResize(input, output, output_h, output_w, 0, 0, interpolation_));
