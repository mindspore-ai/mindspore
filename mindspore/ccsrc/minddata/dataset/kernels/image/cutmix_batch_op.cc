...
Status CutMixBatchOp::ComputeLabelsAndImages(const TensorRow &input, const std::shared_ptr<Tensor> &out_labels,
  ...
  for (int64_t i = 0; i < image_shape[0]; i++) {
    ...
    if (random_number < prob_) {
      ...
      if (image_batch_format_ == ImageBatchFormat::kNHWC) {
        // NHWC Format
        GetCropBox(static_cast<int32_t>(image_shape[dimension_one]), static_cast<int32_t>(image_shape[dimension_two]),
        ...
        label_lam =
          1 - (crop_width * crop_height / static_cast<float>(image_shape[dimension_one] * image_shape[dimension_two]));
      } else {
        // NCHW Format
        ...
        label_lam = 1 - (crop_width * crop_height /
...
Status CutMixBatchOp::Compute(const TensorRow &input, TensorRow *output) {
  ...
  RETURN_IF_NOT_OK(ComputeLabelsAndImages(input, out_labels, image_shape, label_shape, &images));
