...
Status EraseOp::Compute(const std::shared_ptr<Tensor> &input, std::shared_ptr<Tensor> *output) {
  ...
  RETURN_IF_NOT_OK(ValidateImageDtype("EraseOp", input->type()));
  if (input->Rank() != kDefaultImageRank) {
    RETURN_STATUS_UNEXPECTED("EraseOp: input tensor is not in shape of <H,W,C>, but got rank: " +
                             std::to_string(input->Rank()));
  }
  int num_channels = input->shape()[2];
  if (num_channels != kDefaultImageChannel) {
    RETURN_STATUS_UNEXPECTED("EraseOp: channel of input image should be 3, but got: " + std::to_string(num_channels));
  }
