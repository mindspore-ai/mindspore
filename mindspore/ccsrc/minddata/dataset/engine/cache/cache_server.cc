...
Status CacheServer::GlobalMemoryCheck(uint64_t cache_mem_sz) {
  ...
  while (it != end) {
    ...
    if (max_avail <= 0) {
      RETURN_STATUS_OOM("Out of memory, please destroy some sessions.");
    }
    ++it;
  }
...
Status CacheServer::CacheRow(CacheRequest *rq, CacheReply *reply) {
  ...
  if (cs == nullptr) {
    ...
    if (!cs->HasBuildPhase() || cookie == cs->cookie()) {
      ...
      RETURN_STATUS_UNEXPECTED("Cookie mismatch");
    }
  }
  return Status::OK();
}
...
Status CacheServer::FastCacheRow(CacheRequest *rq, CacheReply *reply) {
  ...
  if (cs == nullptr) {
    std::string errMsg = "Cache id " + std::to_string(connection_id) + " not found";
    rc = STATUS_ERROR(StatusCode::kMDUnexpectedError, errMsg);
  } else {
    // Only if the cookie matches, we can accept insert into this cache that has a build phase
    if (!cs->HasBuildPhase() || cookie == cs->cookie()) {
      row_id_type id = -1;
      ...
      if (state != CacheServiceState::kFetchPhase) {
        rc = STATUS_ERROR(StatusCode::kMDUnexpectedError, "Cache service is not in fetch phase. The current phase is " +
                                                            std::to_string(static_cast<int8_t>(state)) +
                                                            ". Client id: " + std::to_string(client_id));
      } else {
        rc = STATUS_ERROR(StatusCode::kMDUnexpectedError, "Cookie mismatch. Client id: " + std::to_string(client_id));
      }
    }
  }
...
Status CacheServer::ProcessRowRequest(CacheServerRequest *cache_req, bool *internal_request) {
  ...
  switch (cache_req->type_) {
    ...
    default:
      ...
      cache_req->rc_ = STATUS_ERROR(StatusCode::kMDUnexpectedError, errMsg);
  }
  return Status::OK();
}
...
Status CacheServer::DestroySession(CacheRequest *rq) {
  ...
  if (n > 0) {
    ...
    if (found) {
      ...
      RETURN_STATUS_ERROR(StatusCode::kMDFileNotExist, errMsg);
    }
  }
}
...
Status CacheServer::Builder::IpcResourceCleanup() {
  ...
  if (num_attached == 0) {
    ...
    RETURN_STATUS_ERROR(StatusCode::kMDDuplicateKey, errMsg);
  }
  return Status::OK();
}
