...
Status ModelProcess::InitInputsBuffer() {
  ...
  for (size_t i = 0; i < input_size; ++i) {
    ...
    const char *input_name_char = CALL_ASCEND_API(aclmdlGetInputNameByIndex, model_desc_, i);
  ...
  for (size_t i = 0; i < output_size; ++i) {
    ...
    if (ret != ACL_ERROR_NONE) {
      ...
      if (!is_run_on_device_) {
        ...
        (void)CALL_ASCEND_API(aclrtFreeHost, data_mem_buffer);
      }
      return kMCDeviceError;
    }
    aclDataType data_type = CALL_ASCEND_API(aclmdlGetOutputDataType, model_desc_, i);
    std::vector<int64_t> shape(dims.dims, dims.dims + dims.dimCount);
    const char *output_name_char = CALL_ASCEND_API(aclmdlGetOutputNameByIndex, model_desc_, i);
...
void ModelProcess::DestroyInputsDataset() {
  ...
  (void)CALL_ASCEND_API(aclmdlDestroyDataset, inputs_);
  inputs_ = nullptr;
}
...
void ModelProcess::DestroyInputsDataMem() {
  if (!is_run_on_device_) {
    for (const auto &item : input_infos_) {
      (void)CALL_ASCEND_API(aclrtFree, item.device_data);
    }
  }
  input_infos_.clear();
}
...
void ModelProcess::DestroyOutputsBuffer() {
  for (const auto &item : output_infos_) {
    if (!is_run_on_device_) {
      ...
      (void)CALL_ASCEND_API(aclrtFreeHost, item.device_data);
    }
  }
  output_infos_.clear();
  ...
  (void)CALL_ASCEND_API(aclmdlDestroyDataset, outputs_);
  outputs_ = nullptr;
}
...
Status ModelProcess::ResetOutputSize() {
  ...
  for (size_t index = 0; index < output_size; index++) {
    ...
    output_infos_[index].buffer_size = LongToSize(dims) * CALL_ASCEND_API(aclDataTypeSize, output_type);
  }
  return kSuccess;
}
