...
bool DistributedCountService::TriggerFirstCountEvent(const std::string &name, std::string *reason) {
  ...
  if (counter_handlers_.count(name) == 0 || !counter_handlers_[name].first_count_handler) {
    MS_LOG(WARNING) << "The counter handler of " << name << " is not registered.";
    return false;
  }
  // Leader server directly calls the callback.
  MS_LOG(DEBUG) << "Leader server call first count handler for " << name;
  auto count_handler = counter_handlers_[name].first_count_handler;
...
bool DistributedCountService::TriggerLastCountEvent(const std::string &name, std::string *reason) {
  ...
  if (counter_handlers_.count(name) == 0 || !counter_handlers_[name].last_count_handler) {
    MS_LOG(WARNING) << "The counter handler of " << name << " is not registered.";
    return false;
  }
  // Leader server directly calls the callback.
  MS_LOG(DEBUG) << "Leader server call last count handler for " << name;
  auto count_handler = counter_handlers_[name].last_count_handler;
  Server::GetInstance().SubmitTask([count_handler]() { count_handler(nullptr); });
