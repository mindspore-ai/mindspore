    ...
    if (fg != nullptr) {
      ...
      if (!input_abs->dynamic_len()) {
        for (size_t i = 0; i < input_abs->size(); ++i) {
          args_.emplace_back(
            fg->NewCNode({NewValueNode(prim::kPrimListGetItem), real_node, NewValueNode(SizeToLong(i))}));
        }
  ...
  AnfNodePtr operator()(const OptimizerPtr &, const AnfNodePtr &node) override {
    ...
    if (fg != nullptr) {
      ...
      if (!input_abs->dynamic_len()) {
        for (size_t i = 0; i < input_abs->size(); ++i) {
          args_.emplace_back(
            fg->NewCNode({NewValueNode(prim::kPrimTupleGetItem), real_node, NewValueNode(SizeToLong(i))}));
        }
        auto new_node = fg->NewCNode(args_);
        new_node->set_abstract(node->abstract());
        return new_node;
