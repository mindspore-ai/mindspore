...
int64_t CalBMMFlops(const std::vector<ShapeVector> &input_shapes, bool transpose_b) {
  ...
  (void)CheckAndConvertUtils::CheckInteger("rank of 'two batchmatmul inputs'", a_shape.size(), kEqual, b_shape.size(),
                                           "BMM");
  ...
  flops *= transpose_b ? k_double_expand_ratio * a_shape[a_dim_index] * a_shape[b_dim_index] * b_shape[a_dim_index]
                       : k_double_expand_ratio * a_shape[a_dim_index] * a_shape[b_dim_index] * b_shape[b_dim_index];
  return flops;
}
...
OpInfoPtr GetBpNodeInfo(const CNodePtr &node, std::unordered_map<std::string, OpInfoPtr> op_info_map_dx_dw_map) {
  ...
  if (IsPrimitiveCNode(node, prim::kPrimFlashAttentionScoreGrad)) {
    ...
    op_info->origin_input_shapes = fwd_info->origin_input_shapes;
    op_info->shard_input_shapes = fwd_info->shard_input_shapes;
    op_info->shard_flops = fwd_info->shard_flops * forward_unique_id_list.size();
    op_info->full_flops = fwd_info->full_flops * forward_unique_id_list.size();
  }
  return op_info;
}
