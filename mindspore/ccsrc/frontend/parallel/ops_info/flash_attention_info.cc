...
Status FlashAttentionPrimitiveInfo::GetAttrs() {
  ...
  if (inputs_shape_.size() == FLASH_ATTENTION_INPUTS_SIZE) {
    MS_LOG(INFO) << name_ << ": Inputs shape size is 6, alibi mask not none.";
    alibi_mask_valid_ = true;
  }
  if (inputs_shape_.size() >= FLASH_ATTENTION_INPUTS_SIZE - 1) {
    MS_LOG(INFO) << name_ << ": Inputs shape size is 5, dropout mask not none.";
    dropout_mask_valid_ = true;
  }
  auto input_sz = inputs_shape_.size();
  bool input_sz_ok = input_sz <= FLASH_ATTENTION_INPUTS_SIZE && input_sz >= FLASH_ATTENTION_INPUTS_SIZE - 2;
  ...
  size_t attn_mask_d_sz = inputs_shape_.at(3).size();
  ...
  if (!(q_dim_sz == qkv_dim_sz && k_dim_sz == qkv_dim_sz && v_dim_sz == qkv_dim_sz &&
        attn_mask_d_sz == attn_mask_dim_sz)) {
    MS_LOG(ERROR) << name_ << ": The dim of Q, K, V, attn_mask dim should be equal to 4, ";
    return FAILED;
  }
  if (dropout_mask_valid_) {
  ...
  if (alibi_mask_valid_) {
    size_t alibi_mask_d_sz = inputs_shape_.at(5).size();
...
Status FlashAttentionPrimitiveInfo::CheckStrategy(const StrategyPtr &strategy) {
  ...
  Dimensions stgy_attn_mask_dim = stra.at(3);
  ...
  if (!(stgy_q_dim_sz == qkv_dim_sz && stgy_k_dim_sz == qkv_dim_sz && stgy_v_dim_sz == qkv_dim_sz &&
        stgy_attn_msk_dim_sz == attn_mask_dim_sz)) {
  ...
  uint64_t attn_mask_input_bs = inputs_shape_[3][0];
  ...
  if (!(q_stgy_shard_ok && k_stgy_shard_ok && v_stgy_shard_ok && attn_msk_stgy_shard_ok)) {
    MS_LOG(ERROR) << name_ << ": sharding strategy configuration wrong.";
    return FAILED;
  }
  if (dropout_mask_valid_) {
    Dimensions stgy_dropout_mask_dim = stra.at(4);
    MS_LOG(INFO) << "stgy_dropout_mask_dim:" << ShapeToString(stgy_dropout_mask_dim);
    size_t stgy_dropout_msk_dim_sz = stgy_dropout_mask_dim.size();
    if (stgy_dropout_msk_dim_sz != dropout_mask_dim_sz) {
      MS_LOG(ERROR) << name_ << ": The dimensions dropout mask strategy is wrong.";
      return FAILED;
    }
    bool dropout_msk_stgy_shard_ok =
      std::equal(stgy_dropout_mask_dim.begin(), stgy_dropout_mask_dim.end(), dp_mp_shard_cfg.begin());
    if (!dropout_msk_stgy_shard_ok) {
      MS_LOG(ERROR) << name_ << ": dropout mask sharding strategy configuration wrong.";
      return FAILED;
    }
  }
  if (alibi_mask_valid_) {
    Dimensions stgy_alibi_mask_dim = stra.at(5);
