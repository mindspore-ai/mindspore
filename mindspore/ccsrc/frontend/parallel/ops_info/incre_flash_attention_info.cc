...
void IncreFlashAttentionInfo::SetOptinalInputs() {
  optinal_inputs.resize(ops::kIncreFlashAttentionInputsNum, true);
  ...
  if (atten_mask_rank == rank_2) {
    optinal_tensor_map[ops::kIncreFlashAttentionInputAttnMaskIndex] = {-1, -1};
    optinal_op_strategies[ops::kIncreFlashAttentionInputAttnMaskIndex] = {0, 0};
  }
  if (atten_mask_rank == rank_3) {
    optinal_tensor_map[ops::kIncreFlashAttentionInputAttnMaskIndex] = {1, -1, -1};
    optinal_op_strategies[ops::kIncreFlashAttentionInputAttnMaskIndex] = {1, 0, 0};
  }
  if (padding_mask_rank == rank_2) {
    optinal_tensor_map[ops::kIncreFlashAttentionInputPaddingMaskIndex] = {-1, -1};
    optinal_op_strategies[ops::kIncreFlashAttentionInputPaddingMaskIndex] = {0, 0};
  }
  if (padding_mask_rank == rank_3) {
    optinal_tensor_map[ops::kIncreFlashAttentionInputPaddingMaskIndex] = {1, -1, -1};
    optinal_op_strategies[ops::kIncreFlashAttentionInputPaddingMaskIndex] = {1, 0, 0};
  }
}
...
Status IncreFlashAttentionInfo::InferTensorMap() {
  ...
  for (auto index = static_cast<size_t>(ops::kIncreFlashAttentionInputAttnMaskIndex); index < optinal_inputs.size();
       index++) {
