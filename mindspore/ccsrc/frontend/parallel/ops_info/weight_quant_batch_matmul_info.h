...
#ifndef MINDSPORE_CCSRC_FRONTEND_PARALLEL_OPS_INFO_WEIGHT_QUANT_BATCH_MATMUL_INFO_H_
#define MINDSPORE_CCSRC_FRONTEND_PARALLEL_OPS_INFO_WEIGHT_QUANT_BATCH_MATMUL_INFO_H_
...
namespace mindspore {
namespace parallel {
class WeightQuantBatchMatmulInfo : public OperatorInfo {
 public:
  // Generate all strategies and the corresponding cost for this WeightQuantBatchMatmulInfo operator
  WeightQuantBatchMatmulInfo(const std::string &name, const Shapes &inputs_shape, const Shapes &outputs_shape,
                             const PrimitiveAttrs &attrs)
      : OperatorInfo(name, inputs_shape, outputs_shape, attrs, std::make_shared<MatMulCost>()) {}
  ~WeightQuantBatchMatmulInfo() override = default;
 ...
 protected:
  ...
  size_t mat_weight_dimension_ = 0;
  Shape origin_dev_matrix_shape_;

 private:
  Status CheckInputStrategy(const Shape &mat_x_strategy, const Shape &mat_weight_strategy);
};

}  // namespace parallel
}  // namespace mindspore
#endif  // MINDSPORE_CCSRC_FRONTEND_PARALLEL_OPS_INFO_WEIGHT_QUANT_BATCH_MATMUL_INFO_H_
