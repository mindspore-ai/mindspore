...
CameAllReduceInserter::CameAllReduceInserter(ParameterPtr origin, const std::vector<AnfNodePtr> &all_parameters,
                                             const NodeUsersMap &node_user_map)
...
void CameAllReduceInserter::FindCameParams() {
  ...
  for (const auto &param_node : all_parameters) {
    ...
    if (param_name == exp_row_name) {
      MS_LOG(WARNING) << "[CAME] found exp_avg_sq_row: " << param_name;
      exp_avg_sq_row = param;
      cur_found_param_count++;
    } else if (param_name == exp_col_name) {
      MS_LOG(WARNING) << "[CAME] found exp_avg_sq_col: " << param_name;
      exp_avg_sq_col = param;
      cur_found_param_count++;
    } else if (param_name == exp_insta_row_name) {
      MS_LOG(WARNING) << "[CAME] found exp_avg_insta_row: " << param_name;
      exp_avg_insta_row = param;
      cur_found_param_count++;
    } else if (param_name == exp_insta_col_name) {
      MS_LOG(WARNING) << "[CAME] found exp_avg_insta_col: " << param_name;
      exp_avg_insta_col = param;
      cur_found_param_count++;
    } else if (param_name == exp_avg_name) {
      MS_LOG(WARNING) << "[CAME] found exp_avg: " << param_name;
      exp_avg = param;
      cur_found_param_count++;
    }
  ...
  MS_LOG(WARNING) << "[CAME] found params corresponding to origin param size: " << cur_found_param_count;
}

std::pair<Status, RankList> CameAllReduceInserter::GetOptShardRankList(const int64_t rank) {
  DeviceMatrix temp_dev_matrix(rank, full_rank_list, tensor_layout->device_arrangement().array());
  RankList group_devices;
  Shape orig_tensor_map = tensor_layout->origin_tensor_map().array();
...
std::pair<Status, RankList> CameAllReduceInserter::GetDimRankList(const int64_t rank, const int64_t dim) {
...
RankList CameAllReduceInserter::ExpandRankListWithOptShard(const RankList &rank_list) {
  if (!is_opt_shard) {
    return rank_list;
  }
...
RankList CameAllReduceInserter::ExpandRankListWithDim(const RankList &rank_list, const int64_t dim) {
  RankList expanded_list;
  for (const auto &rank : rank_list) {
    Status ret_status;
...
CNodePtr CameAllReduceInserter::FindReduceMean(size_t number) {
  if (reduce_mean_numbers.find(number) == reduce_mean_numbers.end()) {
    MS_LOG(ERROR) << "[CAME] invalid reduce mean number: " << number;
  }
...
CNodePtr CameAllReduceInserter::FindReduceMean1256(const ParameterPtr &param) {
  MS_LOG(WARNING) << "[CAME] try find reduce_mean according to " << param->name() << " Assign:";
  if (!param) {
    return nullptr;
  }
...
CNodePtr CameAllReduceInserter::FindReduceMean37(const ParameterPtr &param) {
  if (!param) {
    return nullptr;
  }
  auto param_user_set = node_user_map.at(param->cast<AnfNodePtr>());
  MS_LOG(WARNING) << "[CAME] user map size: " << param_user_set.size();
  size_t load_count = 0;
  for (auto &param_pair : param_user_set) {
    auto user_cnode = param_pair.first->cast<CNodePtr>();
    MS_EXCEPTION_IF_NULL(user_cnode);
    if (IsSomePrimitive(user_cnode, LOAD)) {
      MS_LOG(WARNING) << "[CAME] found load node";
      load_count++;
      // load -> reduce mean
      auto res = ForwardSearchCNode(user_cnode, {LOAD, REDUCE_MEAN}, node_user_map);
      if (res.first) {
        MS_LOG(WARNING) << "[CAME] found reduce mean node size: " << res.second.size();
        return res.second[0];  // get the first one
      }
    }
  }
  MS_LOG(WARNING) << "[CAME] found load count: " << load_count;
  return nullptr;
}

CNodePtr CameAllReduceInserter::FindReduceMean4() {
  MS_LOG(WARNING) << "[CAME] try find reduce_mean no.4 according to exp_avg Assign:";
  if (!exp_avg) {
    return nullptr;
  }
  ...
  for (auto &param_pair : exp_avg_user_set) {
    ...
    MS_EXCEPTION_IF_NULL(user_cnode);
    if (IsSomePrimitive(user_cnode, ASSIGN)) {
      MS_LOG(WARNING) << "[CAME] found exp_avg's assign node";
...
void CameAllReduceInserter::InsertAllReduceAndRealDivToReduceMeanInput(CNodePtr reduce_mean,
                                                                       const RankList &comm_rank_list) {
  ...
  MS_LOG(WARNING) << "[CAME] came allreduce opt shard group: " << group_name;
...
void CameAllReduceInserter::Process() {
...
std::string CameAllReduceInserter::CreateCommGroupFromRankList(const RankList &rank_list) {
  Group comm_group;
...
void HandleAdaFactorOpt(const FuncGraphPtr &root, const std::vector<AnfNodePtr> &all_nodes,
  ...
  for (const auto &origin_param_node : origin_params) {
    auto inserter =
      CameAllReduceInserter(origin_param_node->cast<ParameterPtr>(), root->parameters(), manager->node_users());
    inserter.Process();
  }
}
