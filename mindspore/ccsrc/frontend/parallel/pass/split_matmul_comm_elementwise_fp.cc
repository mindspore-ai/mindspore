...
static void SplitIntoInterleaved(const FuncGraphPtr &func_graph, const FuncGraphManagerPtr &manager,
  ...
  auto matmul_input1_shape = BaseShapeToShape(common::AnfAlgo::GetOutputDetailShape(matmul_input1, 0));
  if (matmul_input1_shape[axis_a_0] % kInt64Num2 != 0) {
    return;
  }
  auto matmul_input2 = matmul_cnode->input(kIndex2);
  auto matmul_input2_shape = BaseShapeToShape(common::AnfAlgo::GetOutputDetailShape(matmul_input2, 0));
...
void SplitMatmulCommElementwiseFp(const FuncGraphPtr &func_graph) {
  ...
  if (speedup_config.find("interleaved_matmul_comm") == speedup_config.end() ||
      !speedup_config["interleaved_matmul_comm"]) {
    return;
  }
