...
CNodePtr GetInputBorderNode(const CNodePtr &node) {
  ...
  while (!anf_queue.empty() && loop_count < 100) {
    ...
    anf_queue.pop();
    if (queue_end->HasAttr("fine_grained_interleaved_border")) {
      return queue_end;
    }
...
void InsertDependForEnd(const FuncGraphManagerPtr &manager,
  ...
  if (!end->HasAttr("fine_grained_interleaved_border")) {
...
void InsertDependForBegin(const FuncGraphManagerPtr &manager,
  ...
  if (!begin->HasAttr("fine_grained_interleaved_border")) {
    return;
  }
  auto comm_node_a_input_node = comm_node_a->input(1)->cast<CNodePtr>();
  std::vector<AnfNodePtr> depend2_inputs{NewValueNode(prim::kPrimDepend), begin, comm_node_a_input_node};
  ...
  depend_node2->set_abstract(begin->abstract()->Clone());
  MS_EXCEPTION_IF_NULL(depend_node2);
  manager->Replace(begin, depend_node2);
}
