...
CNodePtr NewSplitCNodeAndSetAbstract(const FuncGraphPtr &func_graph, const AnfNodePtr &input_node, int64_t axis,
                                     int64_t output_num) {
  MS_EXCEPTION_IF_NULL(input_node);
  auto input_node_shape = BaseShapeToShape(common::AnfAlgo::GetOutputDetailShape(input_node, 0));
  if (output_num == 0) {
...
void SplitLayerNormCommFp(const FuncGraphPtr &func_graph) {
  ...
  if (speedup_config.find("interleaved_layernorm_comm") == speedup_config.end() ||
      !speedup_config["interleaved_layernorm_comm"]) {
    return;
  }
