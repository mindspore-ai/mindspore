...
void SpreadFineGrainedInterleavedIndexForForwardCommNodes(const CNodePtr &cnode, size_t fine_grained_block_index,
  ...
  cnode->AddPrimalAttr(parallel::FINE_GRAINED_INTERLEAVED_BLOCK, MakeValue<int64_t>(fine_grained_block_index));
  cnode->AddAttr(parallel::MICRO_INTERLEAVED_INDEX, MakeValue<size_t>(fine_grained_interleaved_index));
  cnode->AddAttr("fine_grained_interleaved_border", MakeValue<size_t>(1));
}
...
void LabelFineGrainedInterleavedIndex(const FuncGraphPtr &graph) {
  ...
  static const auto is_cell_reuse = common::GetEnv("MS_DEV_CELL_REUSE") != "0";
  if (is_cell_reuse) {
  ...
  auto forward_order_cnodes = forward_graph->GetOrderedCnodes();
  ...
  LabelFineGrainedInterleavedBackWardBeginEnd(backward_order_cnode_list);
  LabelMicroInterleavedIndexForBackwardCommNodes(backward_order_cnode_list);
}
}  // namespace parallel
}  // namespace mindspore
