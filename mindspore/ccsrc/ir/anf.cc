...
static bool IsParrellOps(const std::string& primitive_name) {
  return ((primitive_name == parallel::ALL_GATHER) || (primitive_name == parallel::ALL_REDUCE) ||
          (primitive_name == parallel::REDUCE_SCATTER) || (primitive_name == parallel::ALL_TO_ALL));
}
...
std::string CNode::fullname_with_scope() {
  ...
  if (IsApply(prim::kPrimScalarSummary) || IsApply(prim::kPrimTensorSummary) || IsApply(prim::kPrimImageSummary)) {
    ...
    if (IsParrellOps(prim->name())) {
      fullname_with_scope_ = scope()->name() + "/" + prim->name() + "-op" + get_parallel_ops_id();
      MS_LOG(INFO) << "fullname of parallel ops: " << fullname_with_scope_;
    } else {
      fullname_with_scope_ = scope()->name() + "/" + prim->name() + "-op" + get_id();
    }
  }
...
std::string ValueNode::fullname_with_scope() {
  ...
  MS_EXCEPTION_IF_NULL(scope());
  fullname_with_scope_ = scope()->name() + "/" + "data-" + get_id();
  return fullname_with_scope_;
}
...
std::string get_id() { return std::to_string(id_count++); }

void reset_id() { id_count = 0; }

std::string get_parallel_ops_id() {
  static int parallel_ops_id_count = 0;
  return std::to_string(parallel_ops_id_count++);
}
}  // namespace mindspore
