...
CNodePtr InsertCastForInput(const FuncGraphPtr &func_graph, const CNodePtr &cnode) {
  ...
  for (size_t input_index = 0; input_index < AnfAlgo::GetInputTensorNum(cnode); ++input_index) {
    ...
    if (kernel::IsWeightBoundary(real_input_node) || func_graph->has_attr(FUNC_GRAPH_FLAG_COMPOSITE)) {
      // weight
    ...
    // In composite, we check parameter,
    // the eliminate pass will not eliminate this case, so we just do not insert the noused cast.
    if (func_graph->has_attr(FUNC_GRAPH_FLAG_COMPOSITE) && IsValueNode<tensor::Tensor>(cur_input)) {
