...
class GPUHashTable : public HashTable<Key, Value> {
 public:
  ...
  bool Find(const Key *key, size_t key_num, const std::string &initializer, Value *outputs) override { return true; }
  ...
  bool Find(const Key *key, size_t key_num, const Value &default_value, Value *outputs) override { return true; }
