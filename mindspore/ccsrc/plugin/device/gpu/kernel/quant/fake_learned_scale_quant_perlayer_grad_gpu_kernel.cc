...
bool FakeLearnedScaleQuantPerLayerGradGpuKernelMod::Init(const CNodePtr &kernel_node) {
  ...
  if (input_num != kDimSizeFour) {
    MS_LOG(EXCEPTION) << "For FakeLearnedScaleQuantPerLayerGradGpuKernelMod('" << kernel_name << "'), the number of "
                      << "inputs should be 4, but got " << input_num;
  }

  size_t output_num = common::AnfAlgo::GetOutputTensorNum(kernel_node);
  if (output_num != kDimSizeTwo) {
    MS_LOG(EXCEPTION) << "For FakeLearnedScaleQuantPerLayerGradGpuKernelMod('" << kernel_name << "'), the number "
  ...
  if (quant_delay_ < 0) {
    MS_LOG(EXCEPTION) << "For FakeLearnedScaleQuantPerLayerGradGpuKernelMod('" << kernel_name << "'), the value "
...
bool FakeLearnedScaleQuantPerLayerGradGpuKernelMod::Launch(const std::vector<AddressPtr> &inputs,
  ...
  float *input_quant_max = GetDeviceAddress<float>(inputs, kDimIndexThird);
