...
bool FakeLearnedScaleQuantPerChannelGpuKernelMod::Init(const CNodePtr &kernel_node) {
  ...
  if (input_num != kDimSizeThree) {
    MS_LOG(EXCEPTION) << "For FakeLearnedScaleQuantPerChannelGpuKernelMod('" << kernel_name << "'), the number of "
                      << "inputs should be 3, but got " << input_num;
  }

  size_t output_num = common::AnfAlgo::GetOutputTensorNum(kernel_node);
  if (output_num != 1) {
    MS_LOG(EXCEPTION) << "For FakeLearnedScaleQuantPerChannelGpuKernelMod('" << kernel_name << "'), the number of "
                      << "outputs should be 1, but got " << output_num;
  }
...
bool FakeLearnedScaleQuantPerChannelGpuKernelMod::Launch(const std::vector<AddressPtr> &inputs,
  ...
  float *input_alpha = GetDeviceAddress<float>(inputs, kDimIndexFirst);
  float *input_quant_max = GetDeviceAddress<float>(inputs, kDimIndexSecond);
