...
tensor::BaseTensorPtr NonZeroGPUCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &input_tensor) {
  ...
  auto input_tensor_dim = input_tensor->data().ndim();
  if (input_tensor_dim == 0) {
    MS_EXCEPTION(ValueError) << "For Primitive[NonZero], the dimension of 'x' "
                             << "must be greater than or equal to 1, but got 0.";
  }
