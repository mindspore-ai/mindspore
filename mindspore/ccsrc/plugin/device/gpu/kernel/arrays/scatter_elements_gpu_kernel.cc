...
#include "plugin/device/gpu/kernel/arrays/scatter_elements_gpu_kernel.h"
#include "mindspore/core/abstract/utils.h"

namespace mindspore {
namespace kernel {
void ScatterElementsGpuKernelMod::InitSizeLists() {
  input_size_list_.push_back(input_byte_size_);
  input_size_list_.push_back(indices_byte_size_);
  input_size_list_.push_back(updates_byte_size_);
  output_size_list_.push_back(input_byte_size_);
}

void ScatterElementsGpuKernelMod::ResetResource() {
  input_size_list_.clear();
  output_size_list_.clear();
}

void ScatterElementsGpuKernelMod::FreeResource() {
...
void ScatterElementsGpuKernelMod::MallocResource() {
...
void ScatterElementsGpuKernelMod::GetSize() {
  ...
  updates_byte_size_ = data_unit_size_;
  for (const auto &shape_item : updates_shape_) {
    updates_byte_size_ *= shape_item;
  }
...
void ScatterElementsGpuKernelMod::GetOpTypeAndFuncType(const BaseOperatorPtr &base_operator) {
  static const std::map<std::string, ScatterElementsReductionType> kTensorScatterOpTypeMap = {
    {"ScatterElements", REDUCTION_ASSIGNMENT}, {"ScatterAddWithAxis", REDUCTION_ADD}};
  auto op_type_iter = kTensorScatterOpTypeMap.find(kernel_name_);
  if (op_type_iter == kTensorScatterOpTypeMap.end()) {
    MS_LOG(EXCEPTION)
      << "Only support these scatter reduction function: ScatterElements, ScatterAdd currently, but got "
      << kernel_name_;
    return;
  }
  reduction_type_ = op_type_iter->second;
}

bool ScatterElementsGpuKernelMod::Init(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
  ...
  if (kernel_name_ == kScatterAddWithAxis) {
    ...
    MS_LOG(ERROR) << "'" << kernel_name_ << "' not support.";
    return false;
  }
...
int ScatterElementsGpuKernelMod::ShapeCheck() {
  if (input_dims_ < 1) {
...
int ScatterElementsGpuKernelMod::AxisCheck() {
...
int ScatterElementsGpuKernelMod::Resize(const BaseOperatorPtr &base_operator,
...
bool ScatterElementsGpuKernelMod::LaunchKernel(const std::vector<AddressPtr> &inputs,
  ...
  if (sync_resource_) {
    ...
    CHECK_CUDA_RET_WITH_EXCEPT_NOTRACE(
      ...
      "cudaMemcpy failed in ScatterElementsGpuKernelMod::Launch.");
  ...
  if (update_input_) {
    ScatterElements(reduction_type_, input_dims_, indices_byte_size_ / indices_unit_size_, indices, updates, input,
                    axis_, input_axis_size_, d_indices_stride_, d_output_stride_, device_id_,
                    reinterpret_cast<cudaStream_t>(stream_ptr));
    ...
    ScatterElements(reduction_type_, input_dims_, indices_byte_size_ / indices_unit_size_, indices, updates, output,
                    axis_, input_axis_size_, d_indices_stride_, d_output_stride_, device_id_,
                    reinterpret_cast<cudaStream_t>(stream_ptr));
  }
...
#define SCATTER_ELEMENTS_GPU_REG(MS_T, MS_S, T, S)                                           \
  KernelAttr().AddInputAttr(MS_T).AddInputAttr(MS_S).AddInputAttr(MS_T).AddOutputAttr(MS_T), \
    &ScatterElementsGpuKernelMod::LaunchKernel<T, S>

std::vector<std::pair<KernelAttr, ScatterElementsGpuKernelMod::ScatterElementsFunc>>
  ScatterElementsGpuKernelMod::func_list_ = {
...
std::vector<KernelAttr> ScatterElementsGpuKernelMod::GetOpSupport() {
  std::vector<KernelAttr> support_list;
  (void)std::transform(func_list_.begin(), func_list_.end(), std::back_inserter(support_list),
                       [](const std::pair<KernelAttr, ScatterElementsFunc> &pair) { return pair.first; });
  return support_list;
}

MS_KERNEL_FACTORY_REG(NativeGpuKernelMod, ScatterElements, ScatterElementsGpuKernelMod);
MS_KERNEL_FACTORY_REG(NativeGpuKernelMod, ScatterAddWithAxis, ScatterElementsGpuKernelMod);
}  // namespace kernel
}  // namespace mindspore
