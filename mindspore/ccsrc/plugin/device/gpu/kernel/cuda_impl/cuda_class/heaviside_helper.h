...
#ifndef MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_HEAVISIDE_HELPER_H_
#define MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_HEAVISIDE_HELPER_H_
#include <memory>
#include <string>
#include <vector>
...
template <typename T>
class HeavisideHelperGpuKernel : public GpuKernelHelperBase {
 public:
  explicit HeavisideHelperGpuKernel(const std::string &kernel_name, const uint32_t &device_id)
      : GpuKernelHelperBase(kernel_name, device_id) {
    is_null_input_ = false;
    need_broadcast_ = false;
  }

  virtual ~HeavisideHelperGpuKernel() = default;
  ...
  int Process(const std::vector<void *> &input_ptrs, const std::vector<void *> &output_ptrs,
    ...
    // call cuda kernel
    if (need_broadcast_) {
      BroadcastHeaviside(lhs_shape_, rhs_shape_, output_shape_, inputx_ptr, inputy_ptr, output_ptr, device_id_,
                         reinterpret_cast<cudaStream_t>(cuda_stream));
    } else {
      CalHeaviside(output_num_, inputx_ptr, inputy_ptr, output_ptr, device_id_,
                   reinterpret_cast<cudaStream_t>(cuda_stream));
    }
...
#endif  // MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_HEAVISIDE_HELPER_H_
