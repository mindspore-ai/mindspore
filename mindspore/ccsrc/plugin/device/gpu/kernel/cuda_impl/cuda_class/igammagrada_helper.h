...
#ifndef MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_IGAMMAGRADA_HELPER_H_
#define MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_IGAMMAGRADA_HELPER_H_
#include <memory>
#include <string>
#include <vector>
...
template <typename T>
class IgammaGradAHelperGpuKernel : public GpuKernelHelperBase {
 public:
  explicit IgammaGradAHelperGpuKernel(const std::string &kernel_name, const uint32_t &device_id)
  ...
  int Process(const std::vector<void *> &input_ptrs, const std::vector<void *> &output_ptrs,
    ...
    // call cuda kernel
    if (need_broadcast_) {
      status = CalBroadcastIgammaGradA(lhs_shape_, rhs_shape_, output_shape_, inputa_ptr, inputx_ptr, output_ptr,
                                       device_id_, reinterpret_cast<cudaStream_t>(cuda_stream));
    } else {
      status = CalIgammaGradA(size, nobroadcast_type_, inputa_ptr, inputx_ptr, output_ptr, device_id_,
...
#endif  // MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_IGAMMAGRADA_HELPER_H_
