...
#ifndef MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_ADDCMUL_HELPER_H_
#define MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_ADDCMUL_HELPER_H_
#include <memory>
#include <string>
#include <vector>
#include <typeinfo>
...
class AddcmulHelperGpuKernel : public GpuKernelHelperBase {
 public:
  explicit AddcmulHelperGpuKernel(const std::string &kernel_name, const uint32_t &device_id)
      : GpuKernelHelperBase(kernel_name, device_id) {
    is_null_input_ = false;
  }

  virtual ~AddcmulHelperGpuKernel() = default;
  ...
  int Process(const std::vector<void *> &input_ptrs, const std::vector<void *> &output_ptrs,
    ...
    int64_t input_data_size = static_cast<int64_t>(input_data_shape_.size());
    int64_t x1_size = static_cast<int64_t>(x1_shape_.size());
    int64_t x2_size = static_cast<int64_t>(x2_shape_.size());
    int64_t value_size = static_cast<int64_t>(value_shape_.size());

    input_data_shape_.insert(input_data_shape_.begin(), MAX_SHAPE_SIZE - input_data_size, 1);
    x1_shape_.insert(x1_shape_.begin(), MAX_SHAPE_SIZE - x1_size, 1);
    x2_shape_.insert(x2_shape_.begin(), MAX_SHAPE_SIZE - x2_size, 1);
    value_shape_.insert(value_shape_.begin(), MAX_SHAPE_SIZE - value_size, 1);
...
#endif  // MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_ADDCMUL_HELPER_H_
