...
cudaError_t ApplyLambEraly(const size_t size, T *variable, T *m, T *v, const float *beta1, const float *beta2,
                           ...
                           float *update, float *var_float, float *grad_float, cudaStream_t cuda_stream) {
  ApplyLambEralyKernel<<<GET_BLOCKS(size), GET_THREADS, 0, cuda_stream>>>(
    size, variable, m, v, beta1, beta2, epsilon, decay, global_step, gradient, update, var_float, grad_float);
...
template CUDA_LIB_EXPORT cudaError_t ApplyLambEraly<float>(const size_t size, float *variable, float *m, float *v,
                                                           ...
                                                           float *update_square_ptr, cudaStream_t cuda_stream);
