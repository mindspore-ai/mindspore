...
template <BinaryOpType OP, typename IN0, typename IN1, typename OUT, size_t vec_num>
__device__ void ApplyVec(BinaryFunc<OP, IN0, IN1, OUT> func, ScalarOption scalar_option, IN0 *in0_addr, IN1 *in1_addr,
                         OUT *out_addr) {
  Vec<OUT, vec_num> out_vec;
  if (scalar_option == ScalarOption::NoScalar) {
    Vec<IN0, vec_num> in0_vec = reinterpret_cast<Vec<IN0, vec_num> *>(in0_addr)[0];
    Vec<IN1, vec_num> in1_vec = reinterpret_cast<Vec<IN1, vec_num> *>(in1_addr)[0];
#pragma unroll
...
#pragma unroll
    ...
    Vec<IN0, vec_num> in0_vec = reinterpret_cast<Vec<IN0, vec_num> *>(in0_addr)[0];
    IN1 in1_data = in1_addr[0];
#pragma unroll
    for (size_t idx = 0; idx < vec_num; ++idx) {
      out_vec.data[idx] = func(in0_vec.data[idx], in1_data);
    }
  }
  Vec<OUT, vec_num> *out_data = reinterpret_cast<Vec<OUT, vec_num> *>(out_addr);
  out_data[0] = out_vec;
}
...
__global__ void BinaryWithBroadcastNoVecCuda(BinaryFunc<OP, IN0, IN1, OUT> func, size_t dim_size, size_t total_threads,
                                             BinaryBroadcastStrideInfo strides, IN0 *in0, IN1 *in1, OUT *out) {
...
template <BinaryOpType OP, typename IN0, typename IN1, typename OUT>
__global__ void BinaryWithoutBroadcastNoVecCuda(BinaryFunc<OP, IN0, IN1, OUT> func, ScalarOption scalar_option,
                                                size_t total_threads, IN0 *in0, IN1 *in1, OUT *out) {
  for (size_t pos = blockIdx.x * blockDim.x + threadIdx.x; pos < total_threads; pos += blockDim.x * gridDim.x) {
    IN0 in0_data = (scalar_option == ScalarOption::In0Scalar) ? in0[0] : in0[pos];
    IN1 in1_data = (scalar_option == ScalarOption::In1Scalar) ? in1[0] : in1[pos];
    out[pos] = func(in0_data, in1_data);
  }
}
template <BinaryOpType OP, typename IN0, typename IN1, typename OUT, size_t vec_num>
__global__ void BinaryBroadcastVecWithoutTailCuda(BinaryFunc<OP, IN0, IN1, OUT> func, ScalarOption scalar_option,
  ...
  for (size_t pos = blockIdx.x * blockDim.x + threadIdx.x; pos < total_threads; pos += blockDim.x * gridDim.x) {
    ...
    ApplyVec<OP, IN0, IN1, OUT, vec_num>(func, scalar_option, in0 + in_pos.data[0], in1 + in_pos.data[1],
                                         out + out_pos);
  }
}
template <BinaryOpType OP, typename IN0, typename IN1, typename OUT, size_t vec_num>
__global__ void BinaryBroadcastVecWithTailCuda(BinaryFunc<OP, IN0, IN1, OUT> func, ScalarOption scalar_option,
  ...
  for (size_t pos = blockIdx.x * blockDim.x + threadIdx.x; pos < total_threads; pos += blockDim.x * gridDim.x) {
    ...
    if ((pos + 1) % step != 0) {
      ...
      switch (tail_num) {
        case 1:
          ApplyVec<OP, IN0, IN1, OUT, vec_num + 1>(func, scalar_option, in0 + in_pos.data[0], in1 + in_pos.data[1],
                                                   out + out_pos);
          break;
        case 2:
          ApplyVec<OP, IN0, IN1, OUT, vec_num + 2>(func, scalar_option, in0 + in_pos.data[0], in1 + in_pos.data[1],
                                                   out + out_pos);
          break;
        case 3:
          ApplyVec<OP, IN0, IN1, OUT, vec_num + 3>(func, scalar_option, in0 + in_pos.data[0], in1 + in_pos.data[1],
                                                   out + out_pos);
          break;
      }
    }
...
template <BinaryOpType OP, typename IN0, typename IN1, typename OUT>
cudaError_t BinaryWithBroadcast(BinaryFunc<OP, IN0, IN1, OUT> func, ScalarOption scalar_option, const size_t out_num,
                                const std::vector<int64_t> &in0_shape, const std::vector<int64_t> &in1_shape,
                                const std::vector<int64_t> &out_shape, IN0 *in0, IN1 *in1, OUT *out, size_t device_id,
                                cudaStream_t cuda_stream) {
  cudaDeviceProp prop;
  cudaGetDeviceProperties(&prop, device_id);
  size_t vec_thread_num = prop.multiProcessorCount * 8 * 32;
  const size_t dim_size = out_shape.size();
  constexpr size_t vec_num = VecSize<IN0, IN1, OUT>();
  ...
  if (out_num > vec_thread_num && vec_num > 1) {
    if (out_shape.back() == 2) {
      ...
      BinaryBroadcastVecWithoutTailCuda<OP, IN0, IN1, OUT, 2>
      ...
      size_t thread_num = total_threads > 1024 ? 1024 : total_threads;
      BinaryBroadcastVecWithoutTailCuda<OP, IN0, IN1, OUT, 3>
        <<<CUDA_BLOCKS_CAL(device_id, total_threads, thread_num), thread_num, 0, cuda_stream>>>(
          func, scalar_option, dim_size, total_threads, strides, in0, in1, out);
      CHECK_CUDA_LAUNCH_SUCCESS();
    } else {
      ...
      size_t thread_num = total_threads > 1024 ? 1024 : total_threads;
      if (tail_num == 0) {
        BinaryBroadcastVecWithoutTailCuda<OP, IN0, IN1, OUT, vec_num>
          <<<CUDA_BLOCKS_CAL(device_id, total_threads, thread_num), thread_num, 0, cuda_stream>>>(
            func, scalar_option, dim_size, total_threads, strides, in0, in1, out);
        CHECK_CUDA_LAUNCH_SUCCESS();
      } else {
        BinaryBroadcastVecWithTailCuda<OP, IN0, IN1, OUT, vec_num>
          <<<CUDA_BLOCKS_CAL(device_id, total_threads, thread_num), thread_num, 0, cuda_stream>>>(
    ...
    size_t thread_num = total_threads > 1024 ? 1024 : total_threads;
    BinaryWithBroadcastNoVecCuda<OP, IN0, IN1, OUT>
      <<<CUDA_BLOCKS_CAL(device_id, total_threads, thread_num), thread_num, 0, cuda_stream>>>(
...
template <BinaryOpType OP, typename IN0, typename IN1, typename OUT>
cudaError_t BinaryWithoutBroadcast(BinaryFunc<OP, IN0, IN1, OUT> func, ScalarOption scalar_option, size_t nums,
                                   OUT *out, IN0 *in0, IN1 *in1, size_t device_id, cudaStream_t cuda_stream) {
  size_t thread_num = nums > 1024 ? 1024 : nums;
  BinaryWithoutBroadcastNoVecCuda<OP, IN0, IN1, OUT>
...
cudaError_t BinaryOpWithBroadcastCudaFunc(const bool is_broadcast, const std::vector<int64_t> &in0_shape,
                                          ...
                                          IN0 *in0, IN1 *in1, OUT *out, size_t device_id, cudaStream_t cuda_stream) {
  BinaryFunc<OP, IN0, IN1, OUT> func;
  size_t out_num = 1;
  ...
  if (is_broadcast) {
    ...
    return BinaryWithBroadcast<OP, IN0, IN1, OUT>(func, scalar_option, out_num, in0_shape, in1_shape, out_shape, in0,
                                                  in1, out, device_id, cuda_stream);
  } else {
    ...
    return BinaryWithoutBroadcast<OP, IN0, IN1, OUT>(func, scalar_option, out_num, out, in0, in1, device_id,
                                                     cuda_stream);
  }
}
