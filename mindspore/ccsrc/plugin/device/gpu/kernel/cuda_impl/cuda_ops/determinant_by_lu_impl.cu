...
template <typename T>
__global__ void CalculateDeterminantByLuKernel(const T *lu_input, const int *pivot, int m, int batch_size, T *output) {
  for (size_t index = blockIdx.x * blockDim.x + threadIdx.x; index < (batch_size); index += blockDim.x * gridDim.x) {
    ...
    output[index] = prod_sign * exp(sum_abs_log_det);
  }
}

template <typename T>
CUDA_LIB_EXPORT void CalculateDeterminantByLu(const T *lu_input, const int *pivot, int m, int batch_size, T *output,
                                              cudaStream_t cuda_stream) {
  // Parallelization by batch_size.
  CalculateDeterminantByLuKernel<<<GET_BLOCKS(batch_size), GET_THREADS, 0, cuda_stream>>>(lu_input, pivot, m,
                                                                                          batch_size, output);
}

template CUDA_LIB_EXPORT void CalculateDeterminantByLu<float>(const float *lu_input, const int *pivot, int m,
                                                              int batch_size, float *output, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT void CalculateDeterminantByLu<double>(const double *lu_input, const int *pivot, int m,
                                                               int batch_size, double *output,
                                                               cudaStream_t cuda_stream);
