...
__global__ void Hsvtorgb(const size_t input_size, const T *input, T *output) {
  for (size_t idx = blockDim.x * blockIdx.x + threadIdx.x; idx < input_size / 3; idx += blockDim.x * gridDim.x) {
    T r, g, b;
...
__global__ void Hsvtorgb(const size_t input_size, const half *input, half *output) {
  for (size_t idx = blockDim.x * blockIdx.x + threadIdx.x; idx < input_size / 3; idx += blockDim.x * gridDim.x) {
    float r, g, b;
    hsv2rgb(static_cast<float>(input[idx * 3]), static_cast<float>(input[idx * 3 + 1]),
...
template <typename T>
cudaError_t CalHsvtorgb(const size_t input_size, const T *input, T *output, const uint32_t &device_id,
                        cudaStream_t cuda_stream) {
  Hsvtorgb<<<CUDA_BLOCKS(device_id, input_size), CUDA_THREADS(device_id), 0, cuda_stream>>>(input_size, input, output);
  return GetCudaStatus();
}

template CUDA_LIB_EXPORT cudaError_t CalHsvtorgb<half>(const size_t input_size, const half *input, half *output,
                                                       const uint32_t &device_id, cudaStream_t cuda_stream);
template CUDA_LIB_EXPORT cudaError_t CalHsvtorgb<float>(const size_t input_size, const float *input, float *output,
                                                        const uint32_t &device_id, cudaStream_t cuda_stream);
template CUDA_LIB_EXPORT cudaError_t CalHsvtorgb<double>(const size_t input_size, const double *input, double *output,
                                                         const uint32_t &device_id, cudaStream_t cuda_stream);
