...
__global__ void PDist_Grad_Lt_Two(const size_t y_size, const T *y_grad, const T *x, const T *y, T *buffer,
  ...
  if (dist_k != 0.0 && p >= 1) {
    ...
    for (; x_i < end; x_i += s, x_j += s, buff1 += s, buff2 += s) {
      ...
      T res = static_cast<T>(sign(diff) * pow(static_cast<double>(abs(diff)), static_cast<double>(p - 1)) * (grad_k) /
                             pow(static_cast<double>(dist_k), static_cast<double>(p - 1)));
      *buff1 = res;
      *buff2 = -res;
    }
  }
...
__global__ void PDist_Grad_P(const size_t y_size, const T *y_grad, const T *x, const T *y, T *buffer, const int64_t n,
  ...
  if (dist_k != 0.0) {
    ...
    for (; x_i < end; x_i += s, x_j += s, buff1 += s, buff2 += s) {
      const T diff = (*x_i - *x_j);
      T res = static_cast<T>(diff * pow(static_cast<double>(abs(diff)), static_cast<double>(p - 2)) * grad_k /
                             pow(static_cast<double>(dist_k), static_cast<double>(p - 1)));
      *buff1 = res;
      *buff2 = -res;
    }
