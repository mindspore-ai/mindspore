...
template <typename T>
CUDA_LIB_EXPORT cudaError_t LayerNormGrad(const int row_dim, const int col_dim, const int param_dim,
                                          const float epsilon, const T *dy, const T *x, const float *mean,
                                          const float *var, const T *gamma, T *dx, T *dg, T *db, cudaStream_t stream);
