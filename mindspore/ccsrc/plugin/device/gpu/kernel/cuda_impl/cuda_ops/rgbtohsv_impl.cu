...
__global__ void Rgbtohsv(const size_t input_size, const half *input, half *output) {
  for (size_t idx = blockDim.x * blockIdx.x + threadIdx.x; idx < input_size / 3; idx += blockDim.x * gridDim.x) {
    float h, s, v;
    rgb2hsv(static_cast<float>(input[idx * 3]), static_cast<float>(input[idx * 3 + 1]),
...
template <typename T>
cudaError_t CalRgbtohsv(const size_t input_size, const T *input, T *output, const uint32_t &device_id,
                        cudaStream_t cuda_stream) {
  Rgbtohsv<<<CUDA_BLOCKS(device_id, input_size), CUDA_THREADS(device_id), 0, cuda_stream>>>(input_size, input, output);
  return GetCudaStatus();
}

template CUDA_LIB_EXPORT cudaError_t CalRgbtohsv<half>(const size_t input_size, const half *input, half *output,
                                                       const uint32_t &device_id, cudaStream_t cuda_stream);
template CUDA_LIB_EXPORT cudaError_t CalRgbtohsv<float>(const size_t input_size, const float *input, float *output,
                                                        const uint32_t &device_id, cudaStream_t cuda_stream);
template CUDA_LIB_EXPORT cudaError_t CalRgbtohsv<double>(const size_t input_size, const double *input, double *output,
                                                         const uint32_t &device_id, cudaStream_t cuda_stream);
