...
CUDA_LIB_EXPORT void BernoulliForward(const T *input, S *output, float *rand_f, const size_t num_count,
                                      const uint32_t &device_id, cudaStream_t cuda_stream);

template <typename T, typename S>
CUDA_LIB_EXPORT void BroadcastBernoulliForward(const std::vector<size_t> &x_dims, const std::vector<size_t> &p_dims,
                                               const T *input, S *output, float *rand_f, const size_t num_count,
                                               const uint32_t &device_id, cudaStream_t cuda_stream);

template <typename T>
CUDA_LIB_EXPORT bool CheckRange(const T *input, const size_t num_count, const uint32_t &device_id,
