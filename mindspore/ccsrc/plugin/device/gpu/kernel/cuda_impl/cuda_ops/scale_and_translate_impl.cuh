...
template <typename T>
CUDA_LIB_EXPORT cudaError_t CalScaleAndTranslate(const size_t *thread_num, const T *image, const float *scale,
...
template <typename T>
CUDA_LIB_EXPORT cudaError_t CallScaleAndTranslateGrad(
  const std::string kernel_type, const T *grads, const T *original_image, const float radius,
