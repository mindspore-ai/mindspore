...
__device__ __host__ __forceinline__ OUT CalMid(const IN &inp_val) {
  OUT res(0.5 * log(inp_val * inp_val * 2), 1.0);
  return res;
}
template <>
...
struct BinaryFunc<BinaryOpType::kXlogy, IN0, IN1, Complex<OUT>> {
  __device__ __host__ __forceinline__ BinaryFunc() {}
  __device__ __host__ __forceinline__ Complex<OUT> operator()(const IN0 &lhs, const IN1 &rhs) const {
    ...
    Complex<OUT> mid = CalMid<IN1, Complex<OUT>>(rhs);
    return lhs * mid;
  }
};
...
template <typename IN0, typename IN1, typename OUT>
struct BinaryFunc<BinaryOpType::kSquaredDifference, IN0, IN1, Complex<OUT>> {
  __device__ __host__ __forceinline__ BinaryFunc() {}
  __device__ __host__ __forceinline__ Complex<OUT> operator()(const IN0 &lhs, const IN1 &rhs) const {
    Complex<OUT> diff = lhs - rhs;
    Complex<OUT> conj_diff(diff.real(), -diff.imag());
