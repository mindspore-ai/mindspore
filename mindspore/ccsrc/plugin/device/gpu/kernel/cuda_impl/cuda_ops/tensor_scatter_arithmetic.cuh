...
CUDA_LIB_EXPORT cudaError_t TensorScatterArithmetic(const enum TensorScatterArithmeticFunctionType &func_type,
                                                    const T *input, const S *indices, const T *update, T *output,
...
template <typename T, typename S>
CUDA_LIB_EXPORT cudaError_t CallTensorScatterUpdate(const T *input, const S *indices, const T *update, T *output,
                                                    const size_t &block_size, const size_t &input_size,
                                                    const size_t &output_size, const size_t &indices_dim_0,
                                                    const size_t &indices_dim_1, const TensorScatterInfo<S> &info,
                                                    uint32_t device_id, cudaStream_t stream);
