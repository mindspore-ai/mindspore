...
#include <functional>
#include <map>
#include "mindspore/core/ops/zeta.h"
...
namespace mindspore {
namespace kernel {
class ZetaGpuKernelMod : public NativeGpuKernelMod {
 public:
  ZetaGpuKernelMod() { ResetResource(); }
  ~ZetaGpuKernelMod() override = default;
 ...
 private:
  ...
  using ZetaFunc =
    std::function<bool(ZetaGpuKernelMod *, const std::vector<kernel::AddressPtr> &,
                       const std::vector<kernel::AddressPtr> &, const std::vector<kernel::AddressPtr> &)>;

 private:
  size_t unit_size_{1};
  size_t output_elements_;
  ZetaFunc kernel_func_{};
  BaseOperatorPtr kernel_ptr_{nullptr};
  bool is_null_input_{false};
  void *cuda_stream_{nullptr};
  static std::vector<std::pair<KernelAttr, ZetaFunc>> func_list_;
};
}  // namespace kernel
}  // namespace mindspore
