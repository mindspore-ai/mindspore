...
bool BroadcastOptGpuKernelMod::LaunchKernel(const std::vector<AddressPtr> &inputs,
  ...
  auto lhs = GetDeviceAddress<IN0>(inputs, kIndex0);
  auto rhs = GetDeviceAddress<IN1>(inputs, kIndex1);
  auto out = GetDeviceAddress<OUT>(outputs, kIndex0);
  auto status =
    BinaryOpWithBroadcastCudaFunc<op, IN0, IN1, OUT>(is_broadcast_, simplified_in0_shape_, simplified_in1_shape_,
                                                     simplified_out_shape_, lhs, rhs, out, device_id_, cuda_stream_);
...
#define MS_REG_BROADCAST_OP_DIFF_TYPE(OP_TYPE, IN0_NUM_TYPE, IN1_NUM_TYPE, OUT_NUM_TYPE, IN0_TYPE, IN1_TYPE, OUT_TYPE) \
  {                                                                                                                    \
    KernelAttr().AddInputAttr(IN0_NUM_TYPE).AddInputAttr(IN1_NUM_TYPE).AddOutputAttr(OUT_NUM_TYPE),                    \
      &BroadcastOptGpuKernelMod::LaunchKernel<OP_TYPE, IN0_TYPE, IN1_TYPE, OUT_TYPE>                                   \
  }
