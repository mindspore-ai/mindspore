...
class Conv3dGradFilterGpuKernelMod : public NativeGpuKernelMod {
 public:
  ...
  int Resize(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
    ...
    auto dy_shape = outputs[kIndex0]->GetShapeVector();
    ...
    auto filter_shape = GetValue<std::vector<int64_t>>(base_operator->GetAttr("kernel_size"));
    const size_t kFilterDimSize = 2;
    if (filter_shape.size() < kFilterDimSize) {
      MS_LOG(EXCEPTION) << "For '" << kernel_name_ << "', the dimension of filter must be greater than or equal to 2, "
