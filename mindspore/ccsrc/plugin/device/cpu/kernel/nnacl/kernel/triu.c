...
int TriuGetKValue(TriuStruct *triu) {
  if (triu->base_.in_size_ <= 1) {
    triu->k_ = 0;
    return NNACL_OK;
  }

  TensorC *k_tensor = triu->base_.in_[SECOND_INPUT];
  ...
  switch (k_tensor->data_type_) {
    ...
    case kNumberTypeInt32:
      triu->k_ = *((int *)k_tensor->data_);
      break;
    case kNumberTypeInt64:
      triu->k_ = *((int64_t *)k_tensor->data_);
      break;
    default:
