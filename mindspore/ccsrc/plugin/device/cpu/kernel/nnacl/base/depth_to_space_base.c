    ...
    for (int j = 0; j < in_shape_dim1; ++j) {
      ...
      for (int k = 0; k < in_shape_dim2; ++k) {
        ...
        for (int l = 0; l < in_shape_dim3; ++l) {
          int offset = l % (block_size * block_size);
          int out_offset_c =
            out_offset_w +
          ...
          int out_offset = (out_offset_c + l / (block_size * block_size)) * param->data_type_size_;
          int in_offset = (in_offset_w + l) * param->data_type_size_;
          memcpy((int8_t *)output + out_offset, (int8_t *)input + in_offset, copy_size);
        }
      }
    }
