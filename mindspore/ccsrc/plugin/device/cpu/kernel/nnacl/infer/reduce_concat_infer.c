...
#include "nnacl/infer/reduce_concat_infer.h"
#include "nnacl/infer/infer_register.h"
#include "nnacl/split_parameter.h"

int DataTypeJudge2(const TensorC *input, const TensorC *output) {
...
int ReduceConcatFusionInferShape(const TensorC *const *inputs, size_t inputs_size, TensorC **outputs,
  ...
  for (size_t i = 1; i < inputs_size; ++i) {
    ...
    int data_type_judge = DataTypeJudge2(inputs[i], output);
...
REG_INFER(ReduceConcatFusion, PrimType_Inner_ReduceConcatFusion, ReduceConcatFusionInferShape)
