...
void MaxPool3DGradWithArgmaxCpuKernelMod::InitKernel(const CNodePtr &kernel_node) {
  MS_EXCEPTION_IF_NULL(kernel_node);
  kernel_name_ = common::AnfAlgo::GetCNodeName(kernel_node);
  x_shape_ = AnfAlgo::GetInputDeviceShape(kernel_node, kZero);
  grads_shape_ = AnfAlgo::GetInputDeviceShape(kernel_node, kOne);
  y_shape_ = AnfAlgo::GetOutputDeviceShape(kernel_node, kZero);
  x_dtype_ = AnfAlgo::GetInputDeviceDataType(kernel_node, kZero);
  argmax_dtype_ = AnfAlgo::GetInputDeviceDataType(kernel_node, kTwo);
  ...
  if (pads_addr->isa<Int64Imm>()) {
    ...
    pads_list_.push_back(pads_int_);
  } else {
    pads_list_ = common::AnfAlgo::GetNodeAttr<std::vector<int64_t>>(kernel_node, "pads");
  }
  ...
  if (!is_match) {
    MS_LOG(EXCEPTION) << "MaxPool3DGradWithargmax does not support this kernel data type: " << kernel_attr;
  }
