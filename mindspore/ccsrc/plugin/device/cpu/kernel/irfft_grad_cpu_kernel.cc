...
#include "plugin/device/cpu/kernel/irfft_grad_cpu_kernel.h"
#include <algorithm>
...
#define SWITCH_DIM_CALCULATE(T1, T2)                                                                                \
  if (x_rank_ == 1) {                                                                                               \
    ComputeIRFFTGrad<T1, T2, 1>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else if (x_rank_ == 2) {                                                                                        \
    ComputeIRFFTGrad<T1, T2, 2>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else if (x_rank_ == 3) {                                                                                        \
    ComputeIRFFTGrad<T1, T2, 3>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else if (x_rank_ == 4) {                                                                                        \
    ComputeIRFFTGrad<T1, T2, 4>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else if (x_rank_ == 5) {                                                                                        \
    ComputeIRFFTGrad<T1, T2, 5>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else if (x_rank_ == 6) {                                                                                        \
    ComputeIRFFTGrad<T1, T2, 6>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else if (x_rank_ == 7) {                                                                                        \
    ComputeIRFFTGrad<T1, T2, 7>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  } else {                                                                                                          \
    ComputeIRFFTGrad<T1, T2, 8>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element_nums, \
                                tensor2_shape_);                                                                    \
  }
...
bool IRFFTGradCpuKernelMod::Init(const std::vector<KernelTensor *> &inputs,
...
int IRFFTGradCpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
                                  const std::vector<KernelTensor *> &outputs) {
...
double IRFFTGradGetnormalized(int64_t element_nums, mindspore::NormMode norm_type_) {
  double result = 1.0;
...
bool ComputeIRFFTGrad(T_in *input_ptr, T_out *output_ptr, mindspore::NormMode norm_type, int64_t n, int64_t dim,
...
bool IRFFTGradCpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
  ...
  IRFFTGradGenerateCalculateInput<T_in, T_mid>(input_ptr, calculate_input, input_element_nums_, tensor_shape_,
...
#define ONE_DIM_CPU_REG(MS_Tin1, MS_Tin2, MS_Tout, T_in, T_mid, T_out) \
  KernelAttr()                                                         \
    ...
    &IRFFTGradCpuKernelMod::LaunchKernel<T_in, T_mid, T_out>

std::vector<std::pair<KernelAttr, IRFFTGradCpuKernelMod::IRFFTGradFunc>> IRFFTGradCpuKernelMod::func_list_ = {
  {ONE_DIM_CPU_REG(kNumberTypeFloat32, kNumberTypeInt16, kNumberTypeComplex64, float, float, complex64)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat32, kNumberTypeInt32, kNumberTypeComplex64, float, float, complex64)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat32, kNumberTypeInt64, kNumberTypeComplex64, float, float, complex64)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat32, kNumberTypeFloat16, kNumberTypeComplex64, float, float, complex64)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat32, kNumberTypeFloat32, kNumberTypeComplex64, float, float, complex64)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat64, kNumberTypeFloat64, kNumberTypeComplex128, double, double, complex128)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat32, kNumberTypeComplex64, kNumberTypeComplex64, float, complex64, complex64)},
  {ONE_DIM_CPU_REG(kNumberTypeFloat64, kNumberTypeComplex128, kNumberTypeComplex128, double, complex128, complex128)}};

std::vector<KernelAttr> IRFFTGradCpuKernelMod::GetOpSupport() {
  std::vector<KernelAttr> support_list;
  (void)std::transform(func_list_.begin(), func_list_.end(), std::back_inserter(support_list),
                       [](const std::pair<KernelAttr, IRFFTGradFunc> &pair) { return pair.first; });
  return support_list;
}

MS_KERNEL_FACTORY_REG(NativeCpuKernelMod, IRFFTGrad, IRFFTGradCpuKernelMod);
}  // namespace kernel
}  // namespace mindspore
