...
bool BesselJ0CpuKernelMod::Init(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
  ...
  if (!kernel_ptr) {
    MS_LOG(ERROR) << "For 'BesselJ0CpuKernelMod', BaseOperatorPtr can not dynamic cast to BesselJ0 before initialize!";
    return false;
  }
...
int BesselJ0CpuKernelMod::Resize(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
                                 const std::vector<KernelTensorPtr> &outputs,
