...
class ExpandCpuKernelMod : public NativeCpuKernelMod {
 public:
  ...
  template <typename T>
  bool ExpandCompute(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs);

  template <size_t RANK, typename T>
  bool ExpandCalculate(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs);
