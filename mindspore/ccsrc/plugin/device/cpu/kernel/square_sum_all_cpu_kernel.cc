...
template <typename T>
void SquareSum(const T *in0, const T *in1, T *out0, T *out1, size_t start, size_t end) {
  for (size_t index = start; index < end; index++) {
    ...
    if (index < split) {
      auto ret = pow(in0[index], T(kPowerSquareExp));
      out0[0] = (index == 0) ? ret : out0[0] + ret;
    } else {
      auto ret = pow(in1[index - split], T(kPowerSquareExp));
...
bool SquareSumAllCpuKernelMod::LaunchKernel(const std::vector<kernel::AddressPtr> &inputs,
  ...
  auto task = std::bind(SquareSum<T>, input_0_addr, input_1_addr, output_0_addr, output_1_addr, std::placeholders::_1,
                        std::placeholders::_2);
