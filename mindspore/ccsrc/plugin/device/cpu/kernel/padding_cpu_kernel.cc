...
bool PaddingCpuKernelMod::Init(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
  ...
  if (kernel_name_ != prim::kPrimPadding->name()) {
    MS_LOG(ERROR) << "For 'Padding' kernel name get failed, but got " << kernel_name_;
    return false;
  }
  ...
  if (!is_match) {
    MS_LOG(ERROR) << "Padding does not support this kernel data type: " << kernel_attr;
    return false;
  }
...
bool PaddingCpuKernelMod::LaunchKernel(const std::vector<kernel::AddressPtr> &inputs,
  ...
  if (memset_errno != EOK) {
    MS_LOG(EXCEPTION) << "For '" << kernel_name_ << "', memset output to 0 failed. Error no: " << memset_errno;
  }
