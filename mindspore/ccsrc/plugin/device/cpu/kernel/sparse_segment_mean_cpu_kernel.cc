...
int SparseSegmentMeanCpuKernelMod::Resize(const BaseOperatorPtr &base_operator,
  ...
  dim0_size_ = LongToSize(x_shape.at(kIndex0));
  dimx_size_ = 1;
  for (size_t i = 1; i < x_shape.size(); i++) {
    dimx_size_ *= LongToSize(x_shape[i]);
  }
  indices_size_ = 1;
  for (auto s : indices_shape) {
    indices_size_ *= LongToSize(s);
  }
  return ret;
}
...
bool SparseSegmentMeanCpuKernelMod::LaunchKernel(const std::vector<kernel::AddressPtr> &inputs,
  ...
  for (size_t i = 0; i < indices_size_; i++) {
    if (indices_ptr[i] < 0 || indices_ptr[i] >= static_cast<IndexType>(dim0_size_)) {
      MS_LOG(ERROR) << "For '" << kernel_name_ << "', the " << i << "-th 'indices'[" << indices_ptr[i]
                    << "] is out of range[0, " << dim0_size_ << ").";
      return false;
    }
  }
  ...
  for (size_t i = 1; i < indices_size_; i++) {
  ...
  auto task = [this, &x_ptr, &indices_ptr, &segment_ids_ptr, &y_ptr](size_t start, size_t end) {
    ...
    for (size_t i = 0; i < indices_size_; ++i) {
      auto x_offset = indices_ptr[i] * dimx_size_;
      auto y_offset = segment_ids_ptr[i] * dimx_size_;
  ...
  ParallelLaunchAutoSearch(task, dimx_size_, this, &parallel_search_info_, pool_);
  return true;
}
