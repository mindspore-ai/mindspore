...
bool PadAndShiftCpuKernelMod::Launch(const std::vector<kernel::AddressPtr> &inputs,
  ...
  if (input_x_dtype_ == kNumberTypeInt32) {
    ...
    MS_LOG(EXCEPTION) << "For '" << kernel_name_ << "', the dtype of 'input_x' should be int32 or int64, but got "
                      << input_x_dtype_;
  }
  return true;
}
...
void PadAndShiftCpuKernelMod::LaunchKernel(const std::vector<AddressPtr> &inputs,
  ...
  if (shift_idx >= static_cast<T>(cum_sum_size_)) {
    MS_LOG(EXCEPTION) << "For '" << kernel_name_
                      << "', shift index should be less than cumsum size, but got shift index: " << shift_idx
