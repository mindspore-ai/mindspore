...
bool BesselY0CpuKernelMod::Init(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
  ...
  if (inputs.size() != kBesselY0InputsNum || outputs.size() != kBesselY0OutputsNum) {
    MS_LOG(ERROR) << kernel_name_ << ": input and output size should be " << kBesselY0InputsNum << " and "
...
bool BesselY0CpuKernelMod::Resize(const BaseOperatorPtr &base_operator, const std::vector<KernelTensorPtr> &inputs,
                                  const std::vector<KernelTensorPtr> &outputs,
