...
template <typename S>
void UniformCandidateSamplerCpuKernelMod::ExpectedCount(const int64_t counter, S *true_expected_count,
...
void UniformCandidateSamplerCpuKernelMod::CheckInputsAndOutputs(const std::vector<KernelTensorPtr> &inputs,
  ...
  if (std::any_of(input_shape.begin(), input_shape.end(), [](size_t i) { return i == 0; })) {
    is_null_input_ = true;
    MS_LOG(ERROR) << "For '" << kernel_name_ << "', the input 'true_classes' was empty.";
    return;
  }
