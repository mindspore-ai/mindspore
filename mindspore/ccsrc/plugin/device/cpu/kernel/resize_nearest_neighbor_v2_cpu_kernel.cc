...
void ResizeNearestNeighborV2CpuKernelMod::InitKernel(const CNodePtr &kernel_node) {
  ...
  x_shape_ = common::AnfAlgo::GetPrevNodeOutputInferShape(kernel_node, kIndex0);
...
bool ResizeNearestNeighborV2CpuKernelMod::LaunchKernel(const std::vector<kernel::AddressPtr> &inputs,
                                                       const std::vector<kernel::AddressPtr> &outputs) {
  const size_t batch_size = x_shape_[dim_idx_map_['N']];
  const size_t in_height = x_shape_[dim_idx_map_['H']];
  const size_t in_width = x_shape_[dim_idx_map_['W']];
  const size_t channels = x_shape_[dim_idx_map_['C']];
  ...
  for (size_t b = 0; b < batch_size; ++b) {
    for (size_t y = 0; y < out_height; ++y) {
      ...
      if (half_pixel_centers_) {
        in_y = std::max(static_cast<size_t>(0), in_y);
      }
      for (size_t x = 0; x < out_width; ++x) {
        size_t in_x =
          std::min((align_corners_) ? static_cast<size_t>(roundf(Scaler(x, width_scale, half_pixel_centers_)))
                   ...
                   in_width - 1);
        if (half_pixel_centers_) {
          in_x = std::max(static_cast<size_t>(0), in_x);
        }
        // data_format = NHWC
        if (dim_idx_map_['C'] == kIndex3) {
          std::copy_n(&x_4d(SizeToLong(b), SizeToLong(in_y), SizeToLong(in_x), 0), channels,
                      &y_4d(SizeToLong(b), SizeToLong(y), SizeToLong(x), 0));
        } else {
          // data_format = NCHW
          for (size_t c = 0; c < channels; ++c) {
            y_4d(SizeToLong(b), SizeToLong(c), SizeToLong(y), SizeToLong(x)) =
              x_4d(SizeToLong(b), SizeToLong(c), SizeToLong(in_y), SizeToLong(in_x));
          }
        }
      }
    }
