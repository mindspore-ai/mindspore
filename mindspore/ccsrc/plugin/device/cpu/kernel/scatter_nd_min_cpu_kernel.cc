...
#include "plugin/device/cpu/kernel/scatter_nd_min_cpu_kernel.h"
#include <algorithm>
#include <utility>
...
namespace kernel {
namespace {
constexpr size_t kScatterNdMinInputsNum = 3;
constexpr size_t kScatterNdMinOutputsNum = 1;
constexpr size_t kNumUnits = 2;
constexpr size_t kInputIndices = 2;
constexpr char kKernelName[] = "ScatterNdMin";
...
void ScatterMinCPUKernelMod::InitKernel(const CNodePtr &kernel_node) {
...
bool ScatterMinCPUKernelMod::Launch(const std::vector<kernel::AddressPtr> &inputs,
                                    const std::vector<kernel::AddressPtr> &,
                                    const std::vector<kernel::AddressPtr> &outputs) {
  CHECK_KERNEL_INPUTS_NUM(inputs.size(), kScatterNdMinInputsNum, kernel_name_);
  CHECK_KERNEL_OUTPUTS_NUM(outputs.size(), kScatterNdMinOutputsNum, kernel_name_);
  switch (dtype_) {
...
template <typename T>
bool ScatterMinCPUKernelMod::DoComputeWithIndicesType(const std::vector<kernel::AddressPtr> &inputs,
                                                      const std::vector<kernel::AddressPtr> &outputs,
...
template <typename T, typename S>
bool ScatterMinCPUKernelMod::LaunchKernel(const std::vector<AddressPtr> &inputs,
                                          const std::vector<kernel::AddressPtr> &outputs) {
  T *x = reinterpret_cast<T *>(ScatterMinRealData(inputs, outputs));
...
void *ScatterNdMinCpuKernelMod::ScatterMinRealData(const std::vector<AddressPtr> &inputs,
                                                   const std::vector<kernel::AddressPtr> &) {
  return inputs[0]->addr;
}

MS_KERNEL_FACTORY_REG(NativeCpuKernelMod, ScatterNdMin, ScatterNdMinCpuKernelMod);
}  // namespace kernel
}  // namespace mindspore
