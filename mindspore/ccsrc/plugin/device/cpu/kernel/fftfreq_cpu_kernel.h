...
class FFTFreqCpuKernelMod : public NativeCpuKernelMod {
 public:
  ...
  ~FFTFreqCpuKernelMod() override = default;
 ...
 private:
  ...
  using FFTFreqFunc = std::function<bool(FFTFreqCpuKernelMod *, const std::vector<KernelTensor *> &,
                                         const std::vector<KernelTensor *> &)>;
  static std::vector<std::pair<KernelAttr, FFTFreqFunc>> func_list_;

  FFTFreqFunc kernel_func_;
