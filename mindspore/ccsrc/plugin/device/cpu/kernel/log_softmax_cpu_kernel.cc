...
#include "plugin/device/cpu/kernel/log_softmax_cpu_kernel.h"
#include <algorithm>
#include <memory>
#include "plugin/device/cpu/hal/device/cpu_device_address.h"
#include "plugin/device/cpu/kernel/nnacl/fp32/log_softmax_fp32.h"

namespace mindspore {
namespace kernel {
namespace {
#define LOGSOFTMAX_CPU_REG(M_T, T)                                                                       \
  KernelAttr().AddInputAttr(M_T).AddInputAttr(kObjectTypeTuple, kNumberTypeInt64).AddOutputAttr(M_T), \
    &LogSoftmaxCpuKernelMod::LaunchKernel<T>
...
void LogSoftmaxFunc(const T *input_ptr, T *output_ptr, acc_T *sum_data, int start, int end, int dim_axis, int inner_size) {
  for (int i = start; i < end; i++) {
    ...
    for (int k = 0; k < inner_size; k++) {
      ...
      for (int j = 0; j < dim_axis; j++) {
        ...
        sum_data[k + sum_outter_offset] += static_cast<acc_T>(expf(output_ptr[axis_offset]));
      }
    }
    ...
    for (int j = 0; j < dim_axis; j++) {
      ...
      for (int k = 0; k < inner_size; k++) {
        ...
        output_ptr[inner_offset] = output_ptr[inner_offset] - static_cast<T>(logf(sum_data[k + sum_outter_offset]));
      }
    }
  }
}
...
bool LogSoftmaxCpuKernelMod::CheckAndRectifyAxis(KernelTensor *axis_kernel_tensor) noexcept {
  auto axis_ = axis_kernel_tensor->GetValueWithCheck<std::vector<int64_t>>();
...
template <typename T>
void LogSoftmaxCpuKernelMod::LaunchKernel(const std::vector<KernelTensor *> &inputs,
  ...
  if constexpr (std::is_same_v<T, float>) {
    if (last_axis_) {
      auto task = [this, input_data, output_data](size_t start, size_t end) {
        ...
        (void)LogSoftmaxLastAxis(input_data + offset, output_data + offset, sum_data + offset, batch, dim_axis_);
      };
  ...
  auto task = [this, input_data, output_data, sum_data](int start, int end) {
    LogSoftmaxFunc<T, acc_T>(input_data, output_data, sum_data, start, end, static_cast<int>(dim_axis_),
...
std::vector<std::pair<KernelAttr, LogSoftmaxCpuKernelMod::LaunchFunc>> LogSoftmaxCpuKernelMod::func_list_ = {
