...
namespace mindspore {
namespace kernel {
std::vector<std::pair<KernelAttr, MapTensorGetDataCpuKernelMod::MapTensorGetDataLaunchFunc>>
  MapTensorGetDataCpuKernelMod::map_tensor_get_data_func_list_ = {
    {KernelAttr()
       .AddInputAttr(kObjectTypeMapTensorType)
       .AddOutputAttr(kNumberTypeInt32)
       .AddOutputAttr(kNumberTypeFloat32),
     &MapTensorGetDataCpuKernelMod::LaunchKernel<int32_t, float>},
    {KernelAttr()
       .AddInputAttr(kObjectTypeMapTensorType)
       .AddOutputAttr(kNumberTypeInt64)
       .AddOutputAttr(kNumberTypeFloat32),
     &MapTensorGetDataCpuKernelMod::LaunchKernel<int64_t, float>}};

std::vector<KernelAttr> MapTensorGetDataCpuKernelMod::GetOpSupport() {
  ...
  (void)std::transform(map_tensor_get_data_func_list_.begin(), map_tensor_get_data_func_list_.end(),
                       ...
                       [](const std::pair<KernelAttr, MapTensorGetDataCpuKernelMod::MapTensorGetDataLaunchFunc> &pair) {
                         return pair.first;
                       });
  return support_list;
}
...
MS_KERNEL_FACTORY_REG(NativeCpuKernelMod, MapTensorGetData, MapTensorGetDataCpuKernelMod);
}  // namespace kernel
}  // namespace mindspore
