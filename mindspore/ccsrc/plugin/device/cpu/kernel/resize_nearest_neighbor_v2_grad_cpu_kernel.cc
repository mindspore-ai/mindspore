...
void ResizeNearestNeighborV2GradCpuKernelMod::InitKernel(const CNodePtr &kernel_node) {
  ...
  grads_shape_ = common::AnfAlgo::GetPrevNodeOutputInferShape(kernel_node, kIndex0);
...
bool ResizeNearestNeighborV2GradCpuKernelMod::LaunchKernel(const std::vector<kernel::AddressPtr> &inputs,
  ...
  const size_t batch_size = grads_shape_[dim_idx_map_['N']];
  const size_t in_height = grads_shape_[dim_idx_map_['H']];
  const size_t in_width = grads_shape_[dim_idx_map_['W']];
  const size_t channels = grads_shape_[dim_idx_map_['C']];
  const size_t out_height = y_shape_[dim_idx_map_['H']];
  const size_t out_width = y_shape_[dim_idx_map_['W']];
  ...
  for (size_t y = 0; y < in_height; ++y) {
    size_t out_y =
      std::min((align_corners_) ? static_cast<size_t>(roundf(Scaler(y, height_scale, half_pixel_centers_)))
                                : static_cast<size_t>(floorf(Scaler(y, height_scale, half_pixel_centers_))),
               out_height - 1);
    for (size_t x = 0; x < in_width; ++x) {
      size_t out_x =
        std::min((align_corners_) ? static_cast<size_t>(roundf(Scaler(x, width_scale, half_pixel_centers_)))
                                  : static_cast<size_t>(floorf(Scaler(x, width_scale, half_pixel_centers_))),
                 out_width - 1);
      for (size_t b = 0; b < batch_size; ++b) {
        for (size_t c = 0; c < channels; ++c) {
          // data_format = NHWC
          if (dim_idx_map_['C'] == kIndex3) {
            y_4d(SizeToLong(b), SizeToLong(out_y), SizeToLong(out_x), SizeToLong(c)) +=
              grads_4d(SizeToLong(b), SizeToLong(y), SizeToLong(x), SizeToLong(c));
          } else {
            // data_format = NCHW
            y_4d(SizeToLong(b), SizeToLong(c), SizeToLong(out_y), SizeToLong(out_x)) +=
              grads_4d(SizeToLong(b), SizeToLong(c), SizeToLong(y), SizeToLong(x));
          }
        }
      }
    }
