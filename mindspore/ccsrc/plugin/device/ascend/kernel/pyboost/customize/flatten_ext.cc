...
namespace pyboost {
namespace {
ValueTuplePtr ShapeCalc(const PrimitivePtr &primitive, const BaseTensorPtr &input_x, const int64_t &start_dim,
  ...
  MS_CHECK_VALUE(-dim_size <= start_dim && start_dim < dim_size,
                 CheckAndConvertUtils::FormatCheckInRangeMsg("start_dim", start_dim, kIncludeLeft,
                                                             {-dim_size, dim_size}, primitive));
  MS_CHECK_VALUE(
    -dim_size <= end_dim && end_dim < dim_size,
    CheckAndConvertUtils::FormatCheckInRangeMsg("end_dim", end_dim, kIncludeLeft, {-dim_size, dim_size}, primitive));
...
void FlattenExtAscendCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &input_x_tensor,
  ...
  auto new_shape = ShapeCalc(op->primitive(), input_x_tensor, start_dim_value, end_dim_value);
  ...
  op->set_input_abs({AbstractConvertFunc::ConvertAbstract(input_x_tensor),
                     AbstractConvertFunc::ConvertAbstract(start_dim), AbstractConvertFunc::ConvertAbstract(end_dim)});
