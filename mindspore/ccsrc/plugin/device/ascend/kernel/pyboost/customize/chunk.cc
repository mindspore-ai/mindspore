...
std::vector<tensor::BaseTensorPtr> ChunkAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  if (split_size == 0 && dim_size == 0) {
    ...
    op->set_input_abs({input_tensor->ToAbstract(), split_sizes->ToAbstract(), dim_ptr->ToAbstract()});
  ...
  op->set_input_abs({input_tensor->ToAbstract(), split_size_ptr->ToAbstract(), dim_ptr->ToAbstract()});
