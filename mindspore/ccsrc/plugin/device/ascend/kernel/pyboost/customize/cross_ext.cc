...
#include "plugin/device/ascend/kernel/pyboost/customize/cross_ext.h"
...
namespace pyboost {
namespace {
int64_t UpdateCrossDim(const ShapeVector &input_shape, const std::optional<Int64ImmPtr> &dim_opt) {
  ...
  return GetValue<int64_t>(dim_opt.value());
}
}  // namespace
...
tensor::BaseTensorPtr CrossExtAscendCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &input_tensor,
  ...
  auto dim_imm = UpdateCrossDim(shape, dim);
