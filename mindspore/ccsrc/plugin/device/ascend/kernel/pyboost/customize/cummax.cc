 ...
 */
#include "plugin/device/ascend/kernel/pyboost/customize/cummax.h"
...
std::tuple<tensor::BaseTensorPtr, tensor::BaseTensorPtr> CumMaxAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, axis_imm]() {
    MS_LOG(DEBUG) << "Run device task cummax end";
    ...
    LAUNCH_ACLNN(aclnnCummax, device_context, op->stream_id(), input_tensor, axis_imm, outputs[0], outputs[1]);
