...
#include "plugin/device/ascend/kernel/pyboost/customize/binary_cross_entropy_ext.h"
#include <memory>
...
tensor::BaseTensorPtr BinaryCrossEntropyExtAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  MS_LOG(DEBUG) << "BinaryCrossEntropyExt call start";
  ...
  if (iter == reduction_map.end()) {
    MS_LOG(EXCEPTION) << "For BinaryCrossEntropyExt, the value of reduction is invalid.";
  }
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, target_tensor, weight_tensor, reduction_value]() {
      MS_LOG(DEBUG) << "Run device task BinaryCrossEntropyExt start";
      ...
      MS_LOG(DEBUG) << "Run device task BinaryCrossEntropyExt end";
    }));
  return op->output(0);
}
}  // namespace pyboost
