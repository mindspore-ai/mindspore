...
#include "plugin/device/ascend/kernel/pyboost/customize/elu.h"
#include <memory>
...
namespace kernel {
namespace pyboost {
tensor::BaseTensorPtr EluAscendCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &input,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input, alpha]() {
    MS_LOG(DEBUG) << "Run device task Elu start";
    ...
    MS_LOG(DEBUG) << "Run device task Elu end";
  }));
  return op->output(0);
}
}  // namespace pyboost
