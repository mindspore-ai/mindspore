...
std::tuple<tensor::BaseTensorPtr, tensor::BaseTensorPtr, tensor::BaseTensorPtr> AdamWeightDecayExtAscendCustomize(
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, var, m, v, max_v, grad, step, lr_imm, beta1_imm, beta2_imm,
      ...
      LAUNCH_ACLNN(aclnnInplaceCopy, device_context, op->stream_id(), op->output(kIndex0), var);
      LAUNCH_ACLNN(aclnnInplaceCopy, device_context, op->stream_id(), op->output(kIndex1), m);
      LAUNCH_ACLNN(aclnnInplaceCopy, device_context, op->stream_id(), op->output(kIndex2), v);
