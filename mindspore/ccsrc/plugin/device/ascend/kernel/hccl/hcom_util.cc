...
bool HcomUtil::GetHcomOperationType(const PrimitivePtr &primitive, HcclReduceOp *op_type,
  ...
  if (hcom_op_type == "min") {
    ...
    *collective_reduce_tyep = device::CollectiveOpReduceType::Reduce_Min;
  } else if (hcom_op_type == "max") {
    *op_type = HCCL_REDUCE_MAX;
    *collective_reduce_tyep = device::CollectiveOpReduceType::Reduce_Max;
  } else if (hcom_op_type == "prod") {
    *op_type = HCCL_REDUCE_PROD;
    *collective_reduce_tyep = device::CollectiveOpReduceType::Reduce_Prod;
  } else if (hcom_op_type == "sum") {
    *op_type = HCCL_REDUCE_SUM;
    *collective_reduce_tyep = device::CollectiveOpReduceType::Reduce_Sum;
  } else {
