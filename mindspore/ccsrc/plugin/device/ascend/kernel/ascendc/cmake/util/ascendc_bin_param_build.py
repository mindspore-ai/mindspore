...
class BinParamBuilder(opdesc_parser.OpDesc):
    ...
    def set_opc(self: any, opc_path: str):
        self.opc = opc_path
    ...
    def _write_buld_cmd(self: any, param_file: str, bin_file: str, index: int):
        ...
        with os.fdopen(os.open(compile_file, const_var.WFLAGS, const_var.WMODES), 'w') as fd:
            ...
            cmd = const_var.BIN_CMD.format(opc=self.opc, fun=self.op_intf, soc=hard_soc, param=param_file, impl='""')
            fd.write(cmd)
...
def gen_bin_param_file(cfgfile: str, out_dir: str, soc: str, opc_path: str):
...
if __name__ == '__main__':
    ...
    gen_bin_param_file(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
