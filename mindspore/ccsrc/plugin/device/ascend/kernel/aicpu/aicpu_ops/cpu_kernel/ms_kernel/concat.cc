...
uint32_t ConcatCpuKernel::CheckAndInitParams(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (n_ >= 2), KERNEL_STATUS_PARAM_INVALID, "Attr N must >= 2, but got attr N[%lld]", n_);

  uint32_t input_num = ctx.GetInputsSize();

  // input_num is n_(concat tensor num) + 1(concat_dim)
  CUST_KERNEL_CHECK_FALSE(ctx, (static_cast<int64_t>(input_num) - 1 == n_), KERNEL_STATUS_PARAM_INVALID,
                          "Input num must equal attr N[%lld + 1],"
                          "but got input num[%u]",
                          n_, input_num);

  Tensor *concat_dim_ptr = ctx.Input(0);
  CUST_KERNEL_CHECK_NULLPTR(ctx, concat_dim_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input concat_dim failed.");
  auto concat_dim_shape_ptr = concat_dim_ptr->GetTensorShape();
  CUST_KERNEL_CHECK_NULLPTR(ctx, concat_dim_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input concat_dim shape failed.");
  int32_t concat_dim_dims = concat_dim_shape_ptr->GetDims();
  CUST_KERNEL_CHECK_FALSE(
    ctx, (concat_dim_dims == 0) || ((concat_dim_dims == 1) && (concat_dim_shape_ptr->NumElements() == 1)),
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (concat_dim_data_type == DT_INT32 || concat_dim_data_type == DT_INT64),
  ...
  CUST_KERNEL_CHECK_NULLPTR(ctx, concat_dim_data_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input concat_dim data failed.");
  ...
  for (int64_t i = 1; i < n_; i++) {
    ...
    CUST_KERNEL_CHECK_NULLPTR(ctx, inputi_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input xi failed.");
    auto inputi_shape_ptr = inputi_ptr->GetTensorShape();
    auto inputi_type_ptr = inputi_ptr->GetDataType();
    CUST_KERNEL_CHECK_NULLPTR(ctx, inputi_shape_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input xi shape failed.");
    CUST_KERNEL_CHECK_FALSE(ctx, (input0_type_ptr == inputi_type_ptr), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (0 <= axis_ && axis_ < input_dims_), KERNEL_STATUS_PARAM_INVALID,
...
uint32_t ConcatCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (data_type_) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "unsupported datatype[%d]", DTypeStr(data_type_).c_str());
...
uint32_t ConcatCpuKernel::PrepareInput(CpuKernelContext &ctx,
  ...
  for (uint32_t i = 0; i < n_; ++i) {
    ...
    CUST_KERNEL_CHECK_FALSE(ctx, (input_i_dims == input_dims_), KERNEL_STATUS_PARAM_INVALID,
    ...
    for (int32_t j = 0; j < input_dims_; ++j) {
      ...
      CUST_KERNEL_CHECK_FALSE(ctx, (dim_0j == dim_ij), KERNEL_STATUS_PARAM_INVALID,
    ...
    CUST_KERNEL_CHECK_NULLPTR(ctx, input_i_data_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input x%u data failed.", i);
    ...
    CUST_KERNEL_CHECK_NULLPTR(ctx, input_i, KERNEL_STATUS_PARAM_INVALID, "Create input x%u failed!", i);
...
uint32_t ConcatCpuKernel::PrepareOutput(CpuKernelContext &ctx, std::shared_ptr<typename TTypes<T>::Matrix> &output) {
  ...
  CUST_KERNEL_CHECK_NULLPTR(ctx, output_ptr, KERNEL_STATUS_PARAM_INVALID, "Get output failed.");
  auto output_data_ptr = output_ptr->GetData();
  CUST_KERNEL_CHECK_NULLPTR(ctx, output_data_ptr, KERNEL_STATUS_PARAM_INVALID, "Get output data failed.");
  ...
  output = std::make_shared<typename TTypes<T>::Matrix>(reinterpret_cast<T *>(output_data_ptr), inputs_flat_dim0_,
                                                        output_dim1);
  CUST_KERNEL_CHECK_NULLPTR(ctx, output, KERNEL_STATUS_PARAM_INVALID, "Create output matrix failed.");
  return KERNEL_STATUS_OK;
}
...
uint32_t ConcatCpuKernel::DoCompute(CpuKernelContext &ctx) {
  std::vector<std::shared_ptr<typename TTypes<T>::ConstMatrix>> inputs;
  CUST_KERNEL_CHECK_FALSE(ctx, (PrepareInput<T>(ctx, inputs) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
                          "PrepareInput failed.");
  std::shared_ptr<typename TTypes<T>::Matrix> output = nullptr;
  CUST_KERNEL_CHECK_FALSE(ctx, (PrepareOutput<T>(ctx, output) == KERNEL_STATUS_OK), KERNEL_STATUS_PARAM_INVALID,
...
uint32_t ConcatCpuKernel::ConcatCompute(CpuKernelContext &ctx,
  ...
  auto work = [&row_size, &sizes, &inputs, &output, &num_inputs, &ret, &ctx](int64_t start, int64_t end) {
    ...
    if (out < out_start) {
      for (size_t j = 0; j < num_inputs; ++j) {
        ...
        if (ret != EOK) {
          CUST_KERNEL_LOG_ERROR(ctx, "Memcpy failed.");
    ...
    for (int64_t i = skipped_rows; i < dim0; ++i) {
      for (int64_t j = 0; j < static_cast<int64_t>(num_inputs); ++j) {
        ...
        if (ret != EOK) {
          CUST_KERNEL_LOG_ERROR(ctx, "Memcpy size[%zu] from inp[%llx] to out[%llx] failed.", copy_size, inp[j], out);
          ret = KERNEL_STATUS_INNER_ERROR;
          return;
        }
        out += size;
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (ret == KERNEL_STATUS_OK), KERNEL_STATUS_INNER_ERROR, "ConcatCpuKernel failed.");
  return KERNEL_STATUS_OK;
}
