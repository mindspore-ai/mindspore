...
#include "plugin/device/ascend/kernel/aicpu/aicpu_ops/flatten_kernels.h"
#include <memory.h>
#include <map>
#include <thread>
...
namespace aicpu {
namespace dataset {
uint32_t FlattenKernel::DoCompute() {
...
uint32_t FlattenKernel::ParseKernelParam() {
  AICPU_LOGI("aicpu FlattenKernel");
...
extern "C" {
__attribute__((visibility("default"))) uint32_t Flatten(void *param) {
  aicpu::dataset::FlattenKernel flattenKernel;
  return flattenKernel.Compute(param);
}
}
