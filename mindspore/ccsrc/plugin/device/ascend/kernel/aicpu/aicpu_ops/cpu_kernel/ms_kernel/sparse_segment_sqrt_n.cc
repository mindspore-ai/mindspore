...
uint32_t SparseSegmentSqrtNCpuKernel::Compute(CpuKernelContext &ctx) {
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum), "SparseSegmentSqrtN normalcheck failed.");
  ...
  if (x_data_type != DT_FLOAT && x_data_type != DT_DOUBLE && x_data_type != DT_FLOAT16) {
    CUST_KERNEL_LOG_ERROR(ctx, "SparseSegmentSqrtN kernel data type [%s] not support.", DTypeStr(x_data_type).c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  if ((indices_data_type != DT_INT32 && indices_data_type != DT_INT64) ||
      (segment_ids_data_type != DT_INT32 && segment_ids_data_type != DT_INT64)) {
    CUST_KERNEL_LOG_ERROR(ctx, "SparseSegmentSqrtN kernel data type [%s] not support.",
...
uint32_t CheckParamValidation(CpuKernelContext &ctx) {
  ...
  if (segment_ids_addr[0] != 0) {
    CUST_KERNEL_LOG_ERROR(ctx, "segment_ids can't miss ids.");
    return KERNEL_STATUS_PARAM_INVALID;
  }
...
uint32_t SparseSegmentSqrtNCpuKernel::ComputeKernel(CpuKernelContext &ctx) {
