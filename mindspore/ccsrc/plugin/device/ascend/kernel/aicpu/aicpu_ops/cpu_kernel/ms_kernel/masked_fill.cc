...
template <class T1>
uint32_t MaskedFillCompare(DataType value_type, double raw_value) {
  if (raw_value > static_cast<double>(std::numeric_limits<T1>::max()) ||
      raw_value < static_cast<double>(std::numeric_limits<T1>::lowest())) {
...
uint32_t GetValueFromVariousTypesMaskedFill(const CpuKernelContext &ctx, T &value) {
  ...
  if (value_type != DT_COMPLEX64 && value_type != DT_COMPLEX128) {
    value = static_cast<T>(raw_value);
  } else {
    value = static_cast<T>(raw_value_complex);
  }
  return KERNEL_STATUS_OK;
}
template <typename T>
