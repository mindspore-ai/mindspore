...
#define LIST_DIFF_COMPUTE_CASE(DTYPE, TYPE, OUT_IDX, CTX)        \
  case (DTYPE): {                                                \
    ...
    if (result != KERNEL_STATUS_OK) {                            \
      CUST_KERNEL_LOG_ERROR(ctx, "Less kernel compute failed."); \
      return result;                                             \
    }                                                            \
    break;                                                       \
  }
...
uint32_t ListDiffCpuKernel::ParamCheck(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kListDiffInputNum, kListDiffOutputNum), "[%s] check params failed.",
                           kListDiff);
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, IsVector(x->GetTensorShape()->GetDimSizes()), KERNEL_STATUS_PARAM_INVALID,
                          "Input Tensor x should be a 1D vector.");
  CUST_KERNEL_CHECK_FALSE(ctx, IsVector(y->GetTensorShape()->GetDimSizes()), KERNEL_STATUS_PARAM_INVALID,
  ...
  if (out_idx_att) {
    ...
    CUST_KERNEL_CHECK_FALSE(ctx, (out_idx == DT_INT32 || out_idx == DT_INT64), KERNEL_STATUS_PARAM_INVALID,
                            "attr value 'out_idx_att' should in (DT_INT32, DT_INT64)");
  }
  // datype check for x, y, out, idx
  CUST_KERNEL_CHECK_FALSE(ctx, x->GetDataType() == y->GetDataType() && y->GetDataType() == out->GetDataType(),
                          KERNEL_STATUS_PARAM_INVALID, "The DataType of input x and y should be same");
  CUST_KERNEL_CHECK_FALSE(ctx, idx->GetDataType() == out_idx, KERNEL_STATUS_PARAM_INVALID,
...
uint32_t ListDiffCpuKernel::DoCompute(CpuKernelContext &ctx) {
  ...
  uint32_t ret = CpuKernelAllocatorUtils::AllocateOutputTensorDataMemory(ctx, {out_size}, out_type, out);
  CUST_KERNEL_CHECK_FALSE(ctx, (ret == KERNEL_STATUS_OK), KERNEL_STATUS_INNER_ERROR,
                          "Allocate memory for out tensor failed.");
  ret = CpuKernelAllocatorUtils::AllocateOutputTensorDataMemory(ctx, {out_size}, out_idx, idx);
  CUST_KERNEL_CHECK_FALSE(ctx, (ret == KERNEL_STATUS_OK), KERNEL_STATUS_INNER_ERROR,
                          "Allocate memory for idx tensor failed.")
  // construct EigenTensor
  ...
  for (Tidx i = 0, p = 0; i < static_cast<Tidx>(x_size); ++i) {
    if (0 == y_set.count(x_vec(i))) {
      CUST_KERNEL_CHECK_FALSE(ctx, p < static_cast<Tidx>(out_size), KERNEL_STATUS_INNER_ERROR,
...
uint32_t ListDiffCpuKernel::Compute(CpuKernelContext &ctx) {
  CUST_KERNEL_HANDLE_ERROR(ctx, ParamCheck(ctx), "ListDiffCpuKernel check params failed");
  ...
  switch (data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "ListDiff kernel data type [%s] not support.", DTypeStr(data_type).c_str());
