...
uint32_t SetSizeCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum),
                           "SetSize check input and output number failed.");
  // check dim
  CUST_KERNEL_CHECK_FALSE(ctx, (set_indices_->GetTensorShape()->GetDims() == input_index), KERNEL_STATUS_PARAM_INVALID,
                          "Indices tensor dim size equal to 2, got size [%d].",
                          set_indices_->GetTensorShape()->GetDims())
  CUST_KERNEL_CHECK_FALSE(ctx, (set_values_->GetTensorShape()->GetDims() == 1), KERNEL_STATUS_PARAM_INVALID,
                          "Values tensor dim size equal to 1, got size [%d].", set_values_->GetTensorShape()->GetDims())
  CUST_KERNEL_CHECK_FALSE(ctx, (set_shape_->GetTensorShape()->GetDims() == 1), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (data_type_0 == DT_INT64), KERNEL_STATUS_PARAM_INVALID,
                          "The data type of input0 requested dtype int64 for Tensor "
                          "with dtype [%s]",
                          DTypeStr(data_type_0).c_str())
  CUST_KERNEL_CHECK_FALSE(ctx, (data_type_2 == DT_INT64), KERNEL_STATUS_PARAM_INVALID,
...
uint32_t SetSizeCpuKernel::SparseTensorFromContext(CpuKernelContext &ctx, const bool validate_indices,
  ...
  uint32_t result = st.CreateSparseTensor(ctx, set_indices_, set_values_, dense_shape, order);
...
uint32_t SetSizeCpuKernel::IndicesValid(CpuKernelContext &ctx, SparseTensor &st) {
  ...
  if (dim_size >= kParallelDataNumSameShape) {
    ...
    CUST_KERNEL_HANDLE_ERROR(ctx, CpuKernelUtils::ParallelFor(ctx, dim_size, dim_size / max_core_num, invalid_setsize),
...
uint32_t SetSizeCpuKernel::CheckGroup(CpuKernelContext &ctx, const Group &group,
  ...
  for (int32_t j = 0; j < dims_; ++j) {
    ...
    CUST_KERNEL_CHECK_FALSE(ctx, (dim_size > 0), KERNEL_STATUS_PARAM_INVALID, "Invalid dim_size [%d] = [%d].", j,
                            dim_size)
    for (int64_t i = 0; i < num_values; ++i) {
      const auto index = *(indices_t + (i * dims_) + j);
      CUST_KERNEL_CHECK_FALSE(ctx, (dim_size > 0), KERNEL_STATUS_PARAM_INVALID,
                              "indices[%d, %d] expected < %d, got %d.", i, j, dim_size, index)
    }
  }
  return KERNEL_STATUS_OK;
}
...
uint32_t SetSizeCpuKernel::PopulateFromSparseGroup(CpuKernelContext &ctx, const Group &group,
  ...
  if (dim_size >= kParallelDataNumSameShape) {
    ...
    CUST_KERNEL_HANDLE_ERROR(ctx,
                             CpuKernelUtils::ParallelFor(ctx, dim_size, dim_size / max_core_num, group_value_setsize),
                             "SetSize Compute failed.");
  } else {
...
uint32_t SetSizeCpuKernel::SetSizeCompute(CpuKernelContext &ctx, SparseTensor &st) {
  ...
  if (ret != EOK) {
    CUST_KERNEL_LOG_ERROR(ctx, "For 'SetSize', memset_s failed, ret=%d.", ret);
    return KERNEL_STATUS_INNER_ERROR;
  }
  std::unordered_set<T> group_set;
  for (const auto &group : st.group(ctx, group_ix)) {
