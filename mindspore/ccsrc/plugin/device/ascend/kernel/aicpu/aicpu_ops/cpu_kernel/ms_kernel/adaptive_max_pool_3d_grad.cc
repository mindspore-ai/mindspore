...
#define CREATE_COMPUTE_CASE(DTYPE1, TYPE1, DTYPE2, TYPE2, CTX)                    \
  case (DTYPE1 * kDtypeNum + DTYPE2): {                                           \
    ...
    if (result != KERNEL_STATUS_OK) {                                             \
      CUST_KERNEL_LOG_ERROR(ctx, "AdaptiveMaxPool3dGrad kernel compute failed."); \
      return result;                                                              \
    }                                                                             \
    break;                                                                        \
  }
...
uint32_t AdaptiveMaxPool3dGradCpuKernel::Compute(CpuKernelContext &ctx) {
  // check params
  CUST_KERNEL_HANDLE_ERROR(ctx, AdaptiveMaxPool3dGradCheck(ctx), "AdaptiveMaxPool3dGrad check params failed.");
  ...
  switch (input_grad_dtype * kDtypeNum + output_dtype) {
    ...
    default:
      ...
      if (std::find(support_dtype.begin(), support_dtype.end(), output_dtype) == support_dtype.end()) {
        CUST_KERNEL_LOG_ERROR(ctx, "AdaptiveMaxPool3dGrad kernel input data type [%s] not support.",
...
uint32_t AdaptiveMaxPool3dGradCpuKernel::AdaptiveMaxPool3dGradCheck(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, dim_num == 4 || dim_num == 5, KERNEL_STATUS_PARAM_INVALID,
                          "Input dimensions must be equal to 4 or 5")
  CUST_KERNEL_CHECK_FALSE(ctx, input_grad->GetTensorShape()->GetDims() == dim_num, KERNEL_STATUS_PARAM_INVALID,
                          "Input grad dimensions must be same as input dimensions")
  CUST_KERNEL_CHECK_FALSE(ctx, input_argmax->GetTensorShape()->GetDims() == dim_num, KERNEL_STATUS_PARAM_INVALID,
                          "Input indice dimensions must be same as input dimensions")
  CUST_KERNEL_CHECK_FALSE(ctx, output->GetTensorShape()->GetDims() == dim_num, KERNEL_STATUS_PARAM_INVALID,
                          "output dimensions must be same as input dimensions")
  CUST_KERNEL_CHECK_FALSE(ctx,
                          input_argmax->GetTensorShape()->GetDimSizes() == input_grad->GetTensorShape()->GetDimSizes(),
  ...
  CUST_KERNEL_LOG_DEBUG(ctx,
                        ...
                        DTypeStr(input_argmax->GetDataType()).c_str(), output->GetDataSize(),
...
uint32_t AdaptiveMaxPool3dGradCpuKernel::AdaptiveMaxPool3dGradCompute(CpuKernelContext &ctx) {
  CUST_KERNEL_CHECK_NULLPTR(ctx, ctx.Input(kFirstInputIndex)->GetData(), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_NULLPTR(ctx, ctx.Output(kFirstInputIndex)->GetData(), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, argmax_stride == 0, KERNEL_STATUS_PARAM_INVALID, "argmax shape can not contain 0.")
  ...
  if (enable_parallel) {
    CUST_KERNEL_HANDLE_ERROR(
      ctx, CpuKernelUtils::ParallelFor(ctx, input_num_data, per_unit_size, shard_adaptive_max_pool_3d_grad),
