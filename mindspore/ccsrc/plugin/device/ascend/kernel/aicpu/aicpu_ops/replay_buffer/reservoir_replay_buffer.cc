...
bool ReservoirReplayBuffer::Sample(const size_t &batch_size, const std::vector<uintptr_t> &output) {
  ...
  for (size_t i = 0; i < batch_size; i++) {
    ...
    for (size_t item_index = 0; item_index < schema_.size(); item_index++) {
      void *offset = reinterpret_cast<uint8_t *>(output[item_index]) + schema_[item_index] * i;
      auto ret = memcpy_s(offset, schema_[item_index], transition[item_index]->addr, transition[item_index]->size);
      if (ret != EOK) {
