...
uint32_t UnpackCpuKernel::CheckAndInitParams(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_CHECK_NULLPTR(ctx, value_data_ptr, KERNEL_STATUS_PARAM_INVALID, "Get input value data failed.");
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, unpack_axis_ptr, KERNEL_STATUS_PARAM_INVALID, "get axis failed!");
  unpack_axis = unpack_axis_ptr->GetInt();
  real_unpack_axis = unpack_axis >= 0 ? unpack_axis : unpack_axis + value_dim;
  CUST_KERNEL_CHECK_FALSE(ctx, value_dim > real_unpack_axis, KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, unpack_num == axis_size, KERNEL_STATUS_PARAM_INVALID,
  ...
  for (int64_t i = 0; i < unpack_num; i++) {
    ...
    CUST_KERNEL_CHECK_NULLPTR(ctx, output_ptr, KERNEL_STATUS_PARAM_INVALID, "Get output [%d] failed.", i);
    auto output_data_ptr = output_ptr->GetData();
    CUST_KERNEL_CHECK_NULLPTR(ctx, output_data_ptr, KERNEL_STATUS_PARAM_INVALID, "Get output data [%d] failed.", i);
...
template <typename T>
uint32_t UnpackCpuKernel::UnpackWithOneOutput(CpuKernelContext &ctx, T *input_data_ptr,
...
template <typename T>
uint32_t UnpackCpuKernel::UnpackWithDimZero(CpuKernelContext &ctx, T *input_data_ptr,
                                            std::vector<T *> output_data_vec) {
  if (value_shape_vec[0] == 0) {
    CUST_KERNEL_CHECK_FALSE(ctx, value_shape_vec[0] > 0, KERNEL_STATUS_PARAM_INVALID,
...
uint32_t UnpackCpuKernel::UnpackCompute(T *input_data_ptr, std::vector<T *> output_data_vec, CpuKernelContext &ctx) {
  ...
  for (uint64_t i = 0; i < unpack_axis; i++) {
    if (value_shape_vec[i] == 0) {
      CUST_KERNEL_CHECK_FALSE(ctx, value_shape_vec[i] > 0, KERNEL_STATUS_PARAM_INVALID,
                              "The shape of input tensor is invalid.");
    }
    prefix *= value_shape_vec[i];
  }
  if (unpack_axis >= value_shape_vec.size()) {
    CUST_KERNEL_CHECK_FALSE(ctx, unpack_axis < value_shape_vec.size(), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, CpuKernelUtils::ParallelFor(ctx, unpack_num, unpack_num / max_core_num, shard_unpack),
...
uint32_t UnpackCpuKernel::DoCompute(CpuKernelContext &ctx) {
  ...
  if (unpack_num == 1) {
    CUST_KERNEL_CHECK_FALSE(ctx, (UnpackWithOneOutput<T>(ctx, input_data_ptr, output_data_vec) == KERNEL_STATUS_OK),
                            KERNEL_STATUS_PARAM_INVALID, "UnpackWithOneOutput failed.");
    return KERNEL_STATUS_OK;
  }
  if (unpack_axis == 0) {
    CUST_KERNEL_CHECK_FALSE(ctx, (UnpackWithDimZero<T>(ctx, input_data_ptr, output_data_vec) == KERNEL_STATUS_OK),
                            KERNEL_STATUS_PARAM_INVALID, "UnpackWithDimZero failed.");
    return KERNEL_STATUS_OK;
  }
  CUST_KERNEL_CHECK_FALSE(ctx, (UnpackCompute<T>(input_data_ptr, output_data_vec, ctx) == KERNEL_STATUS_OK),
...
uint32_t UnpackCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "Unsupported data type [%s]", DTypeStr(data_type).c_str());
