...
#include "inc/maximum_grad_grad.h"
#include "register/op_impl_registry.h"
#include "utils/util.h"
#include "utils/common_shape_fns.h"
namespace ge {
// ----------------MaximumGradGrad Begin-------------------
CUST_IMPLEMT_VERIFIER(MaximumGradGrad, MaximumGradGradVerify) {
  ...
  if ((dim_grad_y1 != dims_x1)) {
    std::string err_msg1 = "The input grad_y1 and x1 are not the same.";
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg1);
    return GRAPH_FAILED;
  }
  if ((dim_grad_y2 != dims_x2)) {
    std::string err_msg2 = "The input grad_y2 and x2 are not the same.";
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), err_msg2);
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
}
CUST_VERIFY_FUNC_REG(MaximumGradGrad, MaximumGradGradVerify);

IMPLEMT_COMMON_INFERFUNC(MaximumGradGradInferShape) {
...
CUST_COMMON_INFER_FUNC_REG(MaximumGradGrad, MaximumGradGradInferShape);
// ----------------MaximumGradGrad END-----------
}  // namespace ge