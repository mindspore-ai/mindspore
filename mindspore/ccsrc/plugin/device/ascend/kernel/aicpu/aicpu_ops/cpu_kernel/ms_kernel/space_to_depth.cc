...
#define SPACETODEPTH_COMPUTE_CASE(DTYPE, TYPE, CTX)                      \
  case (DTYPE): {                                                        \
    ...
    if (result != KERNEL_STATUS_OK) {                                    \
      CUST_KERNEL_LOG_ERROR(ctx, "SpaceToDepth kernel compute failed."); \
      return result;                                                     \
    }                                                                    \
    break;                                                               \
  }
...
uint32_t SpaceToDepthCpuKernel::DoCompute(CpuKernelContext &ctx) {
  ...
  if (data_format_ == "NHWC") {
    CUST_KERNEL_CHECK_FALSE(ctx, (input_dims[h_nhwc] % block_size == zero || input_dims[w_nhwc] % block_size == zero),
    ...
    CUST_KERNEL_CHECK_FALSE(ctx, (input_dims[h_nchw] % block_size == 0 || input_dims[w_nchw] % block_size == 0),
                            KERNEL_STATUS_PARAM_INVALID, "Height and Weight must can be divided by block_size.");
...
uint32_t SpaceToDepthCpuKernel::STDParamCheck(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum),
                           "SpaceToDepth check input and output number failed.");
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (block_size >= min_block_size), KERNEL_STATUS_PARAM_INVALID,
                          "The value of block_size must be greater than 2");
  return KERNEL_STATUS_OK;
}

uint32_t SpaceToDepthCpuKernel::Compute(CpuKernelContext &ctx) {
  CUST_KERNEL_HANDLE_ERROR(ctx, STDParamCheck(ctx), "SpaceToDepth check params failed.");
  ...
  switch (input_data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "SpaceToDepth kernel data type[%s] not support.", DTypeStr(input_data_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
  }
  return KERNEL_STATUS_OK;
}
