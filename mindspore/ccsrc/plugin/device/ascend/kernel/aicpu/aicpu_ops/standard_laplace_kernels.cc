 ...
 */
#include "plugin/device/ascend/kernel/aicpu/aicpu_ops/standard_laplace_kernels.h"
...
uint32_t StandardLaplaceKernel::DoCompute() {
  float *tmp_out;
  if (out_count_ > 0) {
  ...
  for (uint64_t i = 0; i < out_count_; ++i) {
    float expo_random = expo(rng_);
    float uni_random = uni(rng_);
...
uint32_t StandardLaplaceKernel::ParseKernelParam() {
...
extern "C" {
__attribute__((visibility("default"))) uint32_t StandardLaplace(void *param) {
  aicpu::StandardLaplaceKernel standardlaplaceKernel;
  return standardlaplaceKernel.Compute(param);
}
}
