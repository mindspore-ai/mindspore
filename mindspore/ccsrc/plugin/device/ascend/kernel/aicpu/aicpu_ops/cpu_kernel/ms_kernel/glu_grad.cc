...
#define GLU_GRAD_COMPUTE_CASE(DTYPE, TYPE, CTX)                     \
  case (DTYPE): {                                                   \
    ...
    if (result != KERNEL_STATUS_OK) {                               \
      CUST_KERNEL_LOG_ERROR(ctx, "GluGrad kernel compute failed."); \
      return result;                                                \
    }                                                               \
    break;                                                          \
  }
...
uint32_t GluGradCpuKernel::Compute(CpuKernelContext &ctx) {
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kGluGradInputNum, kGluGradOutputNum), "GluGrad check params failed.");
  ...
  if (data_type_grads != data_type_x) {
    CUST_KERNEL_LOG_ERROR(ctx, "The data types of grads and x are not matched!");
    return KERNEL_STATUS_PARAM_INVALID;
  }
  if (data_type_x != data_type_y) {
    CUST_KERNEL_LOG_ERROR(ctx, "The data types of x and y are not matched!");
    return KERNEL_STATUS_PARAM_INVALID;
  }
  ...
  switch (data_type_x) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "GluGrad kernel data type [%u] not support.", data_type_x);
      return KERNEL_STATUS_PARAM_INVALID;
  }
...
uint32_t GluGradCpuKernel::GluGradCompute(CpuKernelContext &ctx) {
  ...
  if (dim < -rank || dim >= rank) {
    CUST_KERNEL_LOG_ERROR(ctx, "The axis value is out of range!");
  }
  if (dim < 0) {
    dim = dim + rank;
  }
  ...
  if (dim_sizes[dim] % two != 0) {
    CUST_KERNEL_LOG_ERROR(ctx, "The split dim should be even!");
  }
  int64_t n_m = 1;
  int64_t size_m = 0;
