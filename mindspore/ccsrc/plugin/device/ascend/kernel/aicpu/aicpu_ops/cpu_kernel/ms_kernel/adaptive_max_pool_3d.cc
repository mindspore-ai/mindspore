...
#define ADAPTIVE_MAX_POOL_3D_COMPUTE_CASE(DTYPE, TYPE, CTX)                   \
  case (DTYPE): {                                                             \
    ...
    if (result != KERNEL_STATUS_OK) {                                         \
      CUST_KERNEL_LOG_ERROR(ctx, "AdaptiveMaxPool3d kernel compute failed."); \
      return result;                                                          \
    }                                                                         \
    break;                                                                    \
  }
...
uint32_t AdaptiveMaxPool3dCpuKernel::Compute(CpuKernelContext &ctx) {
  // check params
  CUST_KERNEL_HANDLE_ERROR(ctx, AdaptiveMaxPool3dCheckAndSetShape(ctx), "AdaptiveMaxPool3d check params failed.");
  ...
  switch (compute_dtype) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "AdaptiveMaxPool3d kernel data type [%s] not support.",
                            DTypeStr(compute_dtype).c_str());
...
uint32_t AdaptiveMaxPool3dCpuKernel::AdaptiveMaxPool3dCheckAndSetShape(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(const_cast<CpuKernelContext &>(ctx), input_num, output_num),
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, input_num_dims == 4 || input_num_dims == 5, KERNEL_STATUS_PARAM_INVALID,
                          "Input dimensions must be equal to 4 or 5.");
  CUST_KERNEL_CHECK_FALSE(ctx, output_size->GetTensorShape()->GetDimSize(0) == 3, KERNEL_STATUS_PARAM_INVALID,
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    CUST_KERNEL_CHECK_FALSE(ctx, elem > 0, KERNEL_STATUS_PARAM_INVALID,
                            "Elements of output_size must be greater than 0");
  ...
  CUST_KERNEL_LOG_DEBUG(ctx,
                        ...
                        ctx.GetOpType().c_str(), x->GetDataSize(), DTypeStr(x->GetDataType()).c_str(),
                        output_size->GetDataSize(), DTypeStr(output_size->GetDataType()).c_str(), y->GetDataSize(),
                        DTypeStr(y->GetDataType()).c_str(), argmax->GetDataSize(),
                        DTypeStr(argmax->GetDataType()).c_str());

  return KERNEL_STATUS_OK;
}

int64_t AdaptiveMaxPool3dCpuKernel::ComputeStride(CpuKernelContext &ctx, const std::vector<int64_t> &shape,
                                                  size_t index) {
  CUST_KERNEL_CHECK_FALSE(ctx, index < shape.size(), KERNEL_STATUS_PARAM_INVALID,
                          "Input index must be less than shape dims.");
  int64_t result = 1;
...
uint32_t AdaptiveMaxPool3dCpuKernel::AdaptiveMaxPool3dCompute(CpuKernelContext &ctx) {
  ...
  const int64_t istrideB = ComputeStride(ctx, input_shape, dimB);
  const int64_t istrideD = ComputeStride(ctx, input_shape, dimD);
  const int64_t istrideT = ComputeStride(ctx, input_shape, dimT);
  const int64_t istrideH = ComputeStride(ctx, input_shape, dimH);
  const int64_t istrideW = ComputeStride(ctx, input_shape, dimW);
  ...
  auto get_per_unit_size = [&](int64_t data_size) -> int64_t {
    CUST_KERNEL_CHECK_FALSE(ctx, data_size != 0, KERNEL_STATUS_PARAM_INVALID, "data_size can not be 0.");
    ...
    switch (max_index) {
      case 0: {
        ...
        CUST_KERNEL_HANDLE_ERROR(
          ctx, CpuKernelUtils::ParallelFor(ctx, sizeB, get_per_unit_size(sizeB), shard_adaptive_max_pool_3d),
      ...
      case 1: {
        ...
        CUST_KERNEL_HANDLE_ERROR(
          ctx, CpuKernelUtils::ParallelFor(ctx, sizeD, get_per_unit_size(sizeD), shard_adaptive_max_pool_3d),
          "AdaptiveMaxPool3d kernel compute failed.");
      } break;
      ...
      case 2: {
        ...
        CUST_KERNEL_HANDLE_ERROR(
          ctx, CpuKernelUtils::ParallelFor(ctx, osizeT, get_per_unit_size(osizeT), shard_adaptive_max_pool_3d),
          "AdaptiveMaxPool3d kernel compute failed.");
      } break;
      ...
      case 3: {
        ...
        CUST_KERNEL_HANDLE_ERROR(
          ctx, CpuKernelUtils::ParallelFor(ctx, osizeH, get_per_unit_size(osizeH), shard_adaptive_max_pool_3d),
          "AdaptiveMaxPool3d kernel compute failed.");
      } break;
      ...
      case 4: {
        ...
        CUST_KERNEL_HANDLE_ERROR(
          ctx, CpuKernelUtils::ParallelFor(ctx, osizeW, get_per_unit_size(osizeW), shard_adaptive_max_pool_3d),
          "AdaptiveMaxPool3d kernel compute failed.");
      } break;
    }
  }
