...
class BiasAddCpuKernel : public CpuKernel {
 ...
 private:
  ...
  uint32_t BiasAddCalculateWithAlignedCheck(const CpuKernelContext &ctx, BCalcInfo &calc_info);
  ...
  uint32_t BiasAddCalculate(const CpuKernelContext &ctx, BCalcInfo &calc_info);
};
}  // namespace aicpu
