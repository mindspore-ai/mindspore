...
uint32_t AcoshGradCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum),
                           "AcoshGrad check input and output number failed.");
  CUST_KERNEL_HANDLE_ERROR(ctx, AcoshGradParamCheck(ctx), "AcoshGrad check params failed.");
  ...
  switch (data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "AcoshGrad kernel data type [%s] not support.", DTypeStr(data_type).c_str());
...
uint32_t AcoshGradCpuKernel::AcoshGradParamCheck(CpuKernelContext &ctx) {
...
template <typename T>
uint32_t AcoshGradCpuKernel::AcoshGradComputeComplex(CpuKernelContext &ctx) {
  ...
  if (data_num >= kParallelDataNum) {
    ...
    CUST_KERNEL_HANDLE_ERROR(ctx,
                             CpuKernelUtils::ParallelFor(ctx, data_num, data_num / max_core_num, sharder_acosh_grad),
...
template <typename T>
uint32_t AcoshGradCpuKernel::AcoshGradComputeFP16(CpuKernelContext &ctx) {
  auto in0 = reinterpret_cast<T *>(ctx.Input(0)->GetData());
  auto in1 = reinterpret_cast<T *>(ctx.Input(1)->GetData());
  auto out = reinterpret_cast<T *>(ctx.Output(0)->GetData());
  if (data_num >= kParallelDataNum) {
...
template <typename T>
uint32_t AcoshGradCpuKernel::AcoshGradCompute(CpuKernelContext &ctx) {
  auto in0 = reinterpret_cast<T *>(ctx.Input(0)->GetData());
  auto in1 = reinterpret_cast<T *>(ctx.Input(1)->GetData());
  auto out = reinterpret_cast<T *>(ctx.Output(0)->GetData());
  if (data_num >= kParallelDataNum) {
