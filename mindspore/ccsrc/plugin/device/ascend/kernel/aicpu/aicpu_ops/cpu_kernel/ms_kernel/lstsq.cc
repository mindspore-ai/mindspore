...
uint32_t LstsqCpuKernel::Compute(CpuKernelContext &ctx) {
  // check params
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum), "Lstsq check input and output number failed.");
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (dims_0 == 2), KERNEL_STATUS_PARAM_INVALID,
                          "Dimension of input[0] must be 2, but got[%zu].", dims_0);
  CUST_KERNEL_CHECK_FALSE(ctx, ((dims_1 == 2) || (dims_1 == 1)), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (shape_0->GetDimSize(0) == shape_1->GetDimSize(0)), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_NULLPTR(ctx, I2_regularizer, KERNEL_STATUS_PARAM_INVALID, "Get l2_regularizer failed.");
  CUST_KERNEL_CHECK_NULLPTR(ctx, fast, KERNEL_STATUS_PARAM_INVALID, "Get fast failed.");
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (data_type1 == data_type2), KERNEL_STATUS_PARAM_INVALID,
  ...
  switch (data_type1) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "Lstsq kernel data type [%u] not support.", DTypeStr(data_type1).c_str());
