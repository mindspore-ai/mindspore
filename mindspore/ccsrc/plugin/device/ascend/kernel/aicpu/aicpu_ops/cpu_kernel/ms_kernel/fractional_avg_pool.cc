...
std::vector<int64_t> GeneratePoolingSequence(CpuKernelContext &ctx, int input_length, int output_length,
  ...
  for (int i = 0; i < output_length; i++) {
    if (diff[i] < k || diff[i] > k + 1) {
      CUST_KERNEL_LOG_ERROR(ctx, "FractionalAvgPool kernel GeneratePoolingSequence diff[%d] is error");
    }
  }
  std::vector<int64_t> cum_seq(output_length + 1, 0);
...
uint32_t FractionalAvgPoolCpuKernel::Compute(CpuKernelContext &ctx) {
  CUST_KERNEL_HANDLE_ERROR(ctx, FractionalAvgPoolParamCheck(ctx), "Check FractionalAvgPool params failed.");
  Tensor *input = ctx.Input(0);
  ...
  switch (data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "FractionalAvgPool kernel data type [%s] not support.", DTypeStr(data_type).c_str());
      return KERNEL_STATUS_PARAM_INVALID;
  }
  return KERNEL_STATUS_OK;
}
