...
namespace aicpu {
KernelStatus SparseSegmentMeanGradCpuKernel::CheckDataPara(CpuKernelContext &ctx) const {
...
KernelStatus SparseSegmentMeanGradCpuKernel::CheckShapePara(CpuKernelContext &ctx) const {
  ...
  if (scalarshape->GetDims() > 1) {  // Scalar input becomes 0-D Tensor in the dynamic shape scenerio, has dim 1
    CUST_KERNEL_LOG_ERROR(ctx, "[%s] Tensor outputdim0 should be a scalar.", ctx.GetOpType().c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  if (shape3->GetDims() < 1) {
    CUST_KERNEL_LOG_ERROR(ctx, "[%s] Tensor output0's rank less than 1.", ctx.GetOpType().c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  if (shape1->NumElements() != shape2->NumElements()) {
    CUST_KERNEL_LOG_ERROR(ctx, "[%s] segment _ids and indices should have same size.", ctx.GetOpType().c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  if (shape0->NumElements() / shape0->GetDimSize(0) != shape3->NumElements() / shape3->GetDimSize(0)) {
    CUST_KERNEL_LOG_ERROR(ctx, "[%s] Tensor input0&output0's ranks mismatch.", ctx.GetOpType().c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  if (shape3->GetDimSize(0) != *(output_dim0)) {
    CUST_KERNEL_LOG_ERROR(ctx, "[%s] Tensor output0's dim(0) mismatch with output_dim0.", ctx.GetOpType().c_str());
    return KERNEL_STATUS_PARAM_INVALID;
  }
  return KERNEL_STATUS_OK;
}
...
uint32_t SparseSegmentMeanGradCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  switch (data_type0) {
    ...
    case (DT_FLOAT16):
      ...
      break;
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "SparseSegmentMeanGrad kernel data type [%s] not support.",
...
KernelStatus SparseSegmentMeanGradCpuKernel::ComputeKernelWithType(CpuKernelContext &ctx) {
  ...
  for (int64_t i = 0; i < segment_ids_num; i++) {
    ...
    if ((index >= num_segments) || (index < 0)) {
      CUST_KERNEL_LOG_ERROR(ctx, "Segment id %d out of range [0, %d).", index, num_segments);
      return KERNEL_STATUS_PARAM_INVALID;
    }
  ...
  for (int64_t i = 0; i < segment_ids_num; i++) {
    ...
    if ((output_idx >= output_dim0) || (output_idx < 0)) {
      CUST_KERNEL_LOG_ERROR(ctx, "Index %lld out of range [0, %d).", output_idx, output_dim0);
      return KERNEL_STATUS_PARAM_INVALID;
    }
...
template <typename T>
KernelStatus SparseSegmentMeanGradCpuKernel::ComputeKernel(CpuKernelContext &ctx) {
