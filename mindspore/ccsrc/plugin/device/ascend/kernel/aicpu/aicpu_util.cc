...
int AicpuOpUtil::MsTypeToProtoType(TypeId ms_type) {
  static const std::map<mindspore::TypeId, mindspore::DataType> kMsProtoDataTypeMap = {
...
int AicpuOpUtil::ProtoTypeToMsType(int proto_type) {
  static const std::map<mindspore::DataType, mindspore::TypeId> kProtoDataTypeToMsDataTypeMap = {
  ...
  auto iter = kProtoDataTypeToMsDataTypeMap.find(static_cast<mindspore::DataType>(proto_type));
