...
internal::OpParamPtr InternalFlashAttentionScore::CreateOpParam(const std::vector<KernelTensor *> &inputs,
  ...
  int64_t head_num = primitive_->HasAttr("head_num") ? GetValue<int64_t>(primitive_->GetAttr("head_num")) : 0;
  int64_t head_dim = q_shape[kDim2] / head_num;
  int64_t pre_tokens = primitive_->HasAttr("pre_tokens") ? GetValue<int64_t>(primitive_->GetAttr("pre_tokens")) : 2147483647;
  int64_t next_tokens = primitive_->HasAttr("next_tokens") ? GetValue<int64_t>(primitive_->GetAttr("next_tokens")) : 0;
  
  param_ptr->head_num = head_num;
  param_ptr->inner_precise =
  ...
  param_ptr->sparse_mode = primitive_->HasAttr("sparse_mode") ? GetValue<int64_t>(primitive_->GetAttr("sparse_mode")) : 0;
  ...
  op_param.tor = primitive_->HasAttr("scale_value") ? GetValue<float>(primitive_->GetAttr("scale_value")) : 0;
