...
void UpsampleNearest2DGradAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  GetWorkspaceForResize(inputs[0], output_size, input_size, scales_h, scales_w, outputs[0]);
}
...
bool UpsampleNearest2DGradAscend::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  ParseGenExecutor(
    GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[0], output_size, input_size, scales_h, scales_w, outputs[0]));
