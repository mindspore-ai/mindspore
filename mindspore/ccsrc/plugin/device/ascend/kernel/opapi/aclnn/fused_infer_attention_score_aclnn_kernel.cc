...
void FusedInferAttentionScoreAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  value_tensors.assign(inputs.begin() + dyn_input_sizes[kIndex1],
                       inputs.begin() + dyn_input_sizes[kIndex1] + dyn_input_sizes[kIndex2]);
  ...
  auto scale = transform::ConvertKernelTensor<float>(inputs[real_input_idx_[kIndex18]]);
