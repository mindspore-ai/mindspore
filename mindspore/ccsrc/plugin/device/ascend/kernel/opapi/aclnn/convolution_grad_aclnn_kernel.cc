...
bool ConvolutionGradAscend::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  ParseGenExecutor(GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[kIndex0], inputs[kIndex1], inputs[kIndex2], bias_size_,
                                      stride_, padding_, dilation_, transposed_, output_padding_, groups_, output_mask_,
                                      OpApiUtil::GetCubeMathType(), outputs[kIndex0], outputs[kIndex1],
                                      outputs[kIndex2]));
