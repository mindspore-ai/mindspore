 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/normal_tensor_float_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
void NormalTensorFloatAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  GetWorkspaceForResize(inputs[kIndex0], std_, seed_stub, offset_stub, outputs[kIndex0]);
}
...
bool NormalTensorFloatAscend::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  ParseGenExecutor(GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[kIndex0], std_, seed, offset, outputs[kIndex0]));
