 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/elu_ext_aclnn_kernel.h"
#include <map>
#include <memory>
#include <functional>
#include "ir/tensor.h"
...
bool EluExtAscend::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
  ...
  ParseGenExecutor(
    GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[kIndex0], alpha_, scale_, input_scale_, outputs[kIndex0]));
