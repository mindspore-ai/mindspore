 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/normal_float_tensor_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
bool NormalFloatTensorAscend::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  ParseGenExecutor(GEN_EXECUTOR_BOOST(op_type_, hash_id_, mean_, inputs[kIndex1], seed, offset, outputs[kIndex0]));
