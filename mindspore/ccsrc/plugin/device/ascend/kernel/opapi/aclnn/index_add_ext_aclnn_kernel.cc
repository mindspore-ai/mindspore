 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/index_add_ext_aclnn_kernel.h"
#include "ir/tensor.h"
...
bool IndexAddExtAscend::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
  ...
  ParseGenExecutor(GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[kIndex0], axis_, inputs[kIndex1], inputs[kIndex2],
                                      alpha_, outputs[kIndex0]));
