 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/convolution_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
bool ConvolutionAscend::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
  ...
  ParseGenExecutor(GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[kIndex0], inputs[kIndex1], inputs[kIndex2], stride_,
                                      padding_, dilation_, transposed_, output_padding_, groups_, outputs[kIndex0],
