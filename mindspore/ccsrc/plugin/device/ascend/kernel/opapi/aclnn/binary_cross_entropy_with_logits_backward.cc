 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/binary_cross_entropy_with_logits_backward.h"
#include <vector>
...
namespace mindspore {
namespace kernel {
void BinaryCrossEntropyWithLogitsBackwardAclnnKernelMod::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
...
bool BinaryCrossEntropyWithLogitsBackwardAclnnKernelMod::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  RunOp(stream_ptr, workspace, inputs[kIndex0], inputs[kIndex1], inputs[kIndex2], inputs[kIndex3], inputs[kIndex4],
        reduction, outputs[kIndex0]);
  return true;
}

MS_ACLNN_KERNEL_FACTORY_REG(BinaryCrossEntropyWithLogitsBackward, BinaryCrossEntropyWithLogitsBackwardAclnnKernelMod);
}  // namespace kernel
}  // namespace mindspore
