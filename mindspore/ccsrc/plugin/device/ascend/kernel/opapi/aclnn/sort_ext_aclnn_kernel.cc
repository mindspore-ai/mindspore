 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/sort_ext_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
void SortExtAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  auto descending = transform::ConvertKernelTensor<bool>(inputs[kIndex2]);
  auto stable = transform::ConvertKernelTensor<bool>(inputs[kIndex3]);
...
bool SortExtAscend::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
  ...
  auto descending = transform::ConvertKernelTensor<bool>(inputs[kIndex2]);
  auto stable = transform::ConvertKernelTensor<bool>(inputs[kIndex3]);
  ParseGenExecutor(GEN_EXECUTOR_BOOST(op_type_, hash_id_, inputs[kIndex0], stable, dim, descending, outputs[kIndex0],
                                      outputs[kIndex1]));
