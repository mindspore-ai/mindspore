 ...
 */
#include "plugin/device/ascend/kernel/opapi/aclnn/masked_select_ext_aclnn_kernel.h"
#include "ir/tensor.h"
...
void MaskedSelectExtAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
...
bool MaskedSelectExtAscend::Launch(const std::vector<KernelTensor *> &inputs,
...
void MaskedSelectExtAscend::UpdateOutputShapeAndSize(const std::vector<KernelTensor *> &,
                                                     const std::vector<KernelTensor *> &outputs) {
  outputs[kIndex0]->SetShapeVector(outputs_shape_[kIndex0]);
}
MS_ACLNN_KERNEL_FACTORY_REG(MaskedSelectExt, MaskedSelectExtAscend);
}  // namespace kernel
}  // namespace mindspore
