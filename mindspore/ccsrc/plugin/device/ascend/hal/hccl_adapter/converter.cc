...
template <class T>
static T ConvertAttr(const PrimitivePtr &prim, const ge::OpDescPtr &ge_op, const std::string &anf_attr_name,
                     const std::string &ge_attr_name) {
  MS_EXCEPTION_IF_NULL(prim);
  MS_EXCEPTION_IF_NULL(ge_op);
  if (!prim->HasAttr(anf_attr_name)) {
    MS_LOG(INFO) << "Node " << prim->name() << " has no attr " << anf_attr_name << ", skip.";
    return T();
  }

  bool ret;
  auto attr = GetValue<T>(prim->GetAttr(anf_attr_name));
  ...
  if (!ret) {
    MS_LOG(INTERNAL_EXCEPTION) << "Set attr " << ge_attr_name << " for ge node of " << prim->name() << " failed.";
  }
...
static void SetAllToAllvAttr(const PrimitivePtr &prim, const std::vector<KernelTensor *> &inputs,
                             const std::vector<KernelTensor *> &outputs, const ge::OpDescPtr &ge_op,
                             const std::string &group) {
  MS_EXCEPTION_IF_NULL(prim);
  MS_EXCEPTION_IF_NULL(ge_op);
  if (!IsPrimitiveEquals(prim, prim::kPrimAllToAllv)) {
    return;
  }
  ...
  mindspore::hccl::AllToAllvCalcParam calc(prim, rank_size);
...
std::string GetGeNodeName(const PrimitivePtr &prim) {
  ...
  if (IsPrimitiveEquals(prim, prim::kPrimAllReduce)) {
    return kGeOpNameHcclAllRudece;
  } else if (IsPrimitiveEquals(prim, prim::kPrimAllGather)) {
    return kGeOpNameHcclAllGather;
  } else if (IsPrimitiveEquals(prim, prim::kPrimBroadcast)) {
    return kGeOpNameHcclBroadcast;
  } else if (IsPrimitiveEquals(prim, prim::kPrimReduceScatter)) {
    return kGeOpNameHcclReduceScatter;
  } else if (IsPrimitiveEquals(prim, prim::kPrimSend)) {
    return kGeOpNameHcclSend;
  } else if (IsPrimitiveEquals(prim, prim::kPrimReceive)) {
    return kGeOpNameHcclReceive;
  } else if (IsPrimitiveEquals(prim, prim::kPrimAllToAllv)) {
    return kGeOpNameHcclAllToAllV;
  }

  MS_LOG(EXCEPTION) << "Unknown hccl node type " << prim->name();
}
...
std::tuple<ge::NodePtr, ge::ComputeGraphPtr> GenerateStubGeNode(const PrimitivePtr &prim,
  ...
  ge::OpDescPtr op_desc = std::make_shared<ge::OpDesc>(kStubDataStructureName, GetGeNodeName(prim));
  MS_EXCEPTION_IF_NULL(op_desc);
  for (KernelTensor *tensor : inputs) {
    (void)op_desc->AddInputDesc(ge::GeTensorDesc(ge::GeShape(tensor->GetDeviceShapeVector()), ge::Format::FORMAT_NCHW,
                                                 transform::ConvertDataType(tensor->dtype_id())));
  }
  for (KernelTensor *tensor : outputs) {
    (void)op_desc->AddOutputDesc(ge::GeTensorDesc(ge::GeShape(tensor->GetDeviceShapeVector()), ge::Format::FORMAT_NCHW,
                                                  transform::ConvertDataType(tensor->dtype_id())));
  }
  ...
  if (!ret) {
    MS_LOG(INTERNAL_EXCEPTION) << "Set attr " << ge::HCOM_ATTR_DATA_TYPE << " for ge node of " << prim->name()
                               << " failed.";
  }

  // set node attr
  (void)ConvertAttr<int64_t>(prim, op_desc, kAttrRankSize, ge::HCOM_ATTR_RANK_SIZE);
  auto group = ConvertAttr<std::string>(prim, op_desc, kAttrGroup, ge::HCOM_ATTR_GROUP);
  (void)ConvertAttr<int64_t>(prim, op_desc, kAttrSrcRank, ge::HCOM_ATTR_SRC_RANK);
  (void)ConvertAttr<int64_t>(prim, op_desc, kAttrDestRank, ge::HCOM_ATTR_DEST_RANK);
  (void)ConvertAttr<int64_t>(prim, op_desc, kAttrSrTag, ge::HCOM_ATTR_SR_TAG);
  (void)ConvertAttr<int64_t>(prim, op_desc, kAttrComm, "comm");
  (void)ConvertAttr<std::vector<int64_t>>(prim, op_desc, kAttrShape, ge::HCOM_ATTR_SHAPE);
  SetAllToAllvAttr(prim, inputs, outputs, op_desc, group);
