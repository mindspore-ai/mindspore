...
bool AscendCollectiveCommLib::AllGather(const void *send_buff, void *recv_buff, size_t send_count, TypeId data_type,
  ...
  const auto comm = HcclCommunicator(group_name);
  MS_EXCEPTION_IF_NULL(comm);

  auto hccl_result =
    hccl::HcclAdapter::GetInstance().HcclAllGather(send_buff, recv_buff, hccl_count_, hccl_data_type, stream, comm);
...
bool AscendCollectiveCommLib::AllReduce(const void *send_buff, void *recv_buff, size_t send_count, TypeId data_type,
  ...
  auto hccl_result = hccl::HcclAdapter::GetInstance().HcclAllReduce(send_buff, recv_buff, send_count, hccl_data_type,
                                                                    hccl_reduce_type, stream, comm);
  if (hccl_result != HCCL_SUCCESS) {
...
bool AscendCollectiveCommLib::Broadcast(const void *send_buff, void *, size_t send_count, TypeId data_type,
  ...
  auto hccl_result =
    hccl::HcclAdapter::GetInstance().HcclBroadcast(send_buff, send_count, hccl_data_type, root_rank, stream, comm);
  if (hccl_result != HCCL_SUCCESS) {
...
bool AscendCollectiveCommLib::ReduceScatter(const void *send_buff, void *recv_buff, size_t recv_count, TypeId data_type,
  ...
  auto hccl_result = hccl::HcclAdapter::GetInstance().HcclReduceScatter(send_buff, recv_buff, recv_count,
                                                                        hccl_data_type, hccl_reduce_type, stream, comm);
  if (hccl_result != HCCL_SUCCESS) {
...
bool AscendCollectiveCommLib::Send(const void *send_buff, size_t count, TypeId data_type, uint32_t peer,
  ...
  const auto comm = HcclCommunicator(group_name);
  MS_EXCEPTION_IF_NULL(comm);
  auto hccl_result = hccl::HcclAdapter::GetInstance().HcclSend(send_buff, count, hccl_data_type, peer, stream, comm);
  if (hccl_result != HCCL_SUCCESS) {
