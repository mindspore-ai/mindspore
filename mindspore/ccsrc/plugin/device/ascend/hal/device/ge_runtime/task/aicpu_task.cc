...
void AicpuTask::Distribute() {
  ...
  rtError_t rt_ret = aclrtMalloc(&args_, args_size_, ACL_MEM_TYPE_HIGH_BAND_WIDTH);
  if (rt_ret != RT_ERROR_NONE) {
...
void AicpuTask::SetAicpuParamHead(uint32_t args_size, uint32_t io_addrs_num) {
  ...
  if (ext_info.empty()) {
    ...
    rtError_t flag = aclrtMalloc(&ext_info_addr_, ext_info_handler->GetExtInfoLen(), ACL_MEM_TYPE_HIGH_BAND_WIDTH);
