...
CNodePtr AllToAllUnifyMindIR::CreateSplitNode(const FuncGraphPtr &graph, const CNodePtr &all_to_all) const {
  ...
  if (IsDynamic(shape)) {
    auto min_shape = common::AnfAlgo::GetOutputMinShape(all_to_all_input, IntToSize(0));
    auto max_shape = common::AnfAlgo::GetOutputMaxShape(all_to_all_input, IntToSize(0));
...
CNodePtr AllToAllUnifyMindIR::CreateAllToAllvNode(const FuncGraphPtr &graph, const CNodePtr &all_to_all,
  ...
  auto single_shape = common::AnfAlgo::GetOutputDetailShape(split_outputs[0], IntToSize(0));
  auto single_type = common::AnfAlgo::GetOutputInferDataType(split_outputs[0], IntToSize(0));
...
CNodePtr AllToAllUnifyMindIR::CreateConcatNode(const FuncGraphPtr &graph, const CNodePtr &all_to_all,
  ...
  if (IsDynamic(single_shape)) {
    auto min_shape = common::AnfAlgo::GetOutputMinShape(all_to_all_v_outputs[0], IntToSize(0));
    auto max_shape = common::AnfAlgo::GetOutputMaxShape(all_to_all_v_outputs[0], IntToSize(0));
    if (!min_shape.empty() && !max_shape.empty()) {
    ...
    common::AnfAlgo::SetOutputInferTypeAndShape(
      {common::AnfAlgo::GetOutputInferDataType(all_to_all_v_outputs[0], IntToSize(0))}, {single_shape}, concat.get());
  }
