...
ShapeVector GetShapeVec(const abstract::BaseShapePtr &base_shape) {
...
ValueNodePtr AvgPoolFusion::ConstructFilterValueNode(const KernelGraphPtr &graph, float factor,
  ...
  std::vector<size_t> infer_shape;
  std::transform(assist_shape.begin(), assist_shape.end(), std::back_inserter(infer_shape), LongToSize);
...
ValueNodePtr AvgPoolFusion::ConstructFilterValueNodeDynamic(const KernelGraphPtr &graph, float factor,
  ...
  auto value_node = graph->NewValueNode(assist_abstract, assist_tensor);
  std::vector<size_t> infer_shape;
  std::transform(host_shape.begin(), host_shape.end(), std::back_inserter(infer_shape), LongToSize);
  common::AnfAlgo::SetOutputInferTypeAndShape({kNumberTypeFloat16}, {infer_shape}, value_node.get());
  return value_node;
}
...
const AnfNodePtr AvgPoolFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto detail_in_shape = common::AnfAlgo::GetPrevNodeOutputDetailShape(avgpool, 0);
  auto detail_out_shape = common::AnfAlgo::GetOutputDetailShape(avgpool, 0);
  auto input_shape = GetShapeVec(detail_in_shape);
  auto output_shape = GetShapeVec(detail_out_shape);
