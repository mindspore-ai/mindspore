...
OutputInfo GetNodeOutputInfo(const AnfNodePtr &node) {
  ...
  for (size_t i = 0; i < output_num; i++) {
    (void)output_infer_dtype.emplace_back(AnfAlgo::GetOutputInferDataType(type_ptr, i));
    (void)output_infer_shape.emplace_back(AnfAlgo::GetOutputInferShape(node, shape_ptr, i));
