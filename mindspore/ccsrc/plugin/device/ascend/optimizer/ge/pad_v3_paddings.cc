...
bool PadV3Paddings::HasDynPaddings(const CNodePtr &cnode) const {
...
const CNodePtr PadV3Paddings::CreateStridedSlice(const FuncGraphPtr &func_graph, const AnfNodePtr &input_node,
                                                 int64_t index) const {
  // set inputs
...
const CNodePtr PadV3Paddings::CreateConcatNode(const FuncGraphPtr &func_graph,
...
const CNodePtr PadV3Paddings::ProcessSliceNConcat(const FuncGraphPtr &func_graph, const AnfNodePtr &pad_node,
  ...
  if (paddings_contiguous) {
    ...
    for (int64_t i = 0; i < padding_src_length / 2; ++i) {
      auto slice_node_2 = CreateStridedSlice(func_graph, input_node, i + padding_src_length / 2);
      concat_input_vec.insert(concat_input_vec.begin(), slice_node_2);
  ...
  auto concat_node =
    CreateConcatNode(func_graph, concat_input_vec, pad_node->fullname_with_scope() + "_pad_slice_concat");
  return concat_node;
}

template <typename T, TypeId type_id>
const AnfNodePtr PadV3Paddings::OptimizePaddingsValue(const FuncGraphPtr &graph, const AbstractBasePtr &ori_paddings,
...
const AnfNodePtr PadV3Paddings::Process(const FuncGraphPtr &graph, const AnfNodePtr &node, const EquivPtr &) const {
  ...
  if (HasDynPaddings(cnode)) {
    ...
    auto concat_node = ProcessSliceNConcat(graph, cnode, padding, input_x_shape.size() * 2, padding_shape[0]);
    MS_EXCEPTION_IF_NULL(concat_node);
    ...
    auto paddings_value_node = CreatePaddingsNode(graph, paddings_abstract, paddings_contiguous,
...
const BaseRef PadV3Paddings::DefinePattern() const {
