...
class BackendOpRunTask : public BackendOpTask {
 public:
  BackendOpRunTask(std::shared_ptr<OpTaskContext> context,
                   std::function<void(const std::shared_ptr<OpTaskContext> &context)> run, std::future<bool> future)
      : BackendOpTask(std::move(context), kOpRunTask), run_(std::move(run)), future_(std::move(future)) {}
  ~BackendOpRunTask() override = default;
  void Run() override;

 private:
  std::function<void(const std::shared_ptr<OpTaskContext> &context)> run_;
