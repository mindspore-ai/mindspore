...
namespace mindspore {
namespace runtime {
void KernelResizeActor::Init() {
  KernelActor::Init();

  // Erase output and workspace device tensors which is released by kernel infer actor and kernel actor.
...
void KernelResizeActor::RunOpData(OpData<DeviceTensor> *const input_data, OpContext<DeviceTensor> *const context) {
  ...
  // Without verifying that the device pointer for device tensor is empty, the kernel before the KernelInferActor phase
...
void KernelResizeActor::Run(OpContext<DeviceTensor> *const context) {
  ...
  try {
    ...
    if (!device_contexts_[0]->device_res_manager_->BindDeviceToCurrentThread(false)) {
      std::string error_info = "BindDevice to current thread failed: " + kernel_->fullname_with_scope();
    ...
    std::string error_info =
      "Run kernel resize actor failed for kernel: " + kernel_->fullname_with_scope() + ", exception: " + e.what();
    SET_OPCONTEXT_FAIL_RET_WITH_ERROR_BY_STRATEGY(GraphExecutionStrategy::kPipeline, (*context), error_info);
  }
