...
int BNGradCPUKernel::Run() {
  ...
  int error_code;
  if (thread_num_ == 1) {
    error_code = ParallelLaunch(this->context_->thread_pool_, BNGradRun, this, thread_num_);
    if (error_code != RET_OK) {
    ...
    for (size_t stage = 0; stage < threads.size(); stage++) {
      ...
      error_code = ParallelLaunch(this->context_->thread_pool_, BNGradRun, this, threads.at(stage));
      if (error_code != RET_OK) {
        MS_LOG(ERROR) << "BN function error error_code[" << error_code << "]";
        return RET_ERROR;
      }
