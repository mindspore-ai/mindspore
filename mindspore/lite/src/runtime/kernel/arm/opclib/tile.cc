...
int TileOneDimension(float *input_data, float *output_data, size_t dim, TileParameter *parameter) {
  size_t src_dim_size = parameter->in_shape_[dim];
  if (dim == parameter->in_dim_ - 1) {
    CopyData(input_data, output_data, src_dim_size, parameter->multiples_[dim]);
    return 0;
  }
...
void Tile(float *input_data, float *output_data, TileParameter *parameter) {
  TileOneDimension(input_data, output_data, 0, parameter);
}
