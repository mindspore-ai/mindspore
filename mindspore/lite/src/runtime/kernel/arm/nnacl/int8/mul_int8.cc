...
void MulInt8NEON(int8_t *input0_data, int8_t *input1_data, int8_t *output_data, int64_t real_dst_count,
  ...
  for (; (*index) <= real_dst_count - 8; (*index) += 8) {
    ...
    int16x4_t sum_low =
      ClacSumHalfWord(input0_low, input1_low, left_shift_out_vec, output_multiplier_vec, para);
    int16x4_t sum_high =
      ClacSumHalfWord(input0_high, input1_high, left_shift_out_vec, output_multiplier_vec, para);
...
void Mul(int8_t *input0_data, int8_t *input1_data, int8_t *output_data, int64_t real_dst_count, MulQuantArg para) {
  ...
  for (; index < real_dst_count; ++index) {
    ...
    int32_t mul_result = RoundingDivideByPOT(
      SaturatingRoundingDoublingHighMul(input0_val * input1_val * (1 << para.shift_left_),
                                        para.output_multiplier_), para.shift_right_);
    ...
    if (mul_result > para.output_activation_max_) {
      ...
      output_data[index] = static_cast<int8_t>(mul_result);
    }
  }
  return;
}
