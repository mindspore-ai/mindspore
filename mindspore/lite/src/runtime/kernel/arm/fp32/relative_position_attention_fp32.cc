...
int RelativePositionAttentionCPUKernel::CheckInputs() {
  ...
  if (input_q_tensor_ == nullptr || input_q_tensor_->data_type() != kNumberTypeFloat32 ||
      input_q_tensor_->shape().size() != 3 || input_q_tensor_->shape().at(0) != 1) {
    MS_LOG(ERROR) << "input_q is abnormal.";
    return RET_ERROR;
  }
  input_k_tensor_ = this->in_tensors_.at(1);
  if (input_k_tensor_ == nullptr || input_k_tensor_->data_type() != kNumberTypeFloat32 ||
      input_k_tensor_->shape().size() != 3 || input_k_tensor_->shape().at(0) != 1) {
    MS_LOG(ERROR) << "input_k is abnormal.";
    return RET_ERROR;
  }
  input_v_tensor_ = this->in_tensors_.at(2);
  if (input_v_tensor_ == nullptr || input_v_tensor_->data_type() != kNumberTypeFloat32 ||
      input_v_tensor_->shape().size() != 3 || input_v_tensor_->shape().at(0) != 1) {
    MS_LOG(ERROR) << "input_v is abnormal.";
    return RET_ERROR;
  }
  input_p_tensor_ = this->in_tensors_.at(3);
  if (input_p_tensor_ == nullptr || input_p_tensor_->data_type() != kNumberTypeFloat32 ||
      input_p_tensor_->shape().size() != 3 || input_p_tensor_->shape().at(0) != 1) {
