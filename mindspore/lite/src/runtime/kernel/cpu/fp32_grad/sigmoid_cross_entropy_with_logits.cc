...
#include "src/runtime/kernel/cpu/fp32_grad/sigmoid_cross_entropy_with_logits.h"
...
kernel::LiteKernel *CpuSigmoidCrossEntropyWithLogitsFp32KernelCreator(const std::vector<lite::Tensor *> &inputs,
  ...
  auto *kernel = new (std::nothrow)
    SigmoidCrossEntropyWithLogitsCPUKernel(opParameter, inputs, outputs, static_cast<const lite::InnerContext *>(ctx));
  if (kernel == nullptr) {
