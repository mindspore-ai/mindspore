...
#include "src/runtime/sub_graph_split.h"
#include <cstdlib>
#include <utility>
#include <algorithm>
#include <iterator>
...
void SearchSubGraph::DoOnlineFusion() {
  ...
  DoReduceConcatFusionPass();
  DoCastGatherReduceFusionPass();
}
...
std::vector<std::vector<uint32_t>> SearchSubGraph::GetFrontNodeIndex(LiteGraph::Node *cur_node) {
...
bool SearchSubGraph::SatifyGatherReduceParse(Subgraph *subgraph, uint32_t in_node,
  ...
  if (gather_in0_tensor.type_ != TensorType::CONST) {
    return false;
  }
  auto &gather_in2_tensor = tensors_.at(gather_in_indices.at(C2NUM));
  if (gather_in2_tensor.type_ != TensorType::CONST ||
  ...
  if (reduce_in1_tensor.type_ != TensorType::CONST ||
...
void SearchSubGraph::DeleteReduceConcatOriginNode(Subgraph *subgraph, const std::vector<uint32_t> &positions) {
  auto sub_graph = model_->graph_.sub_graphs_.at(0);
  auto &subgraph_node_indices = sub_graph->node_indices_;
...
void SearchSubGraph::DeleteCastGatherReduceOriginNode(Subgraph *subgraph) {
  auto sub_graph = model_->graph_.sub_graphs_.at(0);
  auto &subgraph_node_indices = sub_graph->node_indices_;
  for (auto &node_index : subgraph->nodes_) {
    // delete tensors_ tensor info
...
bool SearchSubGraph::SatifyReduceConcatParse(uint32_t in_node, int *lastAxisSize) {
...
bool SearchSubGraph::DoReduceConcatFusion(uint32_t node_id) {
  node_list_ = model_->graph_.all_nodes_;
  auto &node = node_list_[node_id];
  // the first node is split op
  if (GetPrimitiveType(node->primitive_, SCHEMA_VERSION::SCHEMA_CUR) != schema::PrimitiveType_Concat) {
...
bool SearchSubGraph::DoCastGatherReduceFusion(uint32_t node_id) {
  node_list_ = model_->graph_.all_nodes_;
  auto &node = node_list_[node_id];
  ...
  if (GetPrimitiveType(node->primitive_, SCHEMA_VERSION::SCHEMA_CUR) == schema::PrimitiveType_Cast) {
    ...
    if (cast_in1_tensor.type_ != TensorType::CONST) {
      return false;
    }
...
int SearchSubGraph::CreateReduceConcatCustomNode(LiteGraph::Node *node, Subgraph *subgraph,
                                                 std::vector<uint32_t> *new_input_indices,
                                                 std::vector<uint32_t> *positions) {
  MS_ASSERT(node != nullptr);
  flatbuffers::FlatBufferBuilder fbb(kInitialSize);
...
int SearchSubGraph::CreateCastGatherReduceCustomNode(LiteGraph::Node *node, Subgraph *subgraph,
                                                     std::vector<uint32_t> *new_input_indices) {
  MS_ASSERT(node != nullptr);
  flatbuffers::FlatBufferBuilder fbb(kInitialSize);
  ...
  node->name_ = "CastGatherReduceFusion";
  node->primitive_ = online_fusion_prim;
  node->node_type_ = PrimType::PrimType_Inner_SplitReduceConcatFusion;
  node->input_indices_ = *new_input_indices;
  node->output_indices_ = model_->graph_.all_nodes_.at(subgraph->ends_.front())->output_indices_;
