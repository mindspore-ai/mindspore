 ...
 */
#include "src/litert/kernel/cpu/fp32/arithmetic_self_fp32.h"
...
ArithmeticSelfIntFunc ArithmeticSelfCPUKernel::GetArithmeticSelfIntFun(int primitive_type) const {
  static std::unordered_map<int, ArithmeticSelfIntFunc> func_table = {
    {mindspore::schema::PrimitiveType_Abs, ElementAbsInt}, {mindspore::schema::PrimitiveType_Neg, ElementNegativeInt}};
  auto iter = func_table.find(primitive_type);
...
int ArithmeticSelfCPUKernel::Prepare() {
  ...
  if (out_tensors_[0]->category() == mindspore::lite::CONST_SCALAR ||
      out_tensors_[0]->category() == mindspore::lite::CONST_TENSOR) {
    MS_LOG(ERROR) << "out_tensors category is invalid.";
    return RET_ERROR;
  }
  if (!InferShapeDone()) {
    return RET_OK;
...
int ArithmeticSelfCPUKernel::DoExecute(int task_id) {
  ...
  if (ret != RET_OK) {
    MS_LOG(ERROR) << "Run failed, illegal input!ret: " << ret;
  }
  return ret;
}
