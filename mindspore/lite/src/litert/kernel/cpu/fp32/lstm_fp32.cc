...
LiteKernel *LstmFp32KernelCreator(const std::vector<lite::Tensor *> &inputs, const std::vector<lite::Tensor *> &outputs,
  ...
  if (inputs.size() == kMindirInputTensorNum) {
    kernel = new (std::nothrow) LstmMindirFp32CPUKernel(parameter, inputs, outputs, ctx);
  } else {
    kernel = new (std::nothrow) LstmNonMindirFp32CPUKernel(parameter, inputs, outputs, ctx);
  }
  if (kernel == nullptr) {
