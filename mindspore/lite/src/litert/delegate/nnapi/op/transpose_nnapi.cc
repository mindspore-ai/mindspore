...
namespace mindspore {
namespace lite {
bool NNAPITranspose::IsSupport() {
  return in_tensors_.size() == kInputSize1 && in_tensors_.at(0).Shape().size() <= DIMENSION_4D;
}

int NNAPITranspose::AddOpToNNAPIModel(ANeuralNetworksModel *nnapi_model,
                                      std::vector<mindspore::MSTensor> *all_tensors) {
  MS_ASSERT(nnapi_model != nullptr && all_tensors != nullptr);
  OperationCode node_type = ANEURALNETWORKS_TRANSPOSE;
