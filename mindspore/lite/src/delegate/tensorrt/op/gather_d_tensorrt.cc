...
#include "src/delegate/tensorrt/op/gather_d_tensorrt.h"
...
int GatherDTensorRT::AddInnerOp(nvinfer1::INetworkDefinition *network) {
  ...
  auto plugin = std::make_shared<GatherDPlugin>(op_name_, dim);
...
size_t GatherDPlugin::getSerializationSize() const noexcept {
  // axis_
  return sizeof(int);
}

void GatherDPlugin::serialize(void *buffer) const noexcept { SerializeValue(&buffer, &axis_, sizeof(int)); }
