...
int NPUInferFormatPass::Run(NPUGraph *subgraph) {
  ...
  std::map<const mindspore::MSTensor *, bool> is_inferred;
  // initialization
  for (auto op : *all_ops_) {
    infer_ops.push(op);
  }
  for (auto tensor : *all_tensors_) {
    is_inferred[tensor] = false;
  }
  for (auto input_tensor : graph_inputs) {
    is_inferred[&input_tensor] = true;
  }
  while (!infer_ops.empty()) {
    auto cur_op = infer_ops.front();
    infer_ops.pop();
    bool input_inferred = std::all_of(cur_op->inputs().begin(), cur_op->inputs().end(),
                                      [&](mindspore::MSTensor in_tensor) { return is_inferred[&in_tensor] == true; });
    if (input_inferred) {
      ...
      for (auto out_tensor : cur_op->outputs()) {
        out_tensor.SetFormat(dst_format);
        is_inferred[&out_tensor] = true;
      }
    } else {
