...
int ConvolutionDepthwiseInt8NPUOp::Init(const schema::Primitive *primitive,
                                        const std::vector<mindspore::MSTensor> &in_tensors,
                                        const std::vector<mindspore::MSTensor> &out_tensors) {
  conv_dw_ = new (std::nothrow) hiai::op::QuantizedConvolutionDepthwise(name_ + "_conv_depthwise");
  if (conv_dw_ == nullptr) {
  ...
  SetQuantParam(conv_dw_, in_tensors);
...
int ConvolutionDepthwiseInt8NPUOp::SetNPUInputs(const std::vector<mindspore::MSTensor> &in_tensors,
                                                const std::vector<mindspore::MSTensor> &out_tensors,
...
ge::Operator *ConvolutionDepthwiseInt8NPUOp::GetNPUOp() {
  if (act_type_ == schema::ActivationType_NO_ACTIVATION) {
    return conv_dw_;
  } else {
    return act_;
...
ConvolutionDepthwiseInt8NPUOp::~ConvolutionDepthwiseInt8NPUOp() {
  if (conv_dw_ != nullptr) {
    delete conv_dw_;
    conv_dw_ = nullptr;
  }
