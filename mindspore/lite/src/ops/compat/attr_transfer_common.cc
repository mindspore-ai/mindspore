...
schema::Tensor *AttrToTensor(const void *data, size_t data_size, bool is_array, TypeId type_id,
  ...
  auto dst_tensor =
    (is_array ? new (std::nothrow) Tensor(type_id, shape, mindspore::NHWC, Tensor::Category::CONST_TENSOR)
              : new (std::nothrow) Tensor(type_id, shape, mindspore::NHWC, Tensor::Category::CONST_SCALAR));
