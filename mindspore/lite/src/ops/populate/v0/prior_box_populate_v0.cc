...
OpParameter *PopulatePriorBoxParameter(const void *prim) {
  ...
  if (prior_box_prim->min_sizes()->size() > PRIOR_BOX_MAX_NUM) {
  ...
  if (prior_box_prim->max_sizes()->size() > PRIOR_BOX_MAX_NUM) {
  ...
  if (prior_box_prim->aspect_ratios()->size() > PRIOR_BOX_MAX_NUM) {
  ...
  if (prior_box_prim->variances()->size() != PRIOR_BOX_VAR_NUM) {
    MS_LOG(ERROR) << "PriorBox variances size should be " << PRIOR_BOX_VAR_NUM << ", got "
                  << prior_box_prim->variances()->size();
    free(prior_box_param);
    return nullptr;
  }
  memcpy(prior_box_param->variances, prior_box_prim->variances()->data(), PRIOR_BOX_VAR_NUM * sizeof(float));
