...
OpParameter *PopulateFlattenParameter(const void *prim) {
  ...
  flatten_param->type_ = schema::PrimitiveType_Flatten;
  return reinterpret_cast<OpParameter *>(flatten_param);
}
}  // namespace

Registry g_flattenV0ParameterRegistry(schema::v0::PrimitiveType_Flatten, PopulateFlattenParameter, SCHEMA_V0);
}  // namespace lite
}  // namespace mindspore
