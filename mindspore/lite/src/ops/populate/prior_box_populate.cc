...
OpParameter *PopulatePriorBoxParameter(const void *prim) {
  ...
  if (value->min_sizes()->size() > PRIOR_BOX_MAX_NUM) {
    MS_LOG(ERROR) << "PriorBox min_sizes size exceeds max num " << PRIOR_BOX_MAX_NUM << ", got " << value->min_sizes();
    free(prior_box_param);
    return nullptr;
  }
  prior_box_param->min_sizes_size = value->min_sizes()->size();
  if (value->max_sizes()->size() > PRIOR_BOX_MAX_NUM) {
    MS_LOG(ERROR) << "PriorBox max_sizes size exceeds max num " << PRIOR_BOX_MAX_NUM << ", got " << value->max_sizes();
    free(prior_box_param);
    return nullptr;
  }
  ...
  if (value->aspect_ratios()->size() > PRIOR_BOX_MAX_NUM) {
  ...
  if (value->variances()->size() != PRIOR_BOX_VAR_NUM) {
    MS_LOG(ERROR) << "PriorBox variances size should be " << PRIOR_BOX_VAR_NUM << ", got "
                  << value->variances()->size();
    free(prior_box_param);
    return nullptr;
  }
  memcpy(prior_box_param->variances, value->variances()->data(), PRIOR_BOX_VAR_NUM * sizeof(float));
