...
int TrainExport::KeepGraphInputsInOrder(const Model *model) {
  ...
  MS_CHECK_TRUE_MSG(meta_graph_->inputIndex.size() <= model->input_indices_.size(), RET_ERROR,
                    "export model input indices size is large than origin input indices size.");
  std::vector<uint32_t> origin_inputs_order;
  for (auto index : model->input_indices_) {
    MS_CHECK_TRUE_MSG(index < model->all_tensors_.size(), RET_ERROR, "input index out of range.");
    auto ori_input_tensor = model->all_tensors_[index];
