...
int SelectTensorRT::IsSupport(const BaseOperatorPtr &base_operator, const std::vector<TensorInfo> &in_tensors,
...
int SelectTensorRT::AddInnerOp(TensorRTContext *ctx) {
  ...
  auto plugin = std::make_shared<SelectPlugin>(op_name_);
...
REGISTER_TENSORRT_CREATOR(ops::kNameSelect, SelectTensorRT)
}  // namespace mindspore::lite
