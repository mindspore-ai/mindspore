...
int ScatterNdTensorRT::AddInnerOp(nvinfer1::INetworkDefinition *network) {
#if TRT_VERSION_GE(8, 2)
  ITensorHelper scatter_input;
  int ret = PreprocessInputs2SameDim(network, tensorrt_in_tensors_[0], &scatter_input);
  ...
  if (tensorrt_in_tensors_.size() < INPUT_SIZE3) {
    auto indices = ConvertConstantTensor(network, in_tensors_[1], op_name_ + "_indice");
    if (indices == nullptr) {
      MS_LOG(ERROR) << "add const input tensor failed for " << op_name_;
      return RET_ERROR;
    }
    tensorrt_in_tensors_.push_back(ITensorHelper{indices});
    auto updates = ConvertConstantTensor(network, in_tensors_[INPUT_SIZE2], op_name_ + "_update");
    if (updates == nullptr) {
      MS_LOG(ERROR) << "add const input tensor failed for " << op_name_;
      return RET_ERROR;
    }
  ...
  ret = PreprocessInputs2SameDim(network, tensorrt_in_tensors_[1], &indices_helper);
  ...
  ret = PreprocessInputs2SameDim(network, tensorrt_in_tensors_[INPUT_SIZE2], &updates_helper);
  if (ret != RET_OK || updates_helper.trt_tensor_ == nullptr) {
    MS_LOG(ERROR) << "PreprocessInputs2SameDim update tensor failed for " << op_name_;
    return ret;
  }

  nvinfer1::IScatterLayer *scatter_layer = network->addScatter(*scatter_input.trt_tensor_, *indices_helper.trt_tensor_,
                                                               *updates_helper.trt_tensor_, nvinfer1::ScatterMode::kND);
