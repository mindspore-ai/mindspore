...
class AscendNativeMatmulKernel : public AscendNativeBaseKernel {
 public:
  ...
  ~AscendNativeMatmulKernel() {
    if (tile_data_d_) ascend_native::FreeDevice(tile_data_d_);
    tile_data_d_ = nullptr;
    if (tile_data_h_) free(tile_data_h_);
    tile_data_h_ = nullptr;
    if (extra_d_) ascend_native::FreeDevice(extra_d_);
    extra_d_ = nullptr;
#ifdef ACL_BLAS
    if (alpha_) ascend_native::FreeDevice(alpha_);
    if (beta_) ascend_native::FreeDevice(beta_);
#endif
  }
