...
void PrepareMatmul(void **tile_data_d, void **tile_data_h, int m, int n, int k, bool ta, bool tb, bool bias, bool bmm,
  ...
  *tile_data_h = malloc(tile_size);
  ...
  int col = n;
  int deep = k;
  int cube_cors = ascend_native::getCubeNum();
  int core_num = (bmm) ? 1 : cube_cors;
  buildTiling(core_num, row, col, deep, ta, tb, bias, *tile_data_h, tile_size);
  *tile_data_d = ascend_native::MallocCopy(*tile_data_h, tile_size);
}

void PrepareMatmulExtra(void **tile_data_d, MMExtra *tile_data_h, void *stream, uint32_t bmm_num, uint32_t lda,
                        uint32_t ldb, uint32_t ldc) {
  int tile_size = sizeof(MMExtra);
  buildMMExtra(tile_data_h, bmm_num, lda, ldb, ldc);
  *tile_data_d = ascend_native::MallocCopy(reinterpret_cast<void *>(tile_data_h), tile_size);
}
