...
class KernelFuseNone {
 public:
  __aicore__ inline KernelFuseNone() = default;
...
class KernelFuseGelu {
 public:
  __aicore__ inline KernelFuseGelu() = default;
...
class KernelFuseMatMul {
 public:
  __aicore__ inline KernelFuseMatMul() = default;
  ...
  __aicore__ inline void Process() {
    ...
    for (uint64_t mmIdx = start_idx; mmIdx < bmm_num_; mmIdx += block_num_) {
      matmul_obj_.SetWorkspace(GetSysWorkSpacePtr(), SYS_WS_RESERVED);
