...
class AscendNativeBaseKernel : public BaseKernel {
 public:
  ...
  void set_alt_stream(const void *stream) { alt_stream_ = stream; }
  const void *get_stream() { return stream_; }
  const void *get_alt_stream() { return alt_stream_; }
  ...
  void set_mask_tensor_name(std::string mask_name) { mask_tensor_name_ = mask_name; }

 protected:
  const void *stream_ = nullptr;
  const void *alt_stream_ = nullptr;
  std::string name_;
