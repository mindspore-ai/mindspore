...
int Coder::Run(const void *model_buff, size_t size, const std::string &model_name, bool end_flag, bool enableFp16) {
  session_ = CreateCoderSession();
  if (session_ == nullptr) {
    MS_LOG(ERROR) << "new session failed while running!";
    return RET_ERROR;
  }
  STATUS status = session_->Init(model_buff, size, kModelIndex, end_flag, enableFp16);
  if (status != RET_OK) {
...
int Coder::MicroSourceCodeGeneration(const schema::MetaGraphT &graph, const std::string &output_path,
                                     ...
                                     bool debug_mode, bool end_flag, bool enableFp16) {
  ...
  status = code_gen.Run(builder.GetBufferPointer(), size, code_gen.model_name_, end_flag, enableFp16);
  if (status != RET_OK) {
