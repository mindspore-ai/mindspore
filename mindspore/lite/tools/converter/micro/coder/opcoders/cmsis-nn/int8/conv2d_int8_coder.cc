...
void Conv2DInt8Coder::CheckSupportOptimize() {
  if ((pad_x_ == 0) && (pad_y_ == 0) && (input_ch_ % kTensorChannelBaseLengthFour == 0) && (stride_x_ == 1) &&
      (stride_y_ == 1) && (kernel_x_ == 1) && (kernel_y_ == 1)) {
    opt_ = Convolve_1x1_fast;
    return;
  }

  if ((output_x_ == 1) && (input_x_ == 1) && (kernel_y_ == 1) && (output_x_ % kTensorChannelBaseLengthFour == 0) &&
      (input_batches_ == 1)) {
    opt_ = Convolve_1_x_n;
    return;
  }
