...
int DeConvolutionFP16Coder::InitWeightBias(CoderContext *const context) {
  ...
  if (input_tensors_.size() == kInputSize2) {
    ...
    MS_CHECK_RET_CODE(memcpy_s(packed_bias_, bias_data_size_, bias_tensor_, bias_tensor_->Size()),
...
int DeConvolutionFP16Coder::DoCode(CoderContext *const context) {
  ...
  for (int batch_index = 0; batch_index < conv_param_->input_batch_; batch_index++) {
    ...
    int cur_plan_rest = input_plane_ - kDefaultTaskId * plan_stride;
    int plan = MSMIN(plan_stride, cur_plan_rest);
    if (plan > 0) {
      input_ptr_ = allocator_->GetRuntimeAddr(input_tensor_) + " + " +
                   std::to_string(kDefaultTaskId * plan_stride * conv_param_->input_channel_);
      auto tmp_pack_in_str = allocator_->GetRuntimeAddr(static_cast<float16 *>(packed_input_)) + " + " +
                             std::to_string(kDefaultTaskId * plan_stride * conv_param_->input_channel_);
      code.CodeFunction("RowMajor2Col16MajorFp16Opt", input_ptr_, tmp_pack_in_str, plan, conv_param_->input_channel_);
    }
    ...
    if (current_oc > 0) {
      auto tmp_output_str =
        tmp_buffer_str + " + " + std::to_string(kDefaultTaskId * col_stride * matmul_param_.row_16_);
    ...
    output_ptr_ = allocator_->GetRuntimeAddr(output_tensor_) + " + " +
                  std::to_string(batch_index * output_plane_ * conv_param_->output_channel_);
