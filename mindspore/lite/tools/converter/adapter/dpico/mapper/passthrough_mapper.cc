...
namespace mindspore {
namespace dpico {
STATUS PassThroughMapper::Map(const CNodePtr &cnode, std::vector<BaseOperatorPtr> *base_operators,
  ...
  if (prim->GetAttr(kNumOutput) != nullptr) {
    pass_through_operator->SetPassThroughNumOutput(GetValue<uint32_t>(prim->GetAttr(kNumOutput)));
  }
  if (prim->GetAttr(kBlockHeight) != nullptr) {
    pass_through_operator->SetPassThroughBlockHeight(GetValue<uint32_t>(prim->GetAttr(kBlockHeight)));
  }
  if (prim->GetAttr(kBlockWidth) != nullptr) {
    pass_through_operator->SetPassThroughBlockWidth(GetValue<uint32_t>(prim->GetAttr(kBlockWidth)));
  }
