...
STATUS CaffePoolingParser::ParsePads(const caffe::PoolingParameter &poolingParam, std::vector<int64_t> *pad) {
  ...
  if (poolingParam.has_pad_h() && poolingParam.has_pad_w()) {
    ...
    (*pad)[dpico::kInputIndex2] = poolingParam.pad_w();
    (*pad)[dpico::kInputIndex3] = poolingParam.pad_w();
  } else {
    (*pad)[0] = poolingParam.pad();
    (*pad)[1] = poolingParam.pad();
    (*pad)[dpico::kInputIndex2] = poolingParam.pad();
    (*pad)[dpico::kInputIndex3] = poolingParam.pad();
  }
  return RET_OK;
}
...
BaseOperatorPtr CaffePoolingParser::Parse(const caffe::LayerParameter &proto, const caffe::LayerParameter &weight) {
  ...
  // parse kernel params
  std::vector<int64_t> windows(kNums2, 0);
  ...
  std::vector<int64_t> strides(kNums2, 0);
