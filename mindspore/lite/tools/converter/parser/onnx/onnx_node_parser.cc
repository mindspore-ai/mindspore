...
STATUS ExternalDataInfo::Create(const google::protobuf::RepeatedPtrField<onnx::StringStringEntryProto> &external_data,
  ...
  for (int i = 0; i != data_size; ++i) {
    ...
    if (string_map.key() == "location" && !string_map.value().empty()) {
      external_data_info->rel_path_ = string_map.value();
    } else if (string_map.key() == "offset" && !string_map.value().empty()) {
    ...
    } else if (string_map.key() == "length" && !string_map.value().empty()) {
    ...
    } else if (string_map.key() == "checksum" && !string_map.value().empty()) {
...
#ifdef _WIN32
  std::string external_data_file = external_tensor_dir + "\\" + external_data_info.GetRelPath();
#else
  std::string external_data_file = external_tensor_dir + "/" + external_data_info.GetRelPath();
#endif
