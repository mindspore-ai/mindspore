...
AnfNodePtr TfBidirectionGruFusion::GetCondGraphPattern(const PrimitiveVarMapPtr &primitive_vars) {
...
ParamValueLitePtr TfBidirectionGruFusion::GetDefaultParamValue(const AnfNodePtr &parameter_anf) {
...
STATUS TfBidirectionGruFusion::ConvertBiasData(const AnfNodePtr &gate_bias, const AnfNodePtr &cand_bias,
  ...
  // update_gate bias
  CopyFlattenMatData(gate_bias_data, 1, 0, 1, hidden_size, hidden_size * 2, tensor_data, false);
  // reset_gate bias
  CopyFlattenMatData(gate_bias_data, 1, 0, 1, 0, hidden_size, tensor_data + hidden_size, false);
  // hidden_gate bias
  CopyFlattenMatData(cand_bias_data, 1, 0, 1, 0, hidden_size, tensor_data + hidden_size * 2, false);
