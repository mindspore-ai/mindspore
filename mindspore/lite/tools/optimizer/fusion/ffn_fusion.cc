...
CNodePtr FFNFusion::CreateFFNFusionNode(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto param1 = utils::cast<AnfNodePtr>((*equiv)[matmul1_b_]);
  MS_CHECK_TRUE_RET(param1 != nullptr, nullptr);
  auto param2 = utils::cast<AnfNodePtr>((*equiv)[add1_x_]);
  MS_CHECK_TRUE_RET(param2 != nullptr, nullptr);
  auto param3 = utils::cast<AnfNodePtr>((*equiv)[matmul2_b_]);
...
bool FFNFusion::Init() const {
  ...
  div2_y_ = std::make_shared<Var>();
  MS_CHECK_TRUE_RET(div2_y_ != nullptr, false);
  add3_y_ = std::make_shared<Var>();
  MS_CHECK_TRUE_RET(add3_y_ != nullptr, false);
  mul4_y_ = std::make_shared<Var>();
  MS_CHECK_TRUE_RET(mul4_y_ != nullptr, false);
  matmul1_b_ = std::make_shared<Var>();
  MS_CHECK_TRUE_RET(matmul1_b_ != nullptr, false);
  add1_x_ = std::make_shared<Var>();
  MS_CHECK_TRUE_RET(add1_x_ != nullptr, false);
  matmul2_b_ = std::make_shared<Var>();
  MS_CHECK_TRUE_RET(matmul2_b_ != nullptr, false);
  return true;
}
...
const VectorRef FFNFusion::DefineFFNPatternForDynamicDims() const {
  ...
  if (!Init()) {
    MS_LOG(ERROR) << "FFNFusion Init Failed.";
    return {};
  }
  ...
  VectorRef div2_ref({is_div2, stridedslice2_ref, div2_y_});
  ...
  VectorRef add3_ref({is_add3, erf_ref, add3_y_});
  ...
  VectorRef mul4_ref({is_mul4, mul3_ref, mul4_y_});
  ...
  VectorRef matmul2_ref({is_matmul2, mul5_ref, matmul2_b_});
  return matmul2_ref;
}
...
bool FFNFusion::CheckPattern(const std::string &pattern_name, const EquivPtr &equiv) const {
  float div2_y = GetFloatParameterValue(equiv, div2_y_);
  if (div2_y < 0 || fabs(div2_y - DIV2_Y) > DIFF_THRESHOLD) {
    return false;
  }
  float add3_y = GetFloatParameterValue(equiv, add3_y_);
  if (add3_y < 0 || fabs(add3_y - ADD3_Y) > DIFF_THRESHOLD) {
    return false;
  }
  float mul4_y = GetFloatParameterValue(equiv, mul4_y_);
...
AnfNodePtr FFNFusion::Process(const std::string &pattern_name, const mindspore::FuncGraphPtr &func_graph,
  ...
  if (cnode == nullptr) {
    MS_LOG(ERROR) << "new FFN node failed.";
    return nullptr;
  }
