...
CNodePtr FlashAttentionFusion::CreatePadCNode(const FuncGraphPtr &func_graph, const AnfNodePtr &node, int32_t pad_size,
  ...
  AnfNodePtr paddings_node = BuildIntVec2DParameterNode(
    func_graph, paddings, node->fullname_with_scope() + std::to_string(g_name_index) + "_paddings");
  ...
  pad_cnode->set_fullname_with_scope(node->fullname_with_scope() + std::to_string(g_name_index++) + kNamePadNodeSuffix);
...
std::shared_ptr<FlashAttentionParm> FlashAttentionFusion::ParseFAParam() const {
  ...
  //  op_attrs=FlashAttention:input_layout:BSH;FlashAttention:seq_threshold:1024;FlashAttention:inner_precise:1;FlashAttention:sparse_mode:0
