...
int MatMulInferShape(const CNodePtr &cnode, const std::vector<ShapeVector> &in_shapes,
  ...
  a_pre_shape.insert(a_pre_shape.end(), out_rank - a_rank, 1);
  a_pre_shape.insert(a_pre_shape.end(), a_shape.begin(), a_shape.begin() + a_rank - C2NUM);
  ShapeVector b_pre_shape;
  b_pre_shape.insert(b_pre_shape.end(), out_rank - b_rank, 1);
  b_pre_shape.insert(b_pre_shape.end(), b_shape.begin(), b_shape.begin() + b_rank - C2NUM);
  ShapeVector out_shape;
...
int ReduceInferShape(const CNodePtr &cnode, const std::vector<ShapeVector> &in_shapes,
  ...
  if (data_info.data_ptr_ == nullptr) {
    ...
    if (data_info.data_type_ == kNumberTypeInt || data_info.data_type_ == kNumberTypeInt32) {
      ...
      temp.insert(temp.begin(), static_cast<int64_t *>(data_info.data_ptr_),
...
int StridedSliceInferShape(const CNodePtr &cnode, const std::vector<ShapeVector> &in_shapes,
  ...
  out_shape.insert(out_shape.end(), in_shape.begin() + index, in_shape.end());
  out_shapes->clear();
  out_shapes->push_back(out_shape);
  return lite::RET_OK;
}
