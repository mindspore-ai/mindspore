...
CNodePtr InputDTypeTransPass::GenCastNode(const FuncGraphPtr &graph, const AnfNodePtr &input_node,
  ...
  auto param_node =
    opt::BuildIntValueParameterNode(graph, static_cast<int32_t>(dst_input_data_type_), cnode_name + "_type");
...
STATUS InputDTypeTransPass::HandleGraphInput(const FuncGraphPtr &graph) {
  ...
  for (const auto &input : graph_inputs) {
    ...
    if (src_input_data_type_ == DataType::kNumberTypeInt64 &&
        (input_data_type == TypeId::kNumberTypeInt32 || input_data_type == TypeId::kNumberTypeInt ||
         input_data_type == TypeId::kNumberTypeInt64)) {
      ...
      element->set_type(TypeIdToType(static_cast<TypeId>(src_input_data_type_)));
...
bool InputDTypeTransPass::Run(const FuncGraphPtr &graph) {
  if (user_set_data_type_ != src_input_data_type_) {
    return true;
  }
