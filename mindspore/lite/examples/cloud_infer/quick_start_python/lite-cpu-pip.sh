...
#
# Usage:
#   Run script like `MINDSPORE_LITE_VERSION=2.0.0 bash ./lite-cpu-pip.sh`.
...
if [ -f "'echo ${BASEPATH}/mindspore_lite*.whl'" ]; then
  ...
  python -m pip uninstall -y mindspore_lite
  python -m pip install https://ms-release.obs.cn-north-4.myhuaweicloud.com/${MINDSPORE_LITE_VERSION}/MindSpore/lite/release/linux/x86_64/cloud_fusion/mindspore_lite-${MINDSPORE_LITE_VERSION/-/}-cp37-cp37m-linux_${arch}.whl --trusted-host ms-release.obs.cn-north-4.myhuaweicloud.com -i https://pypi.tuna.tsinghua.edu.cn/simple
fi
