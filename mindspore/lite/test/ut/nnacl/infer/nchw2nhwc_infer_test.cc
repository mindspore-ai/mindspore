...
#include "mindspore/lite/nnacl/infer/nchw2nhwc_infer.h"

namespace mindspore {

class Nchw2NhwcInferTest : public mindspore::CommonTest {
 public:
  Nchw2NhwcInferTest() {}
};

TEST_F(Nchw2NhwcInferTest, Nchw2NhwcInferTest0) {
  ...
  int ret = Nchw2NhwcInferShape((const TensorC **)inputs.data(), inputs.size(), outputs.data(), outputs.size(),
...
TEST_F(Nchw2NhwcInferTest, Nchw2NhwcInferTest1) {
  size_t inputs_size = 1;
  std::vector<TensorC *> inputs(inputs_size, NULL);
  inputs[0] = new TensorC;
