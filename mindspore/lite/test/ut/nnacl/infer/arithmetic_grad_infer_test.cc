...
#include "mindspore/lite/nnacl/infer/arithmetic_grad_infer.h"

namespace mindspore {

class ArithmeticGradGradInferTest : public mindspore::CommonTest {
 public:
  ArithmeticGradGradInferTest() {}
};

TEST_F(ArithmeticGradGradInferTest, ArithmeticGradGradInferTest0) {
  ...
  ArithmeticGradParameter *parameter = new ArithmeticGradParameter;
  parameter->op_parameter_.infer_flag_ = true;
  parameter->type_ = PrimitiveType_MaximumGrad;
  int ret = ArithmeticGradInferShape((const TensorC **)inputs.data(), inputs.size(), outputs.data(), outputs.size(),
  ...
  // ASSERT_EQ(outputs[1]->format_, Format_NHWC); //maybe you should refer to the maximum_grad
