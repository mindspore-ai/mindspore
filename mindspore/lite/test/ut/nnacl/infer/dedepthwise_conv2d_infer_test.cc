...
#include "mindspore/lite/nnacl/infer/dedepthwise_conv2d_infer.h"

namespace mindspore {

class DeDepthwiseConv2DInferTest : public mindspore::CommonTest {
 public:
  DeDepthwiseConv2DInferTest() {}
};

TEST_F(DeDepthwiseConv2DInferTest, DeDepthwiseConv2DInferTest0) {
  ...
  inputs[1]->shape_[1] = 3;
  inputs[1]->shape_[2] = 3;
  inputs[1]->shape_[3] = 1;  // must be 1, because it is channel_multiplier
...
TEST_F(DeDepthwiseConv2DInferTest, DeDepthwiseConv2DInferTest1) {
  size_t inputs_size = 2;
  std::vector<TensorC *> inputs(inputs_size, NULL);
  inputs[0] = new TensorC;
  inputs[0]->shape_size_ = 4;
...
TEST_F(DeDepthwiseConv2DInferTest, DeDepthwiseConv2DInferTest2) {
  size_t inputs_size = 2;
  std::vector<TensorC *> inputs(inputs_size, NULL);
  inputs[0] = new TensorC;
  inputs[0]->shape_size_ = 4;
