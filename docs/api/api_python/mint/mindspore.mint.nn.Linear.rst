...
.. py:class:: mindspore.mint.nn.Linear(in_channels, out_channels, weight_init=None, bias_init=None, has_bias=True, dtype=mstype.float32)
    ...
    参数：
        - **in_channels** (int) - Linear层输入Tensor的空间维度。
        - **out_channels** (int) - Linear层输出Tensor的空间维度。
