...
.. py:function:: mindspore.mint.unique(input, sorted=True, return_inverse=False, return_counts=False, dim=None)
    ...
    参数：
        ...
        - **sorted** (bool) - 输出是否需要进行降序排序。默认值：``True`` 。
    ...
    返回：
        ...
        - **inverse_indeices** (Tensor) - 当`return_inverse=True`时返回，表示输入Tensor中的元素在输出Tensor中的索引。当`dim='None'`时，shape和input一样；当dim有值的时候，shape是`input.shape[dim]`。
        - **counts** (Tensor) - 当`return_counts=True`时返回，表示输出Tensor中元素在输入Tensor中的数量。当`dim='None'`时，shape和output一样；当dim有值的时候，shape是`output.shape[dim]`。
