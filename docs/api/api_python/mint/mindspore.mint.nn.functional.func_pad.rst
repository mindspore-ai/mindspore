...
================================
.. py:function:: mindspore.mint.nn.functional.pad(input, pad, mode='constant', value=0.0)
    ...
    参数：
        ...
        - **value** (Union[int, float, None]，可选) - 仅在 ``'constant'`` 模式下生效，设置在 ``'constant'`` 模式下的填充值，如果值为 ``None`` ，则会使用0作为默认填充值。默认值： ``0.0`` 。
