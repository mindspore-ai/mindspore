...
.. py:class:: mindspore.rewrite.SymbolTree(handler: SymbolTreeImpl)
    ...
    .. py:method:: mindspore.rewrite.SymbolTree.create_call_function(func, targets, *args, **kwargs)
        ...
        参数：
            ...
            - **args** (Union[MsDtypes, ParamTypes]) - 该节点的参数名称。用作源代码中代码语句的参数。当没有提供值时，表示 `cell` 没有参数输入。
            - **kwargs** (dict{str, Union[MsDtypes, ParamTypes]}) - 键的类型必须是str，值必须是MsDtypes或类型必须是ParamTypes。用来说明带有关键字的形参的输入参数名称。输入名称在源代码中作为语句表达式中的 `kwargs`。当没有提供值时，表示没有 `kwargs` 输入。
