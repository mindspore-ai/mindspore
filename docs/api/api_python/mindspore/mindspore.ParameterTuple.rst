    ...
    .. py:method:: clone(prefix, init='same')
        ...
        参数：
            ...
            - **init** (Union[Tensor, str, numbers.Number]) - 对Parametertuple中Parameter的shape和类型进行克隆，并根据传入的 `init` 设置数值。默认值： ``'same'`` 。
              ...
              - 如果 `init` 是 `str` ，则按照 `Initializer` 模块中对应的同名的初始化方法进行数值设定。若为 ``'same'``，则新参数的数值与原Parameter相同。
        ...
        教程样例：
            - `Cell与参数 - Parameter Tuple
              <https://mindspore.cn/tutorials/zh-CN/r2.3.0rc1/advanced/modules/layer.html#parameter-tuple>`_