...
.. py:function:: mindspore.vmap(fn, in_axes=0, out_axes=0)
    ...
    Vmap由Jax率先提出，它消除了算子对batch维度的限制，并提供更加方便、统一的运算符表达。同时，用户还可以与 :func:`mindspore.grad` 等其它功能模块组合使用，提高开发效率，详情请参见教程 `自动向量化Vmap <https://www.mindspore.cn/tutorials/experts/zh-CN/r2.3.0rc1/vmap/vmap.html>`_ 。
    ...
    .. note::
        - vmap的能力来自于原语的VmapRules实现。虽然我们为用户自定义算子设计了一个通用规则，但是我们无法保证它对所有算子都能很好地工作，可能出现未知异常，用户需知晓使用风险。
