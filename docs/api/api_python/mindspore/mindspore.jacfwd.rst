...
.. py:function:: mindspore.jacfwd(fn, grad_position=0, has_aux=False)
    ...
    通过前向模式计算给定网络的雅可比矩阵，对应 `前向模式自动微分 <https://www.mindspore.cn/docs/zh-CN/master/design/programming_paradigm.html#前向自动微分>`_。当网络输出数量远大于输入数量时，使用前向模式求雅可比矩阵比反向模式性能更好。
    ...
    返回：
        Function，用于计算给定函数的雅可比矩阵。例如 `out1, out2 = fn(*args)` ，若 `has_aux` 为 ``True`` ，梯度函数将返回 `(Jacobian, out2)` 形式的结果，其中 `out2` 不参与求导，若为 ``False`` ，将直接返回 `Jacobian` 。
