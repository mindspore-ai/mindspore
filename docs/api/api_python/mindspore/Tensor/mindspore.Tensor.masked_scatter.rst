...
.. py:method:: mindspore.Tensor.masked_scatter(mask, tensor)
    ...
    参数：
        ...
        - **tensor** (Tensor) - 一个Tensor, 其数据类型与Tensor本身相同。 `tensor` 中的元素数量必须大于等于 `mask` 中的True元素
          的数量。
    ...
    异常：
        - **TypeError** - 如果 `mask` 或者 `tensor` 不是Tensor。
        ...
        - **ValueError** - 如果 `tensor` 中的元素数目小于Tensor本身需要更新的元素数目。
