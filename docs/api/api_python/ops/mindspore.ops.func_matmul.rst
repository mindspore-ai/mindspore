...
.. py:function:: mindspore.ops.matmul(input, other)
    ...
    .. note::
        不支持NumPy参数 `out` 、 `casting` 、 `order` 、 `subok` 、 `signature` 、 `extobj` 。在GPU上支持的数据类型为np.float16和np.float32。在CPU上支持的数据类型为np.float16和np.float32。 `input` 和 `other` 的数据类型必须一致。在Ascend平台上，`input` 和 `other` 的秩必须在 1 到 6 之间。
