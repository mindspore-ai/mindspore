...
.. py:class:: mindspore.ops.MaxUnpool3D(ksize, strides=0, pads=0, output_shape=(), data_format="NCDHW")
    ...
    输出数据的个格式为 :math:`(N, C, D_{out}, H_{out}, W_{out})` ，计算公式如下：
    ...
    参数：
        ...
        - **output_shape** (tuple[int]，可选) - 一个可选的输入，指定目标输出的尺寸。默认值：()。
          ...
          - 如果 :math:`output\_shape != ()` ，则 `output_shape` 必须为 :math:`(N, C, D, H, W)` 或 :math:`(N, D, H, W, C)` ，
    ...
    异常：
        ...
        - **ValueError** - 如果 `strides` (也支持0和(0, 0, 0)) 或 `ksize` 的值不是正数。
        - **ValueError** - 如果 `pads` 的值是负数。
        - **ValueError** - 如果  `ksize` 、 `strides` 或 `pads` 不是长度为3的tuple。
        - **ValueError** - 如果 `data_format` 不是str，同时也不是 `NCDHW` 或 `NDHWC` 。
