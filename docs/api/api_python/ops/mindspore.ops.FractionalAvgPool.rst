...
.. py:class:: mindspore.ops.FractionalAvgPool(pooling_ratio, pseudo_random=False, overlapping=False, deterministic=False, seed=0, seed2=0)
    ...
    参数：
        ...
        - **overlapping** (bool，可选) - 当设置为 ``True`` 时，表示两个单元格都使用相邻池化单元边界的值进行池化，
          设置为False时，表示值不进行重复使用。默认值： ``False`` 。
        - **deterministic** (bool，可选) - 当设置为 ``True`` 时，将在计算图中的FractionalAvgPool节点上进行迭代时使用固定池区域。
          主要用于单元测试，使FractionalAvgPool具有确定性。当设置为 ``False`` 时，将不使用固定池区域。默认值： ``False`` 。
