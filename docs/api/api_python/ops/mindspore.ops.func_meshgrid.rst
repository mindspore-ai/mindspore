...
.. py:function:: mindspore.ops.meshgrid(inputs, indexing='xy')
    ...
    **参数：**
    ...
    - **indexing** ('xy', 'ij', optional) - 'xy'或'ij'。影响输出的网格矩阵的size。以长度为 `M` 和 `N` 的二维输入，取值为'xy'时，输出的shape为 :math:`(N, M)` ，取值为'ij'时，输出的shape为 :math:`(M, N)` 。以长度为 `M` , `N` 和 `P` 的三维输入，取值为'xy'时，输出的shape为 :math:`(N, M, P)` ，取值为'ij'时，输出的shape为 :math:`(M, N, P)` 。默认值：'xy'。
