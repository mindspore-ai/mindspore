...
.. py:function:: mindspore.ops.amax(input, axis=None, keepdims=False, *, initial=None, where=None)
    ...
    关键字参数：
        ...
        - **where** (Tensor[bool], 可选) - 表示是否需要将 `input` 中的原始值替换为 `initial` 值的Tensor。若为True则不替换，若为False则替换。`where` 中为False的位置，必须提供对应的 `initial` 值。默认值： ``None`` ，表示 ``True`` 。

    返回：
        Tensor。

        - 如果 `axis` 为None，且 `keepdims` 为 ``False`` ，则输出一个零维Tensor，表示输入Tensor中所有元素的最大值。
