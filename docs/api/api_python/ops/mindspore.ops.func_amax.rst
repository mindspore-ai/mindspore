...
.. py:function:: mindspore.ops.amax(input, axis=None, keep_dims=False)

    默认情况下，移除输入所有维度，返回 `input` 中的最大值。也可仅缩小指定维度 `axis` 大小至1。 `keep_dims` 控制输出和输入的维度是否相同。
    ...
    返回：
        ...
        - 如果 `axis` 为None，且 `keep_dims` 为False，则输出一个零维Tensor，表示输入Tensor中所有元素的最大值。
