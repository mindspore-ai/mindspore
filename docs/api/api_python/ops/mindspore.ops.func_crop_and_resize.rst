...
.. py:function:: mindspore.ops.crop_and_resize(x, boxes, box_indices, crop_size, method="bilinear", extrapolation_value=0.0)
    ...
    参数：
        - **x** (Tensor) - shape为 :math:`(batch, image_height, image_width, depth)` 的图像Tensor。数据类型：int8, int16, int32, int64, float16, float32, float64, uint8, uint16。
    ...
    异常：
        - **TypeError** - `x`、 `boxes` 或 `box_indices` 不是Tensor。
