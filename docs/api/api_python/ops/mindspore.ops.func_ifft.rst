...
.. py:function:: mindspore.ops.ifft(input, n=None, dim=-1, norm=None)
    ...
    参数：
        ...
        - **n** (int, 可选) - 输出在 `dim` 轴的长度。如果给定，则在计算 `ifft` 之前 `dim` 轴的大小将被零填充或截断至 `n`。
          默认值： ``None`` , 表示无需对 `input` 进行处理。
        - **dim** (int, 可选) - 进行一维 `ifft` 的维度。默认值： ``-1`` ，表示对 `input` 的最后一个维度进行变换。
    ...
    返回： 
        Tensor， `ifft` 的结果。默认与 `input` 同形状，如果给定 `n` ，则 `dim` 轴的大小改为 `n` 。
