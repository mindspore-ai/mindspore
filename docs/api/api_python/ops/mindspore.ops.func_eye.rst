...
.. py:function:: mindspore.ops.eye(n, m=None, dtype=None)
    ...
    参数：
        ...
        - **dtype** (mindspore.dtype) - 指定返回Tensor的数据类型。数据类型必须是 `bool_ <https://www.mindspore.cn/docs/zh-CN/r2.1/api_python/mindspore/mindspore.dtype.html#mindspore.dtype>`_ 或 `number <https://www.mindspore.cn/docs/zh-CN/r2.1/api_python/mindspore/mindspore.dtype.html#mindspore.dtype>`_ 。默认值为 ``None`` ，返回Tensor的数据类型默认为mindspore.float32。
