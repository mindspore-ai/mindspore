...
.. py:function:: mindspore.ops.gather(input_params, input_indices, axis, batch_dims=0)
    ...
    .. note::
        1. input_indices的值必须在 `[0, input_param.shape[axis])` 范围内。CPU与GPU平台越界访问将会抛出异常，Ascend平台越界访问的返回结果是未定义的。
        2. Ascend平台上，input_params的数据类型当前不能是 `bool_ <https://www.mindspore.cn/docs/zh-CN/r2.2/api_python/mindspore/mindspore.dtype.html#mindspore.dtype>`_ 。
    ...
    异常：
        ...
        - **RuntimeError** - `input_indices` 在CPU或GPU平台超出 `[0, input_param.shape[axis])` 范围。
