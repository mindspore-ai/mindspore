...
.. py:class:: mindspore.ops.ScatterNdSub(use_locking=False)
    ...
    **输入：**
    ...
    - **indices** (Tensor) - 指定减法操作的索引，数据类型为int32。索引的rank必须至少为2，并且 `indices_shape[-1] <= len(shape)` 。
    - **updates** (Tensor) - 指定与 `input_x` 相减操作的Tensor，数据类型与输入相同。shape为 `indices_shape[:-1] + x_shape[indices_shape[-1]:]` 。
