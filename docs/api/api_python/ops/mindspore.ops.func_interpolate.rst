...
.. py:function:: mindspore.ops.interpolate(x, roi=None, scales=None, sizes=None, coordinate_transformation_mode="align_corners", mode="bilinear")
    ...
    **参数：**
    ...
    - **x** (Tensor) - 输入Tensor。当 `mode` 是"bilinear"时， `x` 为4维Tensor。
    ...
    - **sizes** (tuple[int]， 可选) - 输入shape指定轴的新维度。 `sizes` 中的数全是正数。 `scales` 和 `sizes` 同时只能指定一个。当 `mode` 是"bilinear"时， `sizes` 为2个int元素 :math:`(new\_height, new\_width)` 的tuple。
    ...
    - **mode** (str) - 所使用的插值方式。 目前仅支持"bilinear"插值方式。
