...
.. py:function:: mindspore.ops.pow(input, exponent)
    ...
    .. note::
        - 输入 `input` 和 `exponent` 遵循 `隐式类型转换规则 <https://www.mindspore.cn/docs/zh-CN/r2.0/note/operator_list_implicit.html>`_ ，使数据类型保持一致。
        - 输入必须是两个Tensor，或一个Tensor和一个Scalar。
        - 当输入是两个Tensor时，它们的数据类型不能同时是bool，并保证其shape可以广播。
        - 当输入是一个Tensor和一个Scalar时，Scalar只能是一个常数。

    参数：
        - **input** (Union[Tensor, number.Number, bool]) - 第一个输入，是一个number.Number、bool值或数据类型为 `number <https://www.mindspore.cn/docs/zh-CN/r2.0/api_python/mindspore.html#mindspore.dtype>`_ 或 `bool_ <https://www.mindspore.cn/docs/zh-CN/r2.0/api_python/mindspore.html#mindspore.dtype>`_ 的Tensor。
