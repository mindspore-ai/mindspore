...
.. py:function:: mindspore.ops.rfftn(input, s=None, dim=None, norm=None)

    计算实值输入的 `N` 维实值快速傅里叶变换。

    .. note::
        - `rfftn` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `rfftn` 尚不支持Windows平台。
    ...
    参数：
        ...
        - **s** (tuple[int], 可选) - 信号长度。如果给定，则在计算 `rfftn` 之前 `dim[i]` 轴的大小将被零填充或截断至 `s[i]`。
          默认值： ``None`` , 表示无需对 `input` 进行处理。
        - **dim** (tuple[int], 可选) - 指定进行 `rfftn` 的维度。默认值： ``None`` ，表示对 `input` 的最后所有维度进行变换。
