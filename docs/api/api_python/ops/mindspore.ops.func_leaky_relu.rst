...
.. py:function:: mindspore.ops.leaky_relu(x, alpha=0.2)

   leaky_relu激活函数。 `x` 中小于0的元素乘以 `alpha` 。
   ...
   参数：
       - **x** (Tensor) - 计算leaky_relu的任意维度的Tensor。
       - **alpha** (Union[int, float]) - `x` 的元素小于0时激活函数的斜率，默认值：0.2。
