...
.. py:function:: mindspore.ops.extend.min(input, dim=None, keepdim=False)
    ...
    返回：
        如果 `dim` 为默认值 ``None`` 返回值为Tensor，其值为输入Tensor的最小值，其shape为 :math:`()` ， 其数据类型与 `input` 相同。

        如果 `dim` 不为默认值 ``None`` 返回值为tuple (Tensor)，表示2个Tensor组成的tuple，包含输入Tensor在给定轴上的最小值和对应的索引：
    ...
    异常：
        - **ValueError** - 如果 `dim` 为默认值 ``None`` ，但 `keepdim` 不为 ``False`` 。
