...
.. py:function:: mindspore.ops.adaptive_max_pool3d(x, output_size, return_indices=False)
    ...
    **参数：**
    ...
    - **output_size** (Union[int, tuple]) - 表示输出特征图的尺寸，可以是tuple math:`(D, H, W)`，也可以是单个数D表示输出尺寸为 math:`(D, D, D)` 。:math:`D` ， :math:`H` 和 :math:`W` 可以是int型整数或者None，其中None表示输出大小与对应的输入的大小相同。
    ...
    **异常：**
    ...
    - **ValueError** - `output_size` 的shape不是(3,)。
