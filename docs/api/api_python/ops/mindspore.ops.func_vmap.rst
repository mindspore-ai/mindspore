...
.. py:function:: mindspore.ops.vmap(fn, in_axes=0, out_axes=0)
    ...
    .. note::
        - vmap的能力来自于原语的VmapRules实现。虽然我们为用户自定义算子设计了一个通用规则，但是我们无法保证它对所有算子都能很好地工作，用户需知晓
          使用风险。如果想要获得更好的性能，请提前花一些时间参阅教程为自定义算子实现特定的VmapRule。
        - 当在vmap作用域内调用随机数生成方法时，每次在向量函数之间生成相同的随机数。如果希望每个向量分支使用不同的随机数，您需要提前从外部生成
          一批随机数，然后将其传入vmap。
    ...
    **返回：**
    ...
    Function，返回 `fn` 的自动向量化/批处理后的函数。此函数的输入参数和输出与 `fn` 的相对应，但它在 `in_axes` 和 `out_axes` 指定的位置新增了额外的批处理维度。

    **异常：**

    - **RuntimeError** - 如果 `in_axes` 或 `out_axes` 中的基本元素不是None或整数。
