...
.. py:function:: mindspore.ops.irfft(input, n=None, dim=-1, norm=None)

    计算 `rfft()` 的逆变换。

    .. note::
        - `irfft` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `irfft` 尚不支持Windows平台。

    参数：
        - **input** (Tensor) - 输入的Tensor。
        - **n** (int, 可选) - 指定轴 `dim` 转换后的长度。如果给定 `n` ，输入会被截断或补零至 `dim` 轴大小为 `n` 。
          默认值： ``None`` ，默认为 :math:`2*(input.shape[dim]-1)` 。
