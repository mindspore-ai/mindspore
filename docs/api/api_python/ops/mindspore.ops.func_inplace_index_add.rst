...
.. py:function:: mindspore.ops.inplace_index_add(var, indices, updates, axis)
    ...
    将 `updates` 添加到 `var` 的指定轴和索引处。

    参数：
        - **x** (Parameter) - 被添加的输入Parameter，数据类型为uint8、int8、int16、int32、float16、float32或float64。
        - **indices** (Tensor) - `axis` 指定轴上执行加法的索引。是一个1D Tensor，shape为 :math:`(updates.shape[axis],)` ，它的每个值应在范围 :math:`[0, var.shape[axis])` 之内，数据类型为 int32。
        - **updates** (Tensor) - 要添加的输入Tensor。必须与 `var` 具有相同的数据类型。 除 `axis` 维度外， `updates` 与 `var` 的shape应一致。
        - **axis** (int) - 要索引的维度。应该在范围 :math:`[0, len(var.dim))` 内。
