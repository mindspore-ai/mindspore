...
.. py:class:: mindspore.ops.SparseTensorDenseMatmul(adjoint_st=False, adjoint_dt=False)
    ...
    输入：
        ...
        - **dense** (Tensor) - 二维Tensor，数据类型与 `values` 相同。
          ...
          - 如果 `adjoint_st` 为 ``False`` ， `adjoint_dt` 为 ``False`` ，则shape必须为 :math:`(C, M)` 。
          - 如果 `adjoint_st` 为 ``False`` ， `adjoint_dt` 为 ``True`` ，则shape必须为 :math:`(M, C)` 。
          - 如果 `adjoint_st` 为 ``True`` ， `adjoint_dt` 为 ``False`` ，则shape必须为 :math:`(N, M)` 。
          - 如果 `adjoint_st` 为 ``True`` ， `adjoint_dt` 为 ``True`` ，则shape必须为 :math:`(M, N)` 。

    输出：
        Tensor，数据类型与 `values` 相同。
        如果 `adjoint_st` 为 ``False`` ，则shape为 :math:`(N, M)` 。
