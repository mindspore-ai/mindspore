...
.. py:function:: mindspore.ops.floor_div(x, y)

    将第一个输入Tensor除以第二个输入Tensor，并向下取整。

    `x` 和 `y` 的输入遵循隐式类型转换规则，使数据类型一致。输入必须是两个Tensor或一个Tensor和一个Scalar。当输入是两个Tensor时，它们的数据类型不能同时为bool，其shape可以广播。当输入是一个Tensor和一个Scalar时，Scalar只能是一个常量。
