...
.. py:function:: mindspore.ops.prod(input, axis=None, keep_dims=False)
    ...
    返回：
        ...
        - 如果 `axis` 为int，取值为1，并且 `keep_dims` 为False，则输出的shape为 :math:`(input_0, input_2, ..., input_R)` 。
        - 如果 `axis` 为tuple(int)或list(int)，取值为(1, 2)，并且 `keep_dims` 为False，则输出Tensor的shape为 :math:`(input_0, input_3, ..., input_R)` 。
