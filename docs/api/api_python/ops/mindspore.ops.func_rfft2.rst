...
.. py:function:: mindspore.ops.rfft2(input, s=None, dim=(-2,-1), norm=None)

    计算实值输入的二维实值快速傅里叶变换。

    .. note::
        - `rfft2` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `rfft2` 尚不支持Windows平台。
    ...
    参数：
        ...
        - **s** (tuple[int], 可选) - 信号长度。如果给定，则在计算 `rfft2` 之前 `dim[i]` 轴的大小将被零填充或截断至 `s[i]`。
          默认值： ``None`` , 表示无需对 `input` 进行处理。
        - **dim** (tuple[int], 可选) - 指定进行 `rfft2` 的维度。默认值： ``(-2,-1)`` ，表示对 `input` 的最后两个维度进行变换。
    ...
    返回：
        Tensor，二维快速傅里叶变换的结果。默认与 `input` 同shape，如果给定 `s`，则 `dim[-1]` 的大小为 :math:`s[-1] // 2 + 1` ，
        其余轴 `dim[i]` 的大小改为 `s[i]` 。
        当输入为 int16、int32、int64、float16、float32 时，返回值类型为complex64。
        当输入为 float64 时，返回值类型为complex128。
