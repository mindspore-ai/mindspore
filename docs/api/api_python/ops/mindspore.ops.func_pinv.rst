...
.. py:function:: mindspore.ops.pinv(x, *, atol=None, rtol=None, hermitian=False)
    ...
    若atol或rtol为Tensor，则其shape必须可广播到 `x.svd() <https://www.mindspore.cn/docs/zh-CN/r2.3.0rc1/api_python/ops/mindspore.ops.svd.html>`_ 返回的奇异值的shape。
    ...
    若rtol和atol均未指定并且x的shape(M, N)，则rtol设置为 :math:`rtol=max(M, N)*\varepsilon` ， :math:`\varepsilon` 为x.dtype的 `eps值 <https://www.mindspore.cn/docs/zh-CN/r2.3.0rc1/api_python/ops/mindspore.ops.Eps.html>`_ 。
    若rtol未指定且atol指定值大于零，则rtol设置为零。

    .. note::
        该函数在内部使用 `svd <https://www.mindspore.cn/docs/zh-CN/r2.3.0rc1/api_python/ops/mindspore.ops.svd.html>`_
        （或 `eigh <https://www.mindspore.cn/docs/zh-CN/r2.3.0rc1/api_python/scipy/mindspore.scipy.linalg.eigh.html>`_ , `hermitian=True` ），
