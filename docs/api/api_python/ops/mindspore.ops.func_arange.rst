...
.. py:function:: mindspore.ops.arange(start=0, end=None, step=1, *, dtype=None)
    ...
    参数：
        - **start** (Union[float, int, Tensor], 可选) - 序列的起始数。如果为Tensor，则shape必须为()。默认值：0。
        - **end** (Union[float, int, Tensor], 可选) - 序列的终止数，不包含在序列中。如果为Tensor，则shape必须为()。默认值：None。如果为None，则默认为 `start` 的值，同时将0作为范围起始值。
        - **step** (Union[float, int, Tensor], 可选) - 表述序列中数值的步长。如果为Tensor，则shape必须为()。默认值：1。

    关键字参数：
        - **dtype** (mindspore.dtype, 可选) - 返回Tensor的所需数据类型。默认值：None。如果未指定或者为None，则使用 `start` 、 `end` 和 `step` 参数中精度最高的类型。
