...
.. py:function:: mindspore.ops.fftn(input, s=None, dim=None, norm=None)

    计算输入的 `N` 维快速傅里叶变换。

    .. note::
        - `fftn` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `fftn` 尚不支持Windows平台。
    ...
    参数：
        ...
        - **s** (tuple[int], 可选) - 信号长度。如果给定，则在计算 `fftn` 之前 `dim[i]` 轴的大小将被零填充或截断至 `s[i]`。
