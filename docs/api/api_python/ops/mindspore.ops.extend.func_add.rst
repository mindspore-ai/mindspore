...
.. py:function:: mindspore.ops.extend.add(input, other, alpha=1)
    ...
    .. note::
        - 当两个输入shape不同时，
          它们必须能够广播到一个共同的shape。
        - 两个输入和 `alpha` 不能同时为 bool 类型，
          [True, Tensor(True, bool\_), Tensor(np.array([True]), bool\_)] 都被视为 bool 类型。
    ...
    异常：
        - **TypeError** - 如果 `input`、`other` 或 `alpha` 不是以下之一：Tensor, number.Number, bool。
