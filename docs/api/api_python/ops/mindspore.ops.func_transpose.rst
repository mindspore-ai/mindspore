...
.. py:function:: mindspore.ops.transpose(input, input_perm)
    ...
    参数：
        - **input** (Tensor) - 输入Tensor，其shape是 :math:`(x_1, x_2, ..., x_R)` 。
        - **input_perm** (tuple[int]) - 指定排列。 `input_perm` 中的元素由 `input` 的每个维度的索引组成。 `input_perm` 的长度和 `input` 的shape相同。只支持常量值。其范围在[-rank(input), rank(input))内。

    返回：
        Tensor，输出Tensor的数据类型与 `input` 相同，输出Tensor的shape由 `input` 的shape和 `input_perm` 的值决定。

    异常：
        - **TypeError** - `input_perm` 不是tuple。
        - **ValueError** - `input` 的shape长度不等于 `input_perm` 的shape长度。
