...
.. py:class:: mindspore.ops.GradOperation(get_all=False, get_by_list=False, sens_param=False)
    ...
    例如构建一个以x和y为输入的函数 `net = Net()` ，并带有一个参数z，详见样例中的 `Net` 。

    - 用于对输入求导：

      1. 返回第一个输入的梯度（见样例中的 `GradNetWrtX` ）：
      ...
      2. 返回所有输入的梯度（见样例中的 `GradNetWrtXY` ）：
    ...
    - 用于对参数求导：
      ...
      返回给定参数的梯度（见样例中的 `GradNetWithWrtParams` ）：
    ...
    - 用于同时对输入和参数求导：
      ...
      以((dx, dy), (dz))的格式返回关于所有输入和给定参数的梯度（见样例中的 `GradNetWrtInputsAndParams` ）：
    ...
    .. note::
        对于上面产生的梯度函数，其返回值会因返回梯度的数量不同而出现差异：
    ...
    参数：
        - **get_all** (bool) - 计算梯度，如果等于 ``False`` ，获得第一个输入的梯度，如果等于 ``True`` ，获得所有输入的梯度。默认值： ``False`` 。
        - **get_by_list** (bool) - 如果 `get_all` 和 `get_by_list` 都等于 ``False`` ，则得到第一个输入的梯度。如果等于 ``True`` ，获得所有Parameter自由变量的梯度。如果 `get_all` 和 `get_by_list` 都等于 ``True`` ，则同时得到输入和Parameter自由变量的梯度，输出形式为（“所有输入的梯度”，“所有Parameter自由变量的梯度”）。默认值： ``False`` 。
        - **sens_param** (bool) - 是否在输入中配置灵敏度（关于输出的梯度）。如果 `sens_param` 等于 ``False`` ，自动添加一个 `ones_like(output)` 灵敏度。如果 `sensor_param` 等于 ``True`` ，灵敏度（输出的梯度），必须通过位置参数或键值对参数来传递，如果是通过键值对参数传递value，那么key必须为sens。默认值： ``False`` 。

    返回：
        将一个函数作为参数，并返回梯度函数的高阶函数。

    异常：
        - **TypeError** - 如果 `get_all` 、 `get_by_list` 或者 `sens_param` 不是bool。
