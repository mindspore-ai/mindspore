...
.. py:function:: mindspore.ops.trapz(y, x=None, *, dx=None, dim=-1)
    ...
    参数：
        - **y** (Tensor) - 要积分的Tensor。
        - **x** (Tensor，可选) - 对应于 `y` 值的样本点。如果 `x` 为 ``None`` ，则采样点均匀间隔 `dx` ，默认值： ``None`` 。如果 `x` 不为 ``None`` ，则由 `dim` 指定的轴减去1后， `x` 的shape应与 `y` 相同或者可以广播到 `y` 。

    关键字参数：
        - **dx** (float，可选) - 当 `x` 为 ``None`` 时，采样点之间的间距。如果 `x` 被指定，则 `dx` 不生效。如果为 ``None`` ，取值视为 ``1.0``。 默认值： ``None`` 。
