...
.. py:function:: mindspore.ops.hfft2(input, s=None, dim=(-2,-1), norm=None)

    计算具有厄密对称输入的二维快速傅里叶变换。

    .. note::
        - `hfft2` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `hfft2` 尚不支持Windows平台。
    ...
    参数：
        ...
        - **s** (tuple[int], 可选) - 输出在 `dim` 轴的长度。如果给定，则在计算 `hfft2` 之前 `dim[i]` 轴的大小将被零填充或截断至 `s[i]`。
          默认值： ``None`` , 表示无需对 `input` 进行处理。
        - **dim** (tuple[int], 可选) - 指定进行 `hfft2` 的维度。默认值： ``(-2,-1)`` ，表示对 `input` 的最后两个维度进行变换。
    ...
    返回： 
        Tensor， `hfft2` 的结果。如果给定 `s`，则 `dim[-1]` 的大小为 :math:`(s[-1] - 1) * 2` ，
        其余轴 `dim[i]` 的大小改为 `s[i]` 。
