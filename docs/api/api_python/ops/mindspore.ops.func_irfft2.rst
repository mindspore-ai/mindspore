...
.. py:function:: mindspore.ops.irfft2(input, s=None, dim=(-2, -1), norm=None)

    计算输入的二维快速傅里叶逆实值变换。

    .. note::
        - `irfft2` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `irfft2` 尚不支持Windows平台。
    ...
    参数：
        ...
        - **s** (tuple[int], 可选) - 信号长度。如果给定，则在计算 `irfft2` 之前 `dim[i]` 轴的大小将被零填充或截断至 `s[i]`。
          默认值： ``None`` , `dim[-1]` 轴取 :math:`2*(input.shape[dim]-1)` ，其余轴和输入大小相同。
        - **dim** (tuple[int], 可选) - 指定进行 `irfft2` 的维度。默认值： ``(-2,-1)`` ，表示对 `input` 的最后两个维度进行变换。
