...
.. py:function:: mindspore.ops.hfftn(input, s=None, dim=None, norm=None)

    计算具有厄密对称输入的N维快速傅里叶变换。

    .. note::
        - `hfftn` 目前仅用于 `mindscience` 科学计算场景，尚不支持其他使用场景。
        - `hfftn` 尚不支持Windows平台。
    ...
    参数：
        ...
        - **s** (tuple[int], 可选) - 输出在 `dim` 轴的长度。如果给定，则在计算 `hfftn` 之前 `dim[i]` 轴的大小将被零填充或截断至 `s[i]`。
    ...
    返回： 
        Tensor， `hfftn` 的结果。如果给定 `s`，则 `dim[-1]` 的大小为 :math:`(s[-1] - 1) * 2` ，
        其余轴 `dim[i]` 的大小改为 `s[i]` 。
