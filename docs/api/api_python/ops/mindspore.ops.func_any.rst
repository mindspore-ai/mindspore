...
.. py:function:: mindspore.ops.any(input, axis=None, keep_dims=False)
    ...
    参数：
        - **input** (Tensor[bool]) - 输入Tensor，其数据类型为bool型。shape是 :math:`(N, *)` ，其中 :math:`*` 表示任意数量的附加维度。
    ...
    返回：
        ...
        - 如果 `axis` 为int，例如取值为2，并且 `keep_dims` 为False，则输出的shape为 :math:`(input_1, input_3, ..., input_R)` 。
