...
.. py:class:: mindspore.dataset.transforms.Mask(operator, constant, dtype=mstype.bool_)
    ...
    参数：
        - **operator** (:class:`~.transforms.Relational`) - 关系操作符，可以取值为Relational.EQ、Relational.NE、Relational.LT、Relational.GT、Relational.LE、Relational.GE。以Relational.EQ为例，将找出Tensor中与 `constant` 相等的元素。
