...
.. py:class:: mindspore.train.RunContext(original_args)
    ...
    Callback的类方法中，调用 `RunContext.original_args()` 可以获取模型当前的上下文信息，用户也可以为此信息添加额外的自定义属性，同时 `request_stop()` 方法可以控制训练过程的停止。具体用法请查看 `回调机制Callback <https://www.mindspore.cn/tutorials/zh-CN/r2.2/advanced/model/callback.html>`_。
    ...
    .. py:method:: original_args()
        ...
        教程样例：
            - `回调机制 Callback - 自定义回调机制
              <https://mindspore.cn/tutorials/zh-CN/r2.2/advanced/model/callback.html#自定义回调机制>`_
    ...
    .. py:method:: request_stop()
        ...
        教程样例：
            - `回调机制 Callback - 自定义终止训练
              <https://mindspore.cn/tutorials/zh-CN/r2.2/advanced/model/callback.html#自定义终止训练>`_
