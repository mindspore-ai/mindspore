...
.. py:class:: mindspore.dataset.Graph(edges, node_feat=None, edge_feat=None, graph_feat=None, node_type=None, edge_type=None, num_parallel_workers=None, working_mode='local', hostname='127.0.0.1', port=50051, num_client=1, auto_shutdown=True)
    ...
    .. py:method:: get_all_edges(edge_type)
        ...
        参数：
            - **edge_type** (str) - 指定边的类型，Graph初始化未指定 `edge_type` 时，默认值为'0'。详见 `加载图数据集 <https://www.mindspore.cn/tutorials/zh-CN/r1.9/advanced/dataset/augment_graph_data.html>`_ 。
    ...
    .. py:method:: get_all_nodes(node_type)
        ...
        参数：
            - **node_type** (str) - 指定节点的类型。Graph初始化未指定 `node_type` 时，默认值为'0'。详见 `加载图数据集 <https://www.mindspore.cn/tutorials/zh-CN/r1.9/advanced/dataset/augment_graph_data.html>`_ 。
