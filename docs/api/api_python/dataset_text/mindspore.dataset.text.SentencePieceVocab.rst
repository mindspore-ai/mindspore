...
.. py:class:: mindspore.dataset.text.SentencePieceVocab
    ...
    .. py:method:: from_dataset(dataset, col_names, vocab_size, character_coverage, model_type, params)
        ...
        参数：
            ...
            - **model_type** (:class:`~.text.SentencePieceModel`) - 其值可以是 ``SentencePieceModel.UNIGRAM`` 、 ``SentencePieceModel.BPE`` 、 ``SentencePieceModel.CHAR`` 或 ``SentencePieceModel.WORD`` 。使用 ``SentencePieceModel.WORD`` 类型时，必须预先标记输入句子。
