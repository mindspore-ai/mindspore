...
.. py:class:: mindspore.dataset.text.SentencePieceVocab
    ...
    .. py:method:: from_dataset(dataset, col_names, vocab_size, character_coverage, model_type, params)
        ...
        参数：
            ...
            - **model_type** (:class:`~.text.SentencePieceModel`) - 其值可以是SentencePieceModel.UNIGRAM、SentencePieceModel.BPE、SentencePieceModel.CHAR或SentencePieceModel.WORD。默认值：SentencePieceModel.UNIgram。使用SentencePieceModel.WORD类型时，必须预先标记输入句子。
