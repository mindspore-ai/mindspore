...
.. py:function:: mindspore.set_fl_context(**kwargs)
    ...
    参数：
        ...
        - **dp_delta** (float) - DP：差分隐私机制的delta预算，通常等于客户端数量的倒数。 `dp_delta` 越小，隐私保护效果越好。默认值： ``0.01`` 。
        ...
        - **pki_verify** (bool) - 如果为 ``True`` ，则将打开服务器和客户端之间的身份验证。
          还应从https://pki.consumer.huawei.com/ca/下载Root CA证书、Root CA G2证书和移动设备CRL证书。
          需要注意的是，只有当客户端是具有HUKS服务的Android环境时， `pki_verify` 可以为 ``True`` 。默认值： ``False`` 。
