...
.. py:function:: mindspore.dataset.audio.melscale_fbanks(n_freqs, f_min, f_max, n_mels, sample_rate, norm=NormType.NONE, mel_type=MelType.HTK)
    ...
    参数：
        ...
        - **norm** (:class:`~.audio.NormType`, 可选) - 标准化方法，可以是NormType.NONE或NormType.SLANEY。默认值：NormType.NONE。
        - **mel_type** (:class:`~.audio.MelType`, 可选) - 使用的标度，可以是MelType.HTK或MelType.SLANEY。默认值：MelType.HTK。
