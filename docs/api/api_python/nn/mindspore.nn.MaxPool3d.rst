...
.. py:class:: mindspore.nn.MaxPool3d(kernel_size, strides=None, pads=0, dilation=1, return_indices=False, ceil_mode=False)
    ...
    异常：
        ...
        - **TypeError** - `kernel_size` 、 `strides` 、 `pads` 、 `dilation` 既不是整数也不是元组。
        - **ValueError** - `kernel_size` 或者 `strides` 小于1。
        - **ValueError** - `pads` 小于0。
