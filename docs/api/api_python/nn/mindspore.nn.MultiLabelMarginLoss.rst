mindspore.nn.multilabel_margin_loss
======================================

.. py:class:: mindspore.nn.MultiLabelSoftMarginLoss(reduction='mean')
    ...
    输入：
        - **x** (Tensor) - 预测值。shape为 :math:`(C)` 或 :math:`(N, C)`，其中 :math:`N` 为批量大小，:math:`C` 为类别数。数据类型必须为：float16或float32。
