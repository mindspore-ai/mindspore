...
.. py:class:: mindspore.nn.FractionalMaxPool2d(kernel_size, output_size=None, output_ratio=None, return_indices=False, _random_samples=None)
    ...
    参数：
        ...
        - **return_indices** (bool) - 如果为 `True` ，返回分数最大池化的最大值的的索引值。默认值：False。
        - **_random_samples** (Tensor) - 3D张量，分数最大池化的随机步长。支持的数据类型：float16、float32、double。数值介于0到1之间。shape为 :math:`(N, C, 2)` 的Tensor。默认值：None。
