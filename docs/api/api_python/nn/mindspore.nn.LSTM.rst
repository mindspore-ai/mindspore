...
.. py:class:: mindspore.nn.LSTM(*args, **kwargs)
    ...
    长短期记忆（LSTM）网络，根据输出序列和给定的初始状态计算返回输出序列和最终状态。

    在LSTM模型中，有两条管道连接两个连续的细胞，一条是细胞状态管道，另一条是隐藏状态管道。将两个连续的时间节点表示为 :math:`t-1` 和 :math:`t` 。指定在 :math:`t` 时刻输入 :math:`x_t` ，在 :math:`{t-1}` 时刻的隐藏状态 :math:`h_{t-1}` 和细胞状态 :math:`c_{t-1}` 。在 :math:`t` 时刻的细胞状态和隐藏状态使用门控机制计算得到。输入门 :math:`i_t` 计算出候选值。遗忘门 :math:`f_t` 决定是否让上一时刻学到的信息通过或部分通过。输出门 :math:`o_t` 决定哪些信息输出。候选细胞状态 :math:`\tilde{c}_t` 是用当前输入计算的。最后，使用遗忘门、输入门、输出门计算得到当前时刻的细胞状态 :math:`c_{t}` 和隐藏状态 :math:`h_{t}` 。完整的公式如下。
    ...
    **输入：**
    ...
    - **seq_length** (Tensor) - 输入batch的序列长度。Tensor的shape :math:`(\text{batch_size})` 。默认：None。这里输入指明真实的序列长度，以避免使用填充后的元素计算隐藏状态，影响最后的输出。推荐这种输入方法。
    ...
    **异常：**
    ...
    - **TypeError** - `dropout` 既不是float也不是整数。
