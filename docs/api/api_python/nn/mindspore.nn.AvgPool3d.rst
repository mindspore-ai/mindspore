...
.. py:class:: mindspore.nn.AvgPool3d(kernel_size=1, stride=1, pad_mode='valid', padding=0, ceil_mode=False, count_include_pad=True, divisor_override=None)
    ...
    参数：
        - **kernel_size** (Union[int, tuple[int]]) - 指定池化核尺寸大小。如果为正整数，则同时代表池化核的深度，高度和宽度。如果为tuple，其值必须包含三个正整数，分别表示池化核的深度，高度和宽度。默认值：1。
        ...
        - **padding** (Union(int, tuple[int], list[int])) - 池化填充值，只有 `pad_mode` 为"pad"时才能设置为非0。默认值：0。只支持以下情况：
