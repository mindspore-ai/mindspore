...
.. py:class:: mindspore.nn.TransformerDecoder(decoder_layer, num_layers, norm=None)
    ...
    输入：
        - **tgt** (Tensor) - 目标序列。如果目标序列没有batch，shape是 :math:`(T, E)` ；否则如果 `batch_first=False` ，则shape为 :math:`(T, N, E)` ，如果 TransformerDecoderLayer中 `batch_first=True` ，则shape为 :math:`(T, N, E)`。 :math:`(T)` 是目标序列的长度，:math:`(N)` 是batch个数，:math:`(E)` 是特性个数。数据类型：float16、float32或者float64。
        - **memory** (Tensor) - TransformerEncoder的最后一层输出序列。数据类型：float16、float32或者float64。
        - **tgt_mask** (Tensor, 可选) - 目标序列的掩码矩阵。shape是 :math:`(T, T)` 或 :math:`(N*nheads, T, T)` 。其中 `nheads` 是TransformerDecoderLayer中的 `nhead` 参数。数据类型：：float16、float32、float64或者布尔。默认值：``None``。
