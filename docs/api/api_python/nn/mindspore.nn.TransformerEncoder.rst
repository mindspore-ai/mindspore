...
.. py:class:: mindspore.nn.TransformerEncoder(encoder_layer, num_layers, norm=None)
    ...
    输入：
        ...
        - **src_mask** (Tensor, 可选) - 源序列的掩码矩阵。shape是 :math:`(S, S)` 或 :math:`(N*nhead, S, S)` 。其中 `nhead` 是 :class:`mindspore.nn.TransformerEncoderLayer` 中的 `nhead` 参数。数据类型：：float16、float32、float64或者布尔。默认值：``None``。
