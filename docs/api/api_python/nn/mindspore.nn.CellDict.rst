...
.. py:class:: mindspore.nn.CellDict(*args, **kwargs)
    ...
    参数：
        - **args** (iterable，可选) - 一个键值对类型为(string, Cell)的可迭代对象，或者是一个从string到Cell的映射（字典），Cell的类型不应为CellDict，CellList或者SequentialCell。通过类型为string的键可以在CellDict中查找其对应的Cell。
    ...
    .. py:method:: update(cells)
        ...
        参数：
            - **cells** (iterable) - 一个键值对类型为(string, Cell)的可迭代对象，或者是一个从string到Cell的映射（字典），Cell的类型不应为CellDict，CellList或者SequentialCell。
        ...
        异常：
            ...
            - **TypeError** - 如果键值对中Cell的类型是CellDict，CellList或者SequentialCell。
