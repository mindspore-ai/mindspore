...
.. py:class:: mindspore_lite.Model()
    ...
    .. py:method:: build_from_file(model_path, model_type, context=None, config_path="")
        ...
        参数：
            - **model_path** (str) - 定义输入模型文件的路径，例如："/home/user/model.ms"。选项有MindSpore模型: "model.mindir" | MindSpore Lite模型: "model.ms"
            - **model_type** (ModelType) - 定义输入模型文件的类型。选项有ModelType::MINDIR | ModelType::MINDIR_LITE。有关详细信息，请参见 `模型类型 <https://mindspore.cn/lite/api/zh-CN/master/mindspore_lite/mindspore_lite.ModelType.html>`_ 。
