...
.. py:class:: mindspore_lite.Converter()
    ...
    .. py:method:: save_type
        ...
        返回：
            ModelType，导出模型文件的类型。选项有ModelType.MINDIR | ModelType.MINDIR_LITE。推荐转换为MindSpore模型。目前，支持转换为MindSpore Lite模型，但是该选项将会被废弃。有关详细信息，请参见 `模型类型 <https://mindspore.cn/lite/api/zh-CN/master/mindspore_lite/mindspore_lite.ModelType.html>`_ 。
