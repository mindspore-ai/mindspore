...
TEST_P(TestSqrtGrad, dyn_shape) {
  ...
  DoFuncImplInferAndCompare<SqrtGradFuncImpl>(kNameSqrtGrad, {dy, y}, expect_shape, expect_type);
}

namespace {
auto SqrtGradOpTypeCases = testing::ValuesIn({
...
INSTANTIATE_TEST_CASE_P(TestSqrtGradGroup, TestSqrtGrad,
                        testing::Combine(EltwiseGradDynShapeTestCases, SqrtGradOpTypeCases));
}  // namespace ops
}  // namespace mindspore
