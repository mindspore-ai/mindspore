...
namespace mindspore {
namespace ops {
class TestUnsqueeze : public UT::Common {
 public:
  TestUnsqueeze() {}
  void SetUp() {}
  void TearDown() {}
...
/*TEST_F(TestUnsqueeze, test_unsqueeze_1) {
  auto unsqueeze = std::make_shared<Unsqueeze>();
  std::vector<int64_t> axis = {};
  unsqueeze->Init(axis);
  auto tensor_x = std::make_shared<tensor::Tensor>(kNumberTypeFloat16, std::vector<int64_t>{1, 3, 2});
  ...
  *tensor_x_data = 6;
  tensor_x_data++;
  auto abstract = unsqueeze->Infer({tensor_x->ToAbstract()});
