...
TEST_F(TestUnsortedSegmentSum, test_cops_unsortedsegmentsum) {
  auto unsortedsegmentsum = std::make_shared<UnsortedSegmentSum>();
  auto tensor_x = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat32, std::vector<int64_t>{1, 4});
  auto segment_ids = TensorConstructUtils::CreateOnesTensor(kNumberTypeInt32, std::vector<int64_t>{1, 4});
  auto num_segments = MakeValue(4);
  MS_EXCEPTION_IF_NULL(tensor_x);
  MS_EXCEPTION_IF_NULL(segment_ids);
  MS_EXCEPTION_IF_NULL(num_segments);
  auto abstract =
    unsortedsegmentsum->Infer({tensor_x->ToAbstract(), segment_ids->ToAbstract(), num_segments->ToAbstract()});
