...
TEST_P(TestPagedAttentionMask, DynShape) {
 ...
 auto out_dtype = func_impl.InferType(prim, {query, key_cache, value_cache, block_tables, context_lens, alibi_mask, num_head, scale_value, kv_head});
 ASSERT_TRUE(*out_dtype == *expect_type);
 auto out_shape = func_impl.InferShape(prim, {query, key_cache, value_cache, block_tables, context_lens, alibi_mask, num_head, scale_value, kv_head});
 ASSERT_TRUE(*out_shape == *expect_shape);
}

INSTANTIATE_TEST_CASE_P(
  TestPagedAttentionMask, TestPagedAttentionMask,
  testing::Values(
    PagedAttentionMaskShapeParams{{2, 40, 128}, kFloat16,
                                  {256, 16, 40, 128}, kFloat16,
                                  {256, 16, 40, 128}, kFloat16,
                                  {2, 32}, kInt32,
                                  {2}, kInt32,
                                  {2, 40, 1, 1024}, kFloat16,
                                  CreateScalar<int>(40),
                                  CreateScalar<float>(1.0),
                                  CreateScalar<int>(40)},
    PagedAttentionMaskShapeParams{{-1, 40, 128}, kFloat16,
                                  {256, 16, 40, 128}, kFloat16,
                                  {256, 16, 40, 128}, kFloat16,
                                  {-1, 32}, kInt32,
                                  {-1}, kInt32,
                                  {-1, 40, 1, 1024}, kFloat16,
                                  CreateScalar<int>(40),
                                  CreateScalar<float>(1.0),
                                  CreateScalar<int>(40)}
  ));
