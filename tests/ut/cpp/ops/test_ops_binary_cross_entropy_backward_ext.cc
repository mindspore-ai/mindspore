...
namespace mindspore {
namespace ops {
struct BinaryCrossEntropyBackwardExtParams {
...
class TestBinaryCrossEntropyBackwardExt : public TestOps, public testing::WithParamInterface<BinaryCrossEntropyBackwardExtParams> {};

TEST_P(TestBinaryCrossEntropyBackwardExt, dyn_shape) {
  ...
  BinaryCrossEntropyBackwardExtFuncImpl binary_cross_entropy_backward_ext_func_impl;
  auto prim = std::make_shared<Primitive>("BinaryCrossEntropyBackwardExt");
  auto out_dtype = binary_cross_entropy_backward_ext_func_impl.InferType(
    prim, {grad_output, input, target, weight, reduction});
  ASSERT_TRUE(*out_dtype == *expect_dtype);
  auto out_shape = binary_cross_entropy_backward_ext_func_impl.InferShape(
    prim, {grad_output, input, target, weight, reduction});
  ASSERT_TRUE(*out_shape == *expect_shape);
}
// enum Reduction : int64_t {REDUCTION_SUM = 0,MEAN = 1,NONE = 2,};
INSTANTIATE_TEST_CASE_P(TestBinaryCrossEntropyBackwardExt, TestBinaryCrossEntropyBackwardExt,
                        testing::Values(BinaryCrossEntropyBackwardExtParams{{3, 4, 5},
                                                                    kFloat32,
                                                                    {3, 4, 5},
                                                                    kFloat32,
                                                                    {3, 4, 5},
                                                                    ...
                                                                    {3, 4, 5}},
                                        BinaryCrossEntropyBackwardExtParams{{3, 4, 5},
                                                                    kFloat32,
                                                                    {3, 4, 5},
                                                                    kFloat32,
                                                                    {3, 4, 5},
