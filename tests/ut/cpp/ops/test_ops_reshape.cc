...
namespace mindspore {
namespace ops {
class TestReshape : public UT::Common {
 public:
  TestReshape() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestReshape, test_ops_reshape1) {
  ...
  auto input_shape = MakeValue(std::vector<int64_t>{6, 2, 10});
  MS_EXCEPTION_IF_NULL(tensor_x);
  auto abstract = reshape->Infer({tensor_x->ToAbstract(), input_shape->ToAbstract()});
