...
namespace mindspore {
namespace ops {
class TestSub : public UT::Common {
 public:
  TestSub() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestSub, test_ops_sub) {
  ...
  auto sub_abstract = sub->Infer({tensor_x->ToAbstract(), tensor_y->ToAbstract()});
  MS_EXCEPTION_IF_NULL(sub_abstract);
  EXPECT_EQ(sub_abstract->isa<abstract::AbstractTensor>(), true);
  auto shape_ptr = sub_abstract->BuildShape();
  MS_EXCEPTION_IF_NULL(shape_ptr);
  EXPECT_EQ(shape_ptr->isa<abstract::Shape>(), true);
  auto sub_shape = shape_ptr->cast<abstract::ShapePtr>();
  MS_EXCEPTION_IF_NULL(sub_shape);
  auto shape_vec = sub_shape->shape();
  auto type = sub_abstract->BuildType();
