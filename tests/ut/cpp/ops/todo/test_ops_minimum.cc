...
namespace mindspore {
namespace ops {
class TestMinimum : public UT::Common {
 public:
  TestMinimum() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestMinimum, test_ops_minimum1) {
  auto minimum = std::make_shared<Minimum>();
  auto tensor_x1 = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat16, std::vector<int64_t>{2, 3, 4, 5});
  auto tensor_x2 = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat16, std::vector<int64_t>{4, 5});
  MS_EXCEPTION_IF_NULL(tensor_x1);
  MS_EXCEPTION_IF_NULL(tensor_x2);
  auto abstract = minimum->Infer({tensor_x1->ToAbstract(), tensor_x2->ToAbstract()});
