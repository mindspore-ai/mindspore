...
namespace mindspore {
namespace ops {
class TestL2Normalize : public UT::Common {
 public:
  TestL2Normalize() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestL2Normalize, test_ops_l2_normalize1) {
  auto l2_normalize = std::make_shared<L2Normalize>();
  l2_normalize->Init(std::vector<int64_t>{0, 1, 2});
  ...
  auto abstract = l2_normalize->Infer({tensor_x->ToAbstract()});
