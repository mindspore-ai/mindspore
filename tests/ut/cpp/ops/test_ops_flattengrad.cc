...
namespace mindspore {
namespace ops {
class TestFlattenGrad : public UT::Common {
 public:
  TestFlattenGrad() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestFlattenGrad, test_ops_flatten_grad1) {
  auto flatten_grad = std::make_shared<FlattenGrad>();
  auto tensor_x = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat16, std::vector<int64_t>{120});
  auto in_shape = MakeValue(std::vector<int64_t>{2, 3, 4, 5});
  MS_EXCEPTION_IF_NULL(tensor_x);
  auto abstract = flatten_grad->Infer({tensor_x->ToAbstract(), in_shape->ToAbstract()});
