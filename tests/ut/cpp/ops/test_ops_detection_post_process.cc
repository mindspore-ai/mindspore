...
namespace mindspore {
namespace ops {
class TestDetectionPostProcess : public UT::Common {
 public:
  TestDetectionPostProcess() {}
  void SetUp() {}
  void TearDown() {}
};

TEST_F(TestDetectionPostProcess, test_detection_post_process) {
  auto op = std::make_shared<DetectionPostProcess>();
  op->Init(2, {1.0, 2.0}, 5.0, 4.0, 3, 3, 3, 3, false, false);
  auto boxes = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat16, std::vector<int64_t>{2, 3, 4});
  auto scores = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat16, std::vector<int64_t>{2, 3, 4});
  auto anchors = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat16, std::vector<int64_t>{3, 4});
  auto abstract = op->Infer({boxes->ToAbstract(), scores->ToAbstract(), anchors->ToAbstract()});
