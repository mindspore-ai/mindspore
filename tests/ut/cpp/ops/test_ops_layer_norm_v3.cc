...
class TestLayerNormV3 : public TestOps, public testing::WithParamInterface<LayerNormV3OpParams> {};

TEST_P(TestLayerNormV3, layer_norm_v3_dyn_shape) {
  auto primitive = std::make_shared<Primitive>("LayerNormV3");
  ...
  auto infer_impl = std::make_shared<LayerNormV3FuncImpl>();
  ...
  ASSERT_TRUE(*((*infer_types)[2]) == *expect_variance_type);
}

INSTANTIATE_TEST_CASE_P(TestLayerNormV3Group, TestLayerNormV3,
                        testing::Values(LayerNormV3OpParams{{-2},
                                                            kFloat32,
                                                            {3, 4},
                                                            kFloat32,
                                                            {3, 4},
