...
TEST_F(TestEmbeddingLookup, test_ops_embedding_lookup1) {
  auto embedding_lookup = std::make_shared<EmbeddingLookup>();
  embedding_lookup->Init(true);
  auto tensor_param = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat32, std::vector<int64_t>{5, 3});
  auto tensor_indice = TensorConstructUtils::CreateOnesTensor(kNumberTypeInt32, std::vector<int64_t>{2, 3});
  auto offset = MakeValue(4);
  MS_EXCEPTION_IF_NULL(tensor_param);
  MS_EXCEPTION_IF_NULL(tensor_indice);
  MS_EXCEPTION_IF_NULL(offset);
  auto abstract =
    embedding_lookup->Infer({tensor_param->ToAbstract(), tensor_indice->ToAbstract(), offset->ToAbstract()});
