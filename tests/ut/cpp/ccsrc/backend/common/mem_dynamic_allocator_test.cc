...
TEST_F(TestMemDynamicAllocator, test_basic_allocation) {
  ...
  EXPECT_EQ(persitent_mem_pool->mem_bufs_.size(), expected_size_one);
  EXPECT_EQ(persitent_mem_pool->mem_bufs_[std::make_pair(kDefaultStreamId, DynamicMemBufStatus::kMemBufIdle)].size(),
            expected_size_one);
  ...
  EXPECT_EQ(common_mem_pool->mem_bufs_.size(), expected_size_one);
  EXPECT_EQ(common_mem_pool->mem_bufs_[std::make_pair(kDefaultStreamId, DynamicMemBufStatus::kMemBufIdle)].size(),
            expected_size_one);
