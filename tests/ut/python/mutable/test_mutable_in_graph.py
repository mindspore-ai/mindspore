...
def test_mutable_wrong_input_out_of_graph():
    ...
    assert "For 'mutable', the 'input_data' should be one of (bool, int, float, Tensor, "\
           "tuple, list, dict) or their nested structures, but got" in str(ex3.value)
...
def test_mutable_wrong_input_nested():
    """
    Feature: create and return mutable object.
    Description: test mutable input out of graph.
    Expectation: raise exception.
    ...
    with pytest.raises(RuntimeError) as ex:
        mutable(input_list)
    assert "For 'mutable', the input list nesting is more than "\
        in str(ex.value)
...
def test_mutable_input_with_dict():
    """
    Feature: Set Constants mutable.
    Description: Set mutable for dict.
    Expectation: No Exception.
    """
    @jit
    def foo(dict1):
        dict2 = mutable({'a': Tensor([[0.5, 0.6, 0.4], [1.2, 1.3, 1.1]], dtype=mstype.float32),
                         'b': (2,),
                         'c': True})
        out = [dict1, dict2, \
               F.isconstant(dict1['a']), F.isconstant(dict1['b']), F.isconstant(dict1['c']),
               F.isconstant(dict2['a']), F.isconstant(dict2['b']), F.isconstant(dict2['c'])]
        return out

    dict1 = mutable({'a': Tensor([[0.5, 0.6, 0.4], [1.2, 1.3, 1.1]], dtype=mstype.float32),
                     'b': (2,),
                     'c': True})
    expect = (Tensor([[0.5, 0.6, 0.4], [1.2, 1.3, 1.1]], dtype=mstype.float32), (2,), True)
    os.environ['MS_DEV_JIT_SYNTAX_LEVEL'] = '0'
    out = foo(dict1)
    assert isinstance(out[0], dict) and dict_compare(out[0], dict1)
    assert isinstance(out[1], tuple) and seq_compare(out[1], expect)
    for ele in out[3:]:
        assert not ele
