...
from mindspore import context, Tensor, Parameter, ops, Symbol
from mindspore.nn import Cell
from mindspore.ops import operations as P
from mindspore.ops.auto_generate import MatMulExt, BatchMatMulExt
...
class BatchMatMulExtNet(Cell):
    def __init__(self, strategy1=None, strategy2=None):
        super().__init__()
        self.w = Parameter(Tensor(np.ones([1, 8, 16]), dtype=ms.float32), "w1")
        self.matmul = BatchMatMulExt().shard(strategy1)
        self.relu = P.ReLU().shard(strategy2)
    def construct(self, x):
        out = self.matmul(x, self.w)
        out = self.relu(out)
...
def test_batch_matmul_ext_dynamic():
    """
    Feature: test dynamic shape for batch matmul ext
    Description: no redistribution
    Expectation: compile success
    """
    context.set_auto_parallel_context(parallel_mode="semi_auto_parallel", device_num=8, global_rank=0)
    ...
    net = BatchMatMulExtNet(strategy1, strategy2)

    s = Symbol(divisor=8)
    input_x = Tensor(shape=[1, s, 8], dtype=ms.float32)
    net.set_inputs(input_x)

    phase = compile_net(net, input_x)
    validator = ParallelValidator(net, phase)
    assert validator.check_node_inputs('ReLU-0', ['AllReduce-0'])
    assert validator.check_node_inputs('AllReduce-0', ['BatchMatMulExt-0'])
    assert validator.check_parameter_shape("w1", [1, 2, 16])
