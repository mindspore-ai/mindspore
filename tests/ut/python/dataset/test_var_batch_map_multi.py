...
def test_variable_size_batch():
    ...
    def test_batch_repeat_with_copy_map(gen_num, r, drop, func):
        ...
        data1 = ds.GeneratorDataset((lambda: gen(gen_num)), ["num"]) \
            .batch(batch_size=func, drop_remainder=drop, input_columns=["num"], per_batch_map=simple_copy,
                   python_multiprocessing=True).repeat(r)
        for item in data1.create_dict_iterator(num_epochs=1, output_numpy=True):
            res.append(item["num"])
        return res
...
def test_basic_batch_map():
    ...
    def batch_map_config(num, r, batch_size, func, res):
        data1 = ds.GeneratorDataset((lambda: gen(num)), ["num"]) \
            .batch(batch_size=batch_size, input_columns=["num"], per_batch_map=func,
                   python_multiprocessing=True).repeat(r)
        for item in data1.create_dict_iterator(num_epochs=1, output_numpy=True):
            res.append(item["num"])
...
def test_batch_multi_col_map():
    ...
    def batch_map_config(num, r, batch_size, func, col_names, res):
        data1 = ds.GeneratorDataset((lambda: gen(num)), ["num", "num_square"]) \
            .batch(batch_size=batch_size, input_columns=col_names, per_batch_map=func,
                   python_multiprocessing=True).repeat(r)
        for item in data1.create_dict_iterator(num_epochs=1, output_numpy=True):
...
def test_var_batch_multi_col_map():
    ...
    def batch_map_config(num, r, fbatch, fmap, col_names, res):
        data1 = ds.GeneratorDataset((lambda: gen_3_cols(num)), ["col1", "col2", "col3"]) \
            .batch(batch_size=fbatch, input_columns=col_names, per_batch_map=fmap, python_multiprocessing=True) \
            .repeat(r)
        for item in data1.create_dict_iterator(num_epochs=1, output_numpy=True):
    ...
    def batch_map_config(num, s, f, in_nms, out_nms, col_order=None):
        try:
            ...
            dst = dst.batch(batch_size=s, input_columns=in_nms, output_columns=out_nms, per_batch_map=f,
                            column_order=col_order, python_multiprocessing=True)
            res = []
...
def skip_test_performance():
    def trans(images, batchInfo):
        start_time = time.time()
        print(os.getppid(), batchInfo.get_batch_num(), time.strftime("%H:%M:%S", time.localtime()))
        for _ in range(50):
            op = Compose([py_vision.Decode(), py_vision.Resize(20), py_vision.ToTensor()])
            images2 = [op(img) for img in images]
        end_time = time.time()
        print(os.getppid(), time.strftime("%H:%M:%S", time.localtime()), end_time - start_time)
        return (images2,)

    def trans2(img):
        start_time = time.time()
        img2 = None
        print(os.getppid(), time.strftime("%H:%M:%S", time.localtime()))
        for _ in range(50):
            op = Compose([py_vision.Decode(), py_vision.Resize(20), py_vision.ToTensor()])
            img2 = op(img)
        end_time = time.time()
    ...
    print(os.getppid())
    data = ds.ImageFolderDataset(IMAGENET_RAWDATA_DIR, shuffle=False).repeat(10)
    print(data.get_dataset_size())
    data = data.batch(1, per_batch_map=trans, input_columns=["image"], num_parallel_workers=12,
                      python_multiprocessing=True)
    data = data.map(operations=trans2, num_parallel_workers=8, python_multiprocessing=False)
    start = time.time()
    for _ in data:
        pass
    end = time.time()
