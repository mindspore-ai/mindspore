...
def test_embedding_cache_distribute_ascend():
    """
    Feature: Test embedding cache feature on ascend with 4 workers, 2 servers.
    ...
    return_code = os.system(f"bash {self_path}/run_test_embedding_cache_distribute.sh Ascend 4 127.0.0.1 8066 0")
    if return_code != 0:
        os.system(f"echo '\n**************** Worker Log ****************'")
