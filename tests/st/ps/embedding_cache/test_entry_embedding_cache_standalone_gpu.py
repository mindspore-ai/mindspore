...
def test_embedding_cache_standalone_gpu():
    """
    Feature: Test embedding cache feature on gpu with 1 worker, 1 server.
    Description: Worker trains network containing embedding layers and enable embedding cache.
    Expectation: All process execute and exit normal.
    """

    self_path = os.path.split(os.path.realpath(__file__))[0]
    return_code = os.system(f"bash {self_path}/run_test_embedding_cache_standalone.sh GPU 127.0.0.1 8033")
    if return_code != 0:
        os.system(f"echo '\n**************** Worker Log ****************'")
