...
def test_embedding_cache_distribute_sparse_gpu():
    """
    Feature: Test embedding cache feature on gpu with 4 workers, 2 servers, enable sparse and
            perform id deduplication in the forward phase.
    ...
    return_code = os.system(f"bash {self_path}/run_test_embedding_cache_distribute.sh GPU 4 127.0.0.1 8099 1")
    if return_code != 0:
        os.system(f"echo '\n**************** Worker Log ****************'")
