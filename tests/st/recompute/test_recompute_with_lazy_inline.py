...
def run_testcase(testcase_name, expect_memory_usage):
    ...
    cmd = f"export GLOG_v=1; export MS_DEV_CELL_REUSE=2; pytest -s test_recompute.py::" + testcase_name + " > " \
          + log_filename + " 2>&1"
...
def test_recompute_op_recompute1():
    ...
    Expectation: Run successfully and the memory usage is reduced.
    """
    run_testcase("test_recompute_op_recompute1", 57)
...
def test_recompute_op_recompute2():
    ...
    Expectation: Run successfully and the memory usage is reduced.
    """
    run_testcase("test_recompute_op_recompute2", 40)
...
def test_recompute_cell_and_op_recompute1():
    ...
    Expectation: Run successfully and the memory usage is reduced.
    """
    run_testcase("test_recompute_cell_and_op_recompute1", 57)
...
def test_recompute_cell_and_op_recompute2():
    ...
    Expectation: Run successfully and the memory usage is reduced.
    """
    run_testcase("test_recompute_cell_and_op_recompute2", 69)
