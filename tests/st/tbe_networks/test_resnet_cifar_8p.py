...
def create_dataset(repeat_num=1, training=True, batch_size=32, rank_id=0, rank_size=1,
                   enable_hccl=False, dry_run=False):
...
def train_process(q, device_id, epoch_size, num_classes, device_num, batch_size, enable_hccl, dry_run):
    ...
    dataset = create_dataset(epoch_size, training=True,
                             ...
                             enable_hccl=enable_hccl, dry_run=dry_run)
    batch_num = dataset.get_dataset_size()
    config_ck = CheckpointConfig(
        save_checkpoint_steps=batch_num * 5, keep_checkpoint_max=10)
    ckpoint_cb = ModelCheckpoint(
        prefix="train_resnet_cifar10_device_id_" + str(device_id), directory="./", config=config_ck)
    loss_cb = LossGet()
...
def test_resnet_cifar_8p():
    ...
    enable_hccl = False
    dry_run = True
    dry_run_process = Process(target = train_process,
                      args=(q, device_id, epoch_size, num_classes, device_num, batch_size, enable_hccl, dry_run))
    dry_run_process.start()
    dry_run_process.join()
    q.get()

    # Start 8 parallel training
    time.sleep(5)
    device_num = 8
    epoch_size = 1
    enable_hccl = True
