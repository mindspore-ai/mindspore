...
from mindspore.experimental.optim import Adadelta
...
class AdadeltaFactory():
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.Adadelta(model.parameters(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.1, 'lr': 0.9, "rho": 0.9, "eps": 1e-7},
                            {'params': no_bias_params, 'lr': 0.66, "rho": 0.88}]
            optimizer = torch.optim.Adadelta(params=group_params, lr=self.lr)
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = Adadelta(model.trainable_params(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.1, 'lr': 0.9, "rho": 0.9, "eps": 1e-7},
                            {'params': no_bias_params, 'lr': 0.66, "rho": 0.88}]
            optimizer = Adadelta(params=group_params, lr=self.lr)
...
def test_adadelta_basic(mode):
    """
    Feature: Test adadelta.
    Description: Test adadelta with default parameter.
    Expectation: success.
    """
...
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adadelta_group(mode):
    """
    Feature: Test adadelta.
    Description: Test adadelta with grouped params.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
...
@arg_mark(plat_marks=['platform_gpu', 'platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adadelta_lr_dynamic(mode):
    """
    Feature: Test adadelta.
    Description: Test adadelta when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
...
@arg_mark(plat_marks=['platform_gpu', 'platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adadelta_group_lr_dynamic(mode):
    """
    Feature: Test adadelta.
    Description: Test adadelta with grouped params when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
...
def test_adadelta_group_lr_dynamic_change_param(mode):
    """
    Feature: Test adadelta.
    Description: Test adadelta with grouped params when optimizer params are changed.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
