...
from mindspore.experimental.optim import ASGD
...
class ASGDFactory():
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.ASGD(model.parameters(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.0, 'lr': 0.9, "alpha": 0.9, "lambd": 0.02},
                            {'params': no_bias_params, 'lr': 0.66, "t0": 1e5}]
            optimizer = torch.optim.ASGD(params=group_params, lr=self.lr)
        ...
        for _ in range(self.epochs):
            ...
            if self.if_change:
                ...
                optimizer.param_groups[1]["lambd"] = 0.003
        output = model(data)
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = ASGD(model.trainable_params(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.0, 'lr': 0.9, "alpha": 0.9, "lambd": 0.02},
                            {'params': no_bias_params, 'lr': 0.66, "t0": 1e5}]
            optimizer = ASGD(params=group_params, lr=self.lr)
...
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_asgd_group(mode):
    """
    Feature: Test asgd.
    Description: Test asgd with grouped params.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = ASGDFactory(True, False)
    fact.result_cmp()
...
@arg_mark(plat_marks=['platform_gpu', 'platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_asgd_lr_dynamic(mode):
    """
    Feature: Test asgd.
    Description: Test asgd when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = ASGDFactory(False, True)
    fact.result_cmp()
...
@arg_mark(plat_marks=['platform_gpu', 'platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_asgd_group_lr_dynamic(mode):
    """
    Feature: Test asgd.
    Description: Test asgd with grouped params when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = ASGDFactory(True, True)
    fact.result_cmp()
...
def test_asgd_group_lr_dynamic_change_param(mode):
    """
    Feature: Test asgd.
    Description: Test asgd with grouped params when optimizer params are changed.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = ASGDFactory(True, True, True)
    fact.result_cmp()
