...
def test_lstsqv2_grad_dynamic_shape(jit_level):
    """
    ...
    Dout2 = generate_random_input((6, 7), np.float32)

    inputs1 = [ms.Tensor(Dout1), ms.Tensor(A1), ms.Tensor(B1)]
    inputs2 = [ms.Tensor(Dout2), ms.Tensor(A2), ms.Tensor(B2)]

    TEST_OP(lstsqv2_grad_forward_func, [inputs1, inputs2], grad=False, jit_level=jit_level)
