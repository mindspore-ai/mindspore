...
def test_tracev2_grad_dynamic_shape(jit_level):
    """
    ...
    Dout2 = generate_random_input((8,), np.float32)

    inputs1 = [ms.Tensor(Dout1), ms.Tensor(A1.shape)]
    inputs2 = [ms.Tensor(Dout2), ms.Tensor(A2.shape)]

    TEST_OP(tracev2_grad_forward_func, [
        inputs1, inputs2], grad=False, jit_level=jit_level)
