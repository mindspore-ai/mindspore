...
def np_gen_int4_data_pergroup(group_size=2):
    """
    gen fp16_activation and int4_weight for test
    :param scale: scale for quant
    ...
    np_x = np.random.rand(8, 8).astype(np.float16)
    np_weight = np.linspace(-0.64, 0.64, 64).astype(np.float16).reshape((8, 8))
    scale = np.random.rand(group_size, 8).astype(np.float16)
    offset = np.random.rand(group_size, 8).astype(np.float16)
    np_quant_int8_data, np_quant_int4_data = np_quant_int4_pergroup(np_weight, scale, offset)
    return np_x, np_quant_int8_data, np_quant_int4_data, scale, offset
...
def test_ms_int4_weight_quant_pergroup_1p_GE(mode):
    ...
    np_x = np.random.rand(batch_seq, channel_in).astype(np.float16)
    np_quant_fp16_data, np_quant_int4_data, antiquant_scale, antiquant_offset = \
        np_quant_int4_pergroup_data_gen(channel_in, channel_out, group_num)
    expect = np.matmul(np_x, np_quant_fp16_data)
    ms_int4_weight = Parameter(Tensor(np_quant_int4_data, dtype=mstype.qint4x2))
    quant_scale = None
    quant_offset = None
    antiquant_scale = Tensor(antiquant_scale)
