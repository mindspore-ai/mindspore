...
def test_bucketize_4x5_float64():
    ...
    """
    x_np = np.array([[0.42987306, 0.02847828, 0.59385591, 0.7040952, 0.27390435],
                     [0.32904094, 0.63063352, 0.70752448, 0.24763578, 0.99662956],
                     [0.66478424, 0.70580542, 0.92749155, 0.72736302, 0.24973136],
                     [0.79918445, 0.68613469, 0.9526593, 0.12412648, 0.15175918]]).astype(np.float64)
    boundaries = [-1.743, -1.397, -0.702, -0.4631, -0.146, 0.0859, 0.814, 0.836, 1.704, 2.392]
    net = BucketizeNet(boundaries)
    output_ms = net(Tensor(x_np))
    expect_output = np.array([[6, 5, 6, 6, 6],
                              [6, 6, 6, 6, 8],
                              [6, 6, 8, 6, 6],
                              [6, 6, 8, 6, 6]])
