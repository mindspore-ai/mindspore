...
@test_utils.run_with_cell
def unique_forward_func_dynamic(inputx, is_sorted=True):
    return unique(inputx, is_sorted, True, True, 1)
...
def test_ops_unique_dynamic_shape(context_mode):
    """
    Feature: pyboost function.
    ...
    x2 = ms.Tensor(generate_random_input((7, 8, 9)))
    sorted2 = False

    test_cell = test_utils.to_cell_obj(unique_forward_func_dynamic)
    TEST_OP(test_cell, [[x1, sorted1], [x2, sorted2]], grad=False, jit_level="O0")
