...
def index_select_forward_func(input_x, axis, index):
    return index_select(input_x, axis, index)


@test_utils.run_with_cell
def index_select_backward_func(input_x, axis, index):
    return ops.grad(index_select_forward_func, (0))(input_x, axis, index)
...
def test_func_index_select_normal(context_mode):
    """
    Feature: pyboost function.
    Description: test function index_select normal.
    ...
    output = index_select_forward_func(ms.Tensor(input_np), axis, ms.Tensor(index_np))
    ...
    grad = index_select_backward_func(ms.Tensor(input_np1), axis1, ms.Tensor(index_np1))
...
@pytest.mark.platform_arm_ascend910b_training
@pytest.mark.parametrize('context_mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_func_index_select_dynamic(context_mode):
    """
    Feature: pyboost function.
    ...
    if context_mode == ms.GRAPH_MODE:
        TEST_OP(index_select, [[input1, axis1, index1], [input2, axis2, index2]], mode=context_mode, grad=True,
                jit_level="O0")
    else:
        TEST_OP(index_select, [[input1, axis1, index1], [input2, axis2, index2]], mode=context_mode, grad=True)
