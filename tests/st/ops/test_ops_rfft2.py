...
from mindspore.ops import rfft2


class RFFT2Net(nn.Cell):
    def __init__(self):
        super(RFFT2Net, self).__init__()
        self.rfft2 = rfft2

    def construct(self, x, s, dim):
        return self.rfft2(x, s, dim)


class RFFT2GradNet(nn.Cell):
    def __init__(self, net, dout):
        super(RFFT2GradNet, self).__init__()
        self.net = net
        self.dout = dout
...
def test_ops_rfft2_forward(mode):
    """
    Feature: ops.rfft2
    Description: test function rfft2 forward.
    Expectation: success
    """
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_rfft2_backward(mode):
    """
    Feature: ops.rfft2
    Description: test function rfft2 backward.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
    ...
    net = RFFT2Net()
    grad_net = RFFT2GradNet(net, ms.Tensor(dout))
    grad_net.set_train()
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_rfft2_forward_dynamic_shape(mode):
    """
    Feature: ops.rfft2
    Description: test function rfft2 forward with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_rfft2_forward_dynamic_rank(mode):
    """
    Feature: ops.rfft2
    Description: test function rfft2 forward with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_rfft2_backward_dynamic_shape(mode):
    """
    Feature: ops.rfft2
    Description: test function rfft2 backward with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    ...
    grad_net = RFFT2GradNet(net, ms.Tensor(dout1))
    grad_net.set_train()
    ...
    grad_net = RFFT2GradNet(net, ms.Tensor(dout2))
    grad_net.set_train()
    grad_net.set_inputs(x_dyn, n_dyn, dim_dyn)
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_rfft2_backward_dynamic_rank(mode):
    """
    Feature: ops.rfft2
    Description: test function rfft2 backward with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
