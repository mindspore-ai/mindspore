...
def get_output_backward(reduction):
    output_sum = np.array([[-5.8785, 0.1493, 0.406], [0.1282, 0.32, -3.9683], [0.2045, -1.5325, 1.3793]])
    output_mean = np.array([[-0.6532, 0.0166, 0.0451], [0.01425, 0.03555, -0.4409], [0.0227, -0.1703, 0.1533]])
    output_none = np.array([[-5.8785, 0.1493, 0.406], [0.1282, 0.32, -3.9683], [0.2045, -1.5325, 1.3793]])
    output = {"mean": output_mean, "sum": output_sum, "none": output_none}
    return output[reduction]
...
def test_ops_binary_cross_entropy_dynamic_shape(context_mode, reduction):
    ...
    TEST_OP(test_cell, [[x1, target1, weight1, reduction], [x2, target2, weight2, reduction]], grad=True,
            jit_level="O0")
