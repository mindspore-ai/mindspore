...
from mindspore.ops import ihfft
...
class IHFFTNet(nn.Cell):
    ...
    def construct(self, x, n, dim):
        return self.ihfft(x, n, dim)

class IHFFTGradNet(nn.Cell):
    def __init__(self, net, dout):
        super(IHFFTGradNet, self).__init__()
        self.net = net
        self.dout = dout
...
def generate_expect_forward_output(x, n, dim):
    return np.fft.ihfft(x, n, dim)


def generate_expect_backward_output(x, dout, n, dim):
    dout = np.fft.irfft(dout, n, dim)
...
def test_ops_ihfft_forward(mode):
    """
    Feature: ops.ihfft
    Description: test function ihfft forward.
    Expectation: success
    """
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft_backward(mode):
    """
    Feature: ops.ihfft
    Description: test function ihfft backward.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    n = 2
    ...
    net = IHFFTNet()
    grad_net = IHFFTGradNet(net, ms.Tensor(dout))
    grad_net.set_train()
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft_forward_dynamic_shape(mode):
    """
    Feature: ops.ihfft
    Description: test function ihfft forward with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    n = 2
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft_forward_dynamic_rank(mode):
    """
    Feature: ops.ihfft
    Description: test function ihfft forward with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    n = 2
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft_backward_dynamic_shape(mode):
    """
    Feature: ops.ihfft
    Description: test function ihfft backward with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    ...
    grad_net = IHFFTGradNet(net, ms.Tensor(dout1))
    grad_net.set_train()
    ...
    grad_net = IHFFTGradNet(net, ms.Tensor(dout2))
    grad_net.set_train()
    grad_net.set_inputs(x_dyn, n_dyn, dim_dyn)
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft_backward_dynamic_rank(mode):
    """
    Feature: ops.ihfft
    Description: test function ihfft backward with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    n = 2
