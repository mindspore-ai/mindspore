...
def test_take_along_dim(mode):
    ...
    Description: Verify the result of take_along_dim when all the input value is valid
    Expectation: success
    """
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_take_along_dim_none_dim(mode):
    """
    Feature: take_along_dim
    Description: Verify the result of take_along_dim when dim is none
    Expectation: success
    """
    ms.set_context(mode=mode)
    net = Net()
