...
def test_ops_trace_forward_dynamic(jit_level):
    """
    Feature: numpy.trace
    ...
    A2 = generate_random_input((3, 6, 8), np.float32)
    offset2 = 0
    axis3 = -1
    axis4 = 0
...
@pytest.mark.env_onecard
@pytest.mark.parametrize('jit_level', ["O0", "O2"])
def test_ops_trace_backward_dynamic(jit_level):
    """
    Feature: numpy.trace
    Description: test function trace backward with dynamic input.
    Expectation: success
    """
    A1 = generate_random_input((6, 5, 5), np.float32)
    offset1 = 0
