...
def test_xlogy_dynamic_shape(mode):
    """
    Feature: pyboost function.
    ...
    x2 = Tensor(generate_random_input((2, 3, 4), np.float32))
    y2 = Tensor(np.abs(generate_random_input((2, 3, 1), np.float32)) + 0.01)

    test_cell = test_utils.to_cell_obj(xlogy_forward_func)
    TEST_OP(test_cell, [[x1, y1], [x2, y2]], grad=True, jit_level="O0")
