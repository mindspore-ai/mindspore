...
class EmbeddingLookup(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.embedding_table = ms.Parameter(ms.common.initializer.initializer(
            ms.common.initializer.TruncatedNormal(config.initializer_range),
            [config.vocab_size, config.embedding_size]), name='embedding_table')
...
class EmbeddingPostprocessor(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.full_position_embeddings = ms.Parameter(ms.common.initializer.initializer(
            ms.common.initializer.TruncatedNormal(config.initializer_range),
...
class AlbertOutput(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.dense = nn.Dense(config.hidden_size, config.hidden_size,
                              weight_init=ms.common.initializer.TruncatedNormal(
                                  config.initializer_range)).to_float(config.compute_type)
        self.dropout = nn.Dropout(1 - config.hidden_dropout_prob)
        self.add = ops.Add()
        self.is_gpu = ms.get_context('device_target') == "GPU"
        if self.is_gpu:
            self.layernorm = nn.LayerNorm((config.hidden_size,)).to_float(ms.float32)
...
class RelaPosEmbeddingsGenerator(nn.Cell):
    ...
    def __init__(self,
        ...
        self.embeddings_table = ms.Parameter(
            ms.common.initializer.initializer(ms.common.initializer.TruncatedNormal(initializer_range),
...
class SaturateCast(nn.Cell):
    ...
    Args:
        src_type (:class:`mindspore.dtype`): The type of the elements of the input tensor. Default: ms.float32.
        dst_type (:class:`mindspore.dtype`): The type of the elements of the output tensor. Default: ms.float32.
    """
...
class AlbertAttention(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.use_relative_positions = config.use_relative_positions
        self.scores_mul = ms.Tensor([1.0 / math.sqrt(float(self.size_per_head))], dtype=config.compute_type)
        self.reshape = ops.Reshape()
        self.shape_from_2d = (-1, config.hidden_size)
        self.shape_to_2d = (-1, config.hidden_size)
        weight = ms.common.initializer.TruncatedNormal(config.initializer_range)
        ...
        self.multiply_data = ms.Tensor([-10000.0], dtype=config.compute_type)
...
class AlbertEncoderCell(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.intermediate = nn.Dense(
            ...
            weight_init=ms.common.initializer.TruncatedNormal(config.initializer_range)
...
class AlbertModelCLS(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.cast = ops.Cast()
        self.weight_init = ms.common.initializer.TruncatedNormal(config.initializer_range)
