...
def test_erf_vmap():
    ...
    Expectation: expect correct result.
    """
    x = Tensor(np.arange(6).reshape(1, 1, 3, 2), ms.float32)
    nest_vmap = ops.vmap(ops.vmap(erf_backward_func, in_axes=-1), in_axes=-1)
    output = nest_vmap(x)
    expect = np.array([[[[1.1283791]], [[2.0666985e-02]], [[1.2698234e-07]]],
                       [[[4.1510752e-01]], [[1.3925304e-04]], [[1.5670867e-11]]]])
    assert np.allclose(output.asnumpy(), expect, rtol=0.001)
