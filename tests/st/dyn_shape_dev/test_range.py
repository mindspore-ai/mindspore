...
def test_range_forward(mode):
    ...
    not including "limit".
    """
    context.set_context(mode=mode, precompile_only=True)
    start = 0
    limit = 10
    delta = 2
    output = range_forward_func(start, limit, delta)
    print("output:", output)
    # expect_output = np.array([0., 2., 4., 6., 8.]).astype(np.float32)
...
def test_range_backward(mode):
    ...
    """
    context.set_context(mode=mode, precompile_only=False)
    start = 0
    limit = 10
    delta = 2
