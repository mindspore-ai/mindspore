...
class AddedToken:
    ...
    def __setstate__(self, state):
        content = state.pop("content", None)
        single_word = state.pop("single_word", None)
        lstrip = state.pop("lstrip", None)
        rstrip = state.pop("rstrip", None)
        special = state.pop("special", None)
        normalized = state.pop("normalized", None)
        self.content = content if content is not None else self.content
        self.single_word = single_word if single_word is not None else self.single_word
        self.lstrip = lstrip if lstrip is not None else self.lstrip
        self.rstrip = rstrip if rstrip is not None else self.rstrip
        self.special = special if special is not None else self.special
        self.normalized = normalized if normalized is not None else self.normalized
        ...
        if truncation_strategy == TruncationStrategy.ONLY_FIRST or (
            ...
            if len(ids) > num_tokens_to_remove:
                ...
                if truncation_strategy == TruncationStrategy.ONLY_FIRST:
                    error_msg = (
                        error_msg + f"Please select another truncation strategy than {truncation_strategy}, "
