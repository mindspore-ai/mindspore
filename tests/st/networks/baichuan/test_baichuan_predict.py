...
def test_baichuan_1p_bs1():
    """
    Feature: kbk predict
    Description: test_baichuan_1p_bs1
    Expectation: AssertionError
    """
    test_case = "test_baichuan_1p_bs1"
    device_id = os.getenv('DEVICE_ID', '0')
    os.environ["ASCEND_RT_VISIBLE_DEVICES"] = device_id
    sh_path = os.path.split(os.path.realpath(__file__))[0]
    ret = os.system(
        f"bash {sh_path}/mpirun_launch.sh {sh_path}/configs/predict_baichuan2.yaml 1 {test_case}")
    log_path = f"{sh_path}/{test_case}.log"
    os.system(f"cat {log_path}")
    assert ret == 0

    expect_peak_memory = 4422
    peak_memory = get_num_from_log(f"{log_path}", PEAK_MEMORY_NAME)
    assert peak_memory <= expect_peak_memory * (1 + TOELERANCE)


@arg_mark(plat_marks=['platform_ascend910b'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
def test_baichuan_1p_bs4():
    """
    Feature: kbk predict
    Description: test_baichuan_1p_bs4
    Expectation: AssertionError
    """
    test_case = "test_baichuan_1p_bs4"
    device_id = os.getenv('DEVICE_ID', '0')
    os.environ["ASCEND_RT_VISIBLE_DEVICES"] = device_id
    sh_path = os.path.split(os.path.realpath(__file__))[0]
    ret = os.system(
...
def test_baichuan_4p_bs1():
    """
    Feature: kbk predict
    Description: test_baichuan_4p_bs1
    Expectation: AssertionError
    """
    test_case = "test_baichuan_4p_bs1"
    sh_path = os.path.split(os.path.realpath(__file__))[0]
    ret = os.system(
...
@arg_mark(plat_marks=['platform_ascend910b'], level_mark='level1', card_mark='allcards', essential_mark='unessential')
def test_baichuan_4p_bs4_bf16():
    """
    Feature: kbk predict
    Description: test_baichuan_4p_bs4_bf16
    Expectation: AssertionError
    """
    test_case = "test_baichuan_4p_bs4_bf16"
    sh_path = os.path.split(os.path.realpath(__file__))[0]
    ret = os.system(
        f"bash {sh_path}/mpirun_launch.sh {sh_path}/configs/predict_baichuan2.yaml 4 {test_case}")
    log_path = f"{sh_path}/{test_case}.log"
