...
def test_resize_nearest_neighbor_grad_op_forward(context_mode, data_type):
    """
    Feature: Ops.
    Description: test op resize_nearest_neighbor_grad forward.
    ...
    size = (2, 2)
    align_corners = False
    out = resize_nearest_neighbor_grad_forward_func(x, size, align_corners)
...
@arg_mark(plat_marks=['platform_gpu', 'cpu_linux'], level_mark='level1', card_mark='onecard',
          essential_mark='unessential')
@pytest.mark.parametrize("context_mode", [ms.GRAPH_MODE])
def test_resize_nearest_neighbor_grad_op_dynamic(context_mode):
    """
    Feature: test dynamic tensor of resize_nearest_neighbor_grad
    ...
    size = (2, 2)
    align_corners = True
    test_cell = test_utils.to_cell_obj(resize_nearest_neighbor_grad_dyn_shape_func)
    test_cell.set_inputs(x_dyn, size, align_corners)
    output = test_cell(x1, size, align_corners)
    expect_out = resize_nearest_neighbor_grad_forward_func(x1, size, align_corners)
    np.testing.assert_allclose(output.asnumpy(), expect_out.asnumpy(), rtol=1e-4)

    x2 = ms.Tensor(np.array([[[[4., 2., 3.], [2., 2., 3.]]]]).astype(np.float32))
    output = test_cell(x2, size, align_corners)
    expect_out = resize_nearest_neighbor_grad_forward_func(x2, size, align_corners)
    np.testing.assert_allclose(output.asnumpy(), expect_out.asnumpy(), rtol=1e-4)
